...
const stdlib_renames = std.StaticStringMap([]const u8).init_comptime(.{
    // Most 64-bit archs.
...
pub fn main() !void {
    ...
    var buf_out = std.io.buffered_writer(std.io.get_std_out().writer());
    ...
    const linux_dir = try std.fs.open_dir_absolute(linux_path, .{});
    ...
    {
        try writer.write_all("pub const X86 = enum(usize) {\n");

        const table = try linux_dir.read_file("arch/x86/entry/syscalls/syscall_32.tbl", buf);
        var lines = mem.tokenize_scalar(u8, table, '\n');
        while (lines.next()) |line| {
            if (line[0] == '#') continue;

            var fields = mem.tokenize_any(u8, line, " \t");
            ...
            try writer.print("    {p} = {s},\n", .{ zig.fmt_id(name), number });
        }

        try writer.write_all("};\n\n");
    }
    {
        try writer.write_all("pub const X64 = enum(usize) {\n");

        const table = try linux_dir.read_file("arch/x86/entry/syscalls/syscall_64.tbl", buf);
        var lines = mem.tokenize_scalar(u8, table, '\n');
        while (lines.next()) |line| {
            if (line[0] == '#') continue;
    ...
    }
    {
        ...
        const table = try linux_dir.read_file("arch/arm/tools/syscall.tbl", buf);
        var lines = mem.tokenize_scalar(u8, table, '\n');
        while (lines.next()) |line| {
            if (line[0] == '#') continue;
    ...
    {
        try writer.write_all("pub const Sparc64 = enum(usize) {\n");
        const table = try linux_dir.read_file("arch/sparc/kernel/syscalls/syscall.tbl", buf);
        var lines = mem.tokenize_scalar(u8, table, '\n');
        while (lines.next()) |line| {
            if (line[0] == '#') continue;
    ...
    }
    {
        ...
        const table = try linux_dir.read_file("arch/mips/kernel/syscalls/syscall_o32.tbl", buf);
        var lines = mem.tokenize_scalar(u8, table, '\n');
        while (lines.next()) |line| {
            if (line[0] == '#') continue;
            ...
            _ = fields.next() orelse return error.Incomplete;
            const name = fields.next() orelse return error.Incomplete;
            if (mem.starts_with(u8, name, "unused")) continue;
    ...
    }
    {
        ...
        const table = try linux_dir.read_file("arch/mips/kernel/syscalls/syscall_n64.tbl", buf);
        var lines = mem.tokenize_scalar(u8, table, '\n');
        while (lines.next()) |line| {
            if (line[0] == '#') continue;
    ...
    {
        try writer.write_all("pub const PowerPC = enum(usize) {\n");

        const table = try linux_dir.read_file("arch/powerpc/kernel/syscalls/syscall.tbl", buf);
        ...
        try writer.write_all(list_64.items);
        try writer.write_all("};\n\n");
    }
    ...
    {
        try writer.write_all("pub const Arm64 = enum(usize) {\n");
        ...
        var lines = mem.tokenize_scalar(u8, defines, '\n');
        loop: while (lines.next()) |line| {
            ...
            if (!std.ascii.is_digit(number[0])) continue;
            if (!mem.starts_with(u8, define, "__NR")) continue;
            const name = mem.trim_left(u8, mem.trim_left(u8, define, "__NR3264_"), "__NR_");
