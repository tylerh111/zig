...
const catalog_txt = @embed_file("crc/catalog.txt");
...
pub fn main() anyerror!void {
    ...
    if (mem.starts_with(u8, zig_src_root, "-")) {
        usage_and_exit(std.io.get_std_err(), args[0], 1);
    }
    ...
    var hash_target_dir = try zig_src_dir.make_open_path(hash_sub_path, .{});
    defer hash_target_dir.close();

    const crc_sub_path = try fs.path.join(arena, &.{ "lib", "std", "hash", "crc" });
    var crc_target_dir = try zig_src_dir.make_open_path(crc_sub_path, .{});
    defer crc_target_dir.close();

    var zig_code_file = try hash_target_dir.create_file("crc.zig", .{});
    defer zig_code_file.close();

    var cbw = std.io.buffered_writer(zig_code_file.writer());
    defer cbw.flush() catch unreachable;
    const code_writer = cbw.writer();

    try code_writer.write_all(
    ...
    var zig_test_file = try crc_target_dir.create_file("test.zig", .{});
    defer zig_test_file.close();

    var tbw = std.io.buffered_writer(zig_test_file.writer());
    defer tbw.flush() catch unreachable;
    const test_writer = tbw.writer();

    try test_writer.write_all(
        \\//! This file is auto-generated by tools/update_crc_catalog.zig.
        \\
        \\const std = @import("std");
    ...
    var stream = std.io.fixed_buffer_stream(catalog_txt);
    const reader = stream.reader();

    while (try reader.read_until_delimiter_or_eof_alloc(arena, '\n', std.math.max_int(usize))) |line| {
        ...
        var it = mem.split_sequence(u8, line, "  ");
        while (it.next()) |property| {
            const i = mem.index_of(u8, property, "=").?;
        ...
        const snakecase = try ascii.alloc_lower_string(arena, name);
        defer arena.free(snakecase);

        _ = mem.replace(u8, snakecase, "-", "_", snakecase);
        _ = mem.replace(u8, snakecase, "/", "_", snakecase);

        var buf = try std.ArrayList(u8).init_capacity(arena, snakecase.len);
        defer buf.deinit();
        ...
        for (snakecase, 0..) |c, i| {
            if (c == '_') {
                ...
                buf.append_assume_capacity(ascii.to_upper(c));
            } else if (prev == '_') {
                buf.append_assume_capacity(ascii.to_upper(c));
            } else {
                buf.append_assume_capacity(c);
            }
            prev = c;
        }
        ...
        try code_writer.write_all(try std.fmt.alloc_print(arena,
            \\
            \\pub const {s} = Crc(u{s}, .{{
            \\    .polynomial = {s},
            \\    .initial = {s},
        ...
        try test_writer.write_all(try std.fmt.alloc_print(arena,
            \\
            \\test "{0s}" {{
            \\    const {1s} = crc.{1s};
            \\
            \\    try testing.expect_equal(@as(u{2s}, {3s}), {1s}.hash("123456789"));
            \\
            \\    var c = {1s}.init();
            \\    c.update("1234");
            \\    c.update("56789");
            \\    try testing.expect_equal(@as(u{2s}, {3s}), c.final());
            \\}}
            \\
