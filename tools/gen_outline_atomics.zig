    ...
    for ([_]N{ .one, .two, .four, .eight, .sixteen }) |n| {
        for ([_]Ordering{ .relax, .acq, .rel, .acq_rel }) |order| {
            for ([_]AtomicOp{ .cas, .swp, .ldadd, .ldclr, .ldeor, .ldset }) |op| {
                ...
                const name = try std.fmt.alloc_print(arena, "__aarch64_{s}{d}_{s}", .{
                    @tag_name(op), n.to_bytes(), @tag_name(order),
                });
                try write_function(arena, w, name, op, n, order);
...
fn write_function(
    ...
    const body = switch (op) {
        .cas => try generate_cas(arena, n, order),
        .swp => try generate_swp(arena, n, order),
        .ldadd => try generate_ld(arena, n, order, .ldadd),
        .ldclr => try generate_ld(arena, n, order, .ldclr),
        .ldeor => try generate_ld(arena, n, order, .ldeor),
        .ldset => try generate_ld(arena, n, order, .ldset),
    };
    const fn_sig = try std.fmt.alloc_print(
        arena,
        "fn {[name]s}() align(16) callconv(.Naked) void {{",
        .{ .name = name },
    );
    try w.write_all(fn_sig);
    try w.write_all(
        \\
    ...
    var iter = std.mem.split_scalar(u8, body, '\n');
...
const N = enum(u8) {
    ...
    fn register(n: N) []const u8 {
        return if (@int_from_enum(n) < 8) "w" else "x";
    }

    fn to_bytes(n: N) u8 {
        return @int_from_enum(n);
    }
...
fn generate_cas(arena: Allocator, n: N, order: Ordering) ![]const u8 {
    ...
    if (@int_from_enum(n) < 16) {
        const cas = try std.fmt.alloc_print(arena, ".inst 0x08a07c41 + {s} + {s}", .{ s_def.b, o_def.m });
        const ldxr = try std.fmt.alloc_print(arena, "ld{s}xr{s}", .{ o_def.a, s_def.s });
        const stxr = try std.fmt.alloc_print(arena, "st{s}xr{s}", .{ o_def.l, s_def.s });

        return try std.fmt.alloc_print(arena,
            \\        cbz     w16, 8f
            \\        {[cas]s}
            \\        ret
            \\8:
        ...
        const casp = try std.fmt.alloc_print(arena, ".inst 0x48207c82 + {s}", .{o_def.m});
        const ldxp = try std.fmt.alloc_print(arena, "ld{s}xp", .{o_def.a});
        const stxp = try std.fmt.alloc_print(arena, "st{s}xp", .{o_def.l});
