...
fn known_option(name: []const u8) ?[]const u8 {
    const chopped_name = if (std.mem.ends_with(u8, name, "=")) name[0 .. name.len - 1] else name;
...
pub fn main() anyerror!void {
    ...
    inline for (@typeInfo(cpu_targets).Struct.decls) |decl| {
        ...
        for (all_features, 0..) |feat, i| {
            ...
            const zig_name = @tag_name(zig_feat);
    ...
    const child_args = [_][]const u8{
        llvm_tblgen_exe,
        "--dump-json",
        try std.fmt.alloc_print(allocator, "{s}/clang/include/clang/Driver/Options.td", .{llvm_src_root}),
        try std.fmt.alloc_print(allocator, "-I={s}/llvm/include", .{llvm_src_root}),
        try std.fmt.alloc_print(allocator, "-I={s}/clang/include/clang/Driver", .{llvm_src_root}),
    };
    ...
    const parsed = try json.parse_from_slice(json.Value, allocator, json_text, .{});
    defer parsed.deinit();
    ...
    std.mem.sort(*json.ObjectMap, all_objects.items, {}, object_less_than);

    var buffered_stdout = std.io.buffered_writer(std.io.get_std_out().writer());
    ...
    for (all_objects.items) |obj| {
        ...
        const syntax = obj_syntax(obj) orelse continue;
        ...
        if (std.mem.eql(u8, name, "MT") and syntax == .flag) {
        ...
        } else if (known_option(name)) |ident| {
            ...
            if ((std.mem.starts_with(u8, name, "mno-") and
                llvm_to_zig_cpu_features.contains(name["mno-".len..])) or
                (std.mem.starts_with(u8, name, "m") and
...
const Syntax = union(enum) {
    ...
    pub fn format(
        ...
        _ = options;
        switch (self) {
            .multi_arg => |n| return out_stream.print(".{{.{s}={}}}", .{ @tag_name(self), n }),
            else => return out_stream.print(".{s}", .{@tag_name(self)}),
        }
    }
};

fn obj_syntax(obj: *json.ObjectMap) ?Syntax {
    const num_args = @as(u8, @int_cast(obj.get("NumArgs").?.integer));
...
fn object_less_than(context: void, a: *json.ObjectMap, b: *json.ObjectMap) bool {
    ...
    const a_syntax = obj_syntax(a) orelse return false;
    const b_syntax = obj_syntax(b) orelse return true;

    const a_match_with_eql = syntax_matches_with_eql(a_syntax);
    const b_match_with_eql = syntax_matches_with_eql(b_syntax);
    ...
    return std.mem.less_than(u8, a_key, b_key);
}
