...
pub fn add_global(self: *LinkerDefined, name: [:0]const u8, elf_file: *Elf) !u32 {
    ...
    const gpa = comp.gpa;
    try self.symtab.ensure_unused_capacity(gpa, 1);
    ...
    self.symtab.append_assume_capacity(.{
        ...
        .st_other = @int_from_enum(elf.STV.HIDDEN),
...
pub fn resolve_symbols(self: *LinkerDefined, elf_file: *Elf) void {
    for (self.symbols.items, 0..) |index, i| {
        const sym_idx = @as(Symbol.Index, @int_cast(i));
...
pub fn fmt_symtab(self: *LinkerDefined, elf_file: *Elf) std.fmt.Formatter(format_symtab) {
    return .{ .data = .{
        .self = self,
        .elf_file = elf_file,
    } };
