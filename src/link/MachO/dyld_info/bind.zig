...
pub const Entry = struct {
    ...
    pub fn less_than(ctx: *MachO, entry: Entry, other: Entry) bool {
        if (entry.segment_id == other.segment_id) {
            ...
            const entry_name = ctx.get_symbol(entry.target).get_name(ctx);
            const other_name = ctx.get_symbol(other.target).get_name(ctx);
            return std.mem.less_than(u8, entry_name, other_name);
        }
...
pub const Bind = struct {
    ...
    pub fn size(self: Self) u64 {
        return @int_cast(self.buffer.items.len);
    }
    ...
    pub fn finalize(self: *Self, gpa: Allocator, ctx: *MachO) !void {
        ...
        std.mem.sort(Entry, self.entries.items, ctx, Entry.less_than);

        var start: usize = 0;
        var seg_id: ?u8 = null;
        for (self.entries.items, 0..) |entry, i| {
            if (seg_id != null and seg_id.? == entry.segment_id) continue;
            try finalize_segment(self.entries.items[start..i], ctx, writer);
            seg_id = entry.segment_id;
            start = i;
        }

        try finalize_segment(self.entries.items[start..], ctx, writer);
        try done(writer);
    }
    ...
    fn finalize_segment(entries: []const Entry, ctx: *MachO, writer: anytype) !void {
        ...
        try set_segment_offset(seg_id, 0, writer);
        ...
        while (i < entries.len) : (i += 1) {
            ...
            if (target == null or target.? != current.target) {
                switch (state) {
                    .start => {},
                    .bind_single => try do_bind(writer),
                    .bind_times_skip => try do_bind_times_skip(count, skip, writer),
                }
                state = .start;
                target = current.target;

                const sym = ctx.get_symbol(current.target);
                const name = sym.get_name(ctx);
                const flags: u8 = if (sym.weak_ref(ctx)) macho.BIND_SYMBOL_FLAGS_WEAK_IMPORT else 0;
                const ordinal: i16 = ord: {
                    ...
                    if (sym.flags.import) {
                        ...
                        if (sym.get_dylib_ordinal(ctx)) |ord| break :ord @bit_cast(ord);
                    }
                ...
                try set_symbol(name, flags, writer);
                try set_type_pointer(writer);
                try set_dylib_ordinal(ordinal, writer);

                if (current.addend != addend) {
                    addend = current.addend;
                    try set_addend(addend, writer);
                }
            }

            log.debug("{x}, {d}, {x}, {?x}, {s}", .{ offset, count, skip, addend, @tag_name(state) });
            log.debug("  => {x}", .{current.offset});
            switch (state) {
                .start => {
                    if (current.offset < offset) {
                        try add_addr(@bit_cast(@as(i64, @int_cast(current.offset)) - @as(i64, @int_cast(offset))), writer);
                ...
                .bind_times_skip => {
                    if (current.offset < offset) {
                        count -= 1;
                        if (count == 1) {
                            try do_bind_add_addr(skip, writer);
                        } else {
                            try do_bind_times_skip(count, skip, writer);
                        }
                        state = .start;
    ...
    pub fn write(self: Self, writer: anytype) !void {
        ...
        try writer.write_all(self.buffer.items);
    }
};
    ...
    fn finalize_segment(entries: []const Entry, ctx: *MachO, writer: anytype) !void {
        ...
        while (i < entries.len) : (i += 1) {
            ...
            switch (state) {
                .start => {
                    if (current.offset < offset) {
                        try add_addr(@as(u64, @bit_cast(@as(i64, @int_cast(current.offset)) - @as(i64, @int_cast(offset)))), writer);
                        offset = offset - (offset - current.offset);
                    } else if (current.offset > offset) {
                        const delta = current.offset - offset;
                        try add_addr(delta, writer);
...
pub const LazyBind = struct {
    ...
    pub fn finalize(self: *Self, gpa: Allocator, ctx: *MachO) !void {
        try self.offsets.ensure_total_capacity_precise(gpa, self.entries.items.len);
        ...
        for (self.entries.items) |entry| {
            self.offsets.append_assume_capacity(@int_cast(self.buffer.items.len));

            const sym = ctx.get_symbol(entry.target);
            const name = sym.get_name(ctx);
            const flags: u8 = if (sym.weak_ref(ctx)) macho.BIND_SYMBOL_FLAGS_WEAK_IMPORT else 0;
            const ordinal: i16 = ord: {
                if (sym.flags.interposable) break :ord macho.BIND_SPECIAL_DYLIB_FLAT_LOOKUP;
            ...
            try set_segment_offset(entry.segment_id, entry.offset, writer);
            try set_symbol(name, flags, writer);
            try set_dylib_ordinal(ordinal, writer);

            if (entry.addend != addend) {
                try set_addend(entry.addend, writer);
                addend = entry.addend;
            }
...
fn set_segment_offset(segment_id: u8, offset: u64, writer: anytype) !void {
    ...
    try writer.write_byte(macho.BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB | @as(u4, @truncate(segment_id)));
...
fn set_symbol(name: []const u8, flags: u8, writer: anytype) !void {
    ...
    try writer.write_byte(macho.BIND_OPCODE_SET_SYMBOL_TRAILING_FLAGS_IMM | @as(u4, @truncate(flags)));
...
fn set_type_pointer(writer: anytype) !void {
    ...
    try writer.write_byte(macho.BIND_OPCODE_SET_TYPE_IMM | @as(u4, @truncate(macho.BIND_TYPE_POINTER)));
}
...
fn set_dylib_ordinal(ordinal: i16, writer: anytype) !void {
    if (ordinal <= 0) {
        ...
        const cast = @as(u16, @bit_cast(ordinal));
        try writer.write_byte(macho.BIND_OPCODE_SET_DYLIB_SPECIAL_IMM | @as(u4, @truncate(cast)));
    } else {
        const cast = @as(u16, @bit_cast(ordinal));
        log.debug(">>> set dylib ordinal: {d}", .{ordinal});
        if (cast <= 0xf) {
            try writer.write_byte(macho.BIND_OPCODE_SET_DYLIB_ORDINAL_IMM | @as(u4, @truncate(cast)));
        } else {
            try writer.write_byte(macho.BIND_OPCODE_SET_DYLIB_ORDINAL_ULEB);
            try std.leb.write_uleb128(writer, cast);
        }
    }
}
...
fn set_addend(addend: i64, writer: anytype) !void {
    ...
    try writer.write_byte(macho.BIND_OPCODE_SET_ADDEND_SLEB);
    try std.leb.write_ileb128(writer, addend);
}

fn do_bind(writer: anytype) !void {
    log.debug(">>> bind", .{});
    try writer.write_byte(macho.BIND_OPCODE_DO_BIND);
}
...
fn do_bind_add_addr(addr: u64, writer: anytype) !void {
    ...
    try writer.write_byte(macho.BIND_OPCODE_DO_BIND_ADD_ADDR_ULEB);
...
fn do_bind_times_skip(count: usize, skip: u64, writer: anytype) !void {
    ...
    try writer.write_byte(macho.BIND_OPCODE_DO_BIND_ULEB_TIMES_SKIPPING_ULEB);
...
fn add_addr(addr: u64, writer: anytype) !void {
    ...
    try writer.write_byte(macho.BIND_OPCODE_ADD_ADDR_ULEB);
    try std.leb.write_uleb128(writer, addr);
}

fn done(writer: anytype) !void {
    log.debug(">>> done", .{});
    try writer.write_byte(macho.BIND_OPCODE_DONE);
}
