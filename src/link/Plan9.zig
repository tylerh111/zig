...
const LazySymbolMetadata = struct {
    ...
    fn number_of_atoms(self: LazySymbolMetadata) u32 {
        var n: u32 = 0;
...
pub const Atom = struct {
    ...
    const CodePtr = struct {
        ...
        fn from_slice(slice: []u8) CodePtr {
            return .{ .code_ptr = slice.ptr, .other = .{ .code_len = slice.len } };
        }
        fn get_code(self: CodePtr, plan9: *const Plan9) []u8 {
            ...
            return if (self.code_ptr) |p| p[0..self.other.code_len] else blk: {
                ...
                if (decl.type_of(mod).zig_type_tag(mod) == .Fn) {
                    const table = plan9.fn_decl_table.get(decl.get_file_scope(mod)).?.functions;
        ...
        fn get_owned_code(self: CodePtr) ?[]u8 {
    ...
    pub fn get_or_create_offset_table_entry(self: *Atom, plan9: *Plan9) usize {
        if (self.got_index == null) self.got_index = plan9.allocate_got_index();
        return self.got_index.?;
    }

    pub fn get_or_create_symbol_table_entry(self: *Atom, plan9: *Plan9) !usize {
        if (self.sym_index == null) self.sym_index = try plan9.allocate_symbol_index();
        return self.sym_index.?;
    }

    // asserts that self.got_index != null
    pub fn get_offset_table_address(self: Atom, plan9: *Plan9) u64 {
        const target = plan9.base.comp.root_mod.resolved_target.result;
        const ptr_bytes = @div_exact(target.ptr_bit_width(), 8);
...
const DeclMetadata = struct {
    ...
    fn get_export(m: DeclMetadata, p9: *const Plan9, name: []const u8) ?usize {
...
fn get_addr(self: Plan9, addr: u64, t: aout.Sym.Type) u64 {
...
fn get_sym_addr(self: Plan9, s: aout.Sym) u64 {
    return self.get_addr(s.value, s.type);
}

pub fn default_base_addrs(arch: std.Target.Cpu.Arch) Bases {
    return switch (arch) {
        .x86_64 => .{
            // header size => 40 => 0x28
            .text = 0x200028,
...
pub fn create_empty(
    ...
    const sixtyfour_bit: bool = switch (target.ptr_bit_width()) {
        0...32 => false,
        33...64 => true,
    ...
    self.* = .{
        ...
        .bases = undefined,
        .magic = try aout.magic_from_arch(target.cpu.arch),
    };
...
fn put_fn(self: *Plan9, decl_index: InternPool.DeclIndex, out: FnDeclOutput) !void {
    const gpa = self.base.comp.gpa;
    const mod = self.base.comp.module.?;
    const decl = mod.decl_ptr(decl_index);
    const fn_map_res = try self.fn_decl_table.get_or_put(gpa, decl.get_file_scope(mod));
    if (fn_map_res.found_existing) {
        if (try fn_map_res.value_ptr.functions.fetch_put(gpa, decl_index, out)) |old_entry| {
            gpa.free(old_entry.value.code);
            gpa.free(old_entry.value.lineinfo);
        }
    } else {
        const file = decl.get_file_scope(mod);
        ...
        fn_map_res.value_ptr.* = .{
            .sym_index = blk: {
                ...
                try self.syms.append(gpa, undefined);
                break :blk @as(u32, @int_cast(self.syms.items.len - 1));
            },
        };
        ...
        try a.writer().write_int(u16, 1, .big);
        ...
        try self.add_path_components(full_path, &a);

        // null terminate
        try a.append(0);
        const final = try a.to_owned_slice();
        ...
        self.syms.items[fn_map_res.value_ptr.sym_index] = .{
            ...
            .value = std.math.max_int(u31),
            .name = &.{ 0, 0 },
        };
    }
}

fn add_path_components(self: *Plan9, path: []const u8, a: *std.ArrayList(u8)) !void {
    const gpa = self.base.comp.gpa;
    const sep = std.fs.path.sep;
    var it = std.mem.tokenize_scalar(u8, path, sep);
    while (it.next()) |component| {
        if (self.file_segments.get(component)) |num| {
            try a.writer().write_int(u16, num, .big);
        } else {
            self.file_segments_i += 1;
            try self.file_segments.put(gpa, component, self.file_segments_i);
            try a.writer().write_int(u16, self.file_segments_i, .big);
        }
    }
}

pub fn update_func(self: *Plan9, mod: *Module, func_index: InternPool.Index, air: Air, liveness: Liveness) !void {
    ...
    const atom_idx = try self.see_decl(decl_index);
    ...
    var dbg_info_output: DebugInfoOutput = .{
        ...
        .pc_quanta = aout.get_pcquant(target.cpu.arch) catch unreachable,
    };
    ...
    const code = switch (res) {
        .ok => try code_buffer.to_owned_slice(),
        .fail => |em| {
    ...
    self.get_atom_ptr(atom_idx).code = .{
        .code_ptr = null,
        .other = .{ .decl_index = decl_index },
    };
    const out: FnDeclOutput = .{
        .code = code,
        .lineinfo = try dbg_info_output.dbg_line.to_owned_slice(),
        .start_line = dbg_info_output.start_line.?,
        .end_line = dbg_info_output.end_line,
    };
    try self.put_fn(decl_index, out);
    return self.update_finish(decl_index);
}

pub fn lower_unnamed_const(self: *Plan9, val: Value, decl_index: InternPool.DeclIndex) !u32 {
    const gpa = self.base.comp.gpa;
    _ = try self.see_decl(decl_index);
    var code_buffer = std.ArrayList(u8).init(gpa);
    defer code_buffer.deinit();
    ...
    const sym_index = try self.allocate_symbol_index();
    const new_atom_idx = try self.create_atom();
    const info: Atom = .{
        .type = .d,
        .offset = null,
        .sym_index = sym_index,
        .got_index = self.allocate_got_index(),
    ...
    const res = try codegen.generate_symbol(&self.base, decl.src_loc(mod), val, &code_buffer, .{
        .none = {},
    }, .{
    ...
    const new_atom = self.get_atom_ptr(new_atom_idx);
    new_atom.* = info;
...
pub fn update_decl(self: *Plan9, mod: *Module, decl_index: InternPool.DeclIndex) !void {
    const gpa = self.base.comp.gpa;
    const decl = mod.decl_ptr(decl_index);
    ...
    const res = try codegen.generate_symbol(&self.base, decl.src_loc(mod), decl_val, &code_buffer, .{ .none = {} }, .{
        .parent_atom_index = @as(Atom.Index, @int_cast(atom_idx)),
    });
    const code = switch (res) {
        .ok => code_buffer.items,
        .fail => |em| {
    ...
    try self.data_decl_table.ensure_unused_capacity(gpa, 1);
    const duped_code = try gpa.dupe(u8, code);
    self.get_atom_ptr(self.decls.get(decl_index).?.index).code = .{ .code_ptr = null, .other = .{ .decl_index = decl_index } };
    if (self.data_decl_table.fetch_put_assume_capacity(decl_index, duped_code)) |old_entry| {
        gpa.free(old_entry.value);
    }
    return self.update_finish(decl_index);
}
/// called at the end of update{Decl,Func}
fn update_finish(self: *Plan9, decl_index: InternPool.DeclIndex) !void {
    const gpa = self.base.comp.gpa;
    const mod = self.base.comp.module.?;
    const decl = mod.decl_ptr(decl_index);
    const is_fn = (decl.type_of(mod).zig_type_tag(mod) == .Fn);
    const sym_t: aout.Sym.Type = if (is_fn) .t else .d;

    const atom = self.get_atom_ptr(self.decls.get(decl_index).?.index);
    // write the internal linker metadata
    atom.type = sym_t;
    // write the symbol
    // we already have the got index
    const sym: aout.Sym = .{
        .value = undefined, // the value of stuff gets filled in in flush_module
        .type = atom.type,
        .name = try gpa.dupe(u8, decl.name.to_slice(&mod.intern_pool)),
    };

    if (atom.sym_index) |s| {
        self.syms.items[s] = sym;
    } else {
        const s = try self.allocate_symbol_index();
        atom.sym_index = s;
        self.syms.items[s] = sym;
    }
}

fn allocate_symbol_index(self: *Plan9) !usize {
    const gpa = self.base.comp.gpa;
    if (self.syms_index_free_list.pop_or_null()) |i| {
        return i;
    } else {
        _ = try self.syms.add_one(gpa);
        return self.syms.items.len - 1;
    }
}

fn allocate_got_index(self: *Plan9) usize {
    if (self.got_index_free_list.pop_or_null()) |i| {
        return i;
    } else {
        self.got_len += 1;
...
pub fn flush(self: *Plan9, arena: Allocator, prog_node: std.Progress.Node) link.File.FlushError!void {
    ...
    switch (link.File.effective_output_mode(use_lld, comp.config.output_mode)) {
        .Exe => {},
...
pub fn change_line(l: *std.ArrayList(u8), delta_line: i32) !void {
    if (delta_line > 0 and delta_line < 65) {
        const toappend = @as(u8, @int_cast(delta_line));
        try l.append(toappend);
    } else if (delta_line < 0 and delta_line > -65) {
        const toadd: u8 = @as(u8, @int_cast(-delta_line + 64));
        try l.append(toadd);
    } else if (delta_line != 0) {
        try l.append(0);
        try l.writer().write_int(i32, delta_line, .big);
    }
}

fn extern_count(self: *Plan9) usize {
...
fn atom_count(self: *Plan9) usize {
    ...
    while (it_lazy.next()) |kv| {
        lazy_atom_count += kv.value_ptr.number_of_atoms();
    }
    const anon_atom_count = self.anon_decls.count();
    const extern_atom_count = self.extern_count();
    return data_decl_count + fn_decl_count + unnamed_const_count + lazy_atom_count + extern_atom_count + anon_atom_count;
}

pub fn flush_module(self: *Plan9, arena: Allocator, prog_node: std.Progress.Node) link.File.FlushError!void {
    if (build_options.skip_non_native and builtin.object_format != .plan9) {
        @panic("Attempted to compile for object format that was disabled by build configuration");
    }
    ...
    if (self.lazy_syms.get_ptr(.none)) |metadata| {
        ...
        if (metadata.text_state != .unused) self.update_lazy_symbol_atom(
            File.LazySymbol.init_decl(.code, null, mod),
            metadata.text_atom,
        ) catch |err| return switch (err) {
            error.CodegenFail => error.FlushFailure,
            else => |e| e,
        };
        if (metadata.rodata_state != .unused) self.update_lazy_symbol_atom(
            File.LazySymbol.init_decl(.const_data, null, mod),
    ...
    const atom_count = self.atom_count();
    ...
    var iovecs = try gpa.alloc(std.posix.iovec_const, self.atom_count() + 4 - self.extern_count());
        ...
        while (it_file.next()) |fentry| {
            ...
            while (it.next()) |entry| {
                ...
                const out = entry.value_ptr.*;
                {
                    ...
                    const delta_line = @as(i32, @int_cast(out.start_line)) - @as(i32, @int_cast(linecount));

                    try change_line(&linecountinfo, delta_line);
                    // TODO change the pc too (maybe?)

                    // write out the actual info that was generated in codegen now
                    try linecountinfo.append_slice(out.lineinfo);
                    linecount = out.end_line;
                }
                foff += out.code.len;
                iovecs[iovecs_i] = .{ .base = out.code.ptr, .len = out.code.len };
                iovecs_i += 1;
                const off = self.get_addr(text_i, .t);
                text_i += out.code.len;
                atom.offset = off;
                ...
                if (!self.sixtyfour_bit) {
                    mem.write_int(u32, got_table[atom.got_index.? * 4 ..][0..4], @as(u32, @int_cast(off)), target.cpu.arch.endian());
                } else {
                    mem.write_int(u64, got_table[atom.got_index.? * 8 ..][0..8], off, target.cpu.arch.endian());
                }
                self.syms.items[atom.sym_index.?].value = off;
                if (mod.decl_exports.get(decl_index)) |exports| {
                    try self.add_decl_exports(mod, decl_index, exports.items);
                }
            }
        }
    ...
    // the text lazy symbols
    {
        ...
        while (it.next()) |kv| {
            ...
            const text_atom = if (meta.text_state != .unused) self.get_atom_ptr(meta.text_atom) else continue;
            const code = text_atom.code.get_owned_code().?;
            foff += code.len;
            ...
            if (!self.sixtyfour_bit) {
                mem.write_int(u32, got_table[text_atom.got_index.? * 4 ..][0..4], @as(u32, @int_cast(off)), target.cpu.arch.endian());
            } else {
                mem.write_int(u64, got_table[text_atom.got_index.? * 8 ..][0..8], off, target.cpu.arch.endian());
            }
    ...
    if (self.etext_edata_end_atom_indices[0]) |etext_atom_idx| {
        const etext_atom = self.get_atom(etext_atom_idx);
        const val = self.get_addr(text_i, .t);
        self.syms.items[etext_atom.sym_index.?].value = val;
        if (!self.sixtyfour_bit) {
            mem.write_int(u32, got_table[etext_atom.got_index.? * 4 ..][0..4], @as(u32, @int_cast(val)), target.cpu.arch.endian());
        } else {
            mem.write_int(u64, got_table[etext_atom.got_index.? * 8 ..][0..8], val, target.cpu.arch.endian());
        }
    }
    ...
    var data_i: u64 = got_size;
    {
        ...
        while (it.next()) |entry| {
            ...
            iovecs[iovecs_i] = .{ .base = code.ptr, .len = code.len };
            iovecs_i += 1;
            const off = self.get_addr(data_i, .d);
            data_i += code.len;
            atom.offset = off;
            if (!self.sixtyfour_bit) {
                mem.write_int(u32, got_table[atom.got_index.? * 4 ..][0..4], @as(u32, @int_cast(off)), target.cpu.arch.endian());
        ...
        while (it_unc.next()) |unnamed_consts| {
            for (unnamed_consts.value_ptr.items) |atom_idx| {
                const atom = self.get_atom_ptr(atom_idx);
                const code = atom.code.get_owned_code().?; // unnamed consts must own their code
        ...
        // the anon decls
        {
            ...
            while (it_anon.next()) |kv| {
                const atom = self.get_atom_ptr(kv.value_ptr.*);
                const code = atom.code.get_owned_code().?;
        ...
        while (it_lazy.next()) |kv| {
            const meta = kv.value_ptr;
            const data_atom = if (meta.rodata_state != .unused) self.get_atom_ptr(meta.rodata_atom) else continue;
            const code = data_atom.code.get_owned_code().?; // lazy symbols must own their code
            foff += code.len;
            iovecs[iovecs_i] = .{ .base = code.ptr, .len = code.len };
            iovecs_i += 1;
            const off = self.get_addr(data_i, .d);
            ...
            if (!self.sixtyfour_bit) {
                mem.write_int(u32, got_table[data_atom.got_index.? * 4 ..][0..4], @as(u32, @int_cast(off)), target.cpu.arch.endian());
            } else {
                mem.write_int(u64, got_table[data_atom.got_index.? * 8 ..][0..8], off, target.cpu.arch.endian());
            }
            self.syms.items[data_atom.sym_index.?].value = off;
        }
        // edata symbol
        if (self.etext_edata_end_atom_indices[1]) |edata_atom_idx| {
            const edata_atom = self.get_atom(edata_atom_idx);
            const val = self.get_addr(data_i, .b);
            self.syms.items[edata_atom.sym_index.?].value = val;
            if (!self.sixtyfour_bit) {
                mem.write_int(u32, got_table[edata_atom.got_index.? * 4 ..][0..4], @as(u32, @int_cast(val)), target.cpu.arch.endian());
            } else {
                mem.write_int(u64, got_table[edata_atom.got_index.? * 8 ..][0..8], val, target.cpu.arch.endian());
            }
        }
        // end symbol (same as edata because native backends don't do .bss yet)
        if (self.etext_edata_end_atom_indices[2]) |end_atom_idx| {
            const end_atom = self.get_atom(end_atom_idx);
            const val = self.get_addr(data_i, .b);
            self.syms.items[end_atom.sym_index.?].value = val;
            if (!self.sixtyfour_bit) {
                mem.write_int(u32, got_table[end_atom.got_index.? * 4 ..][0..4], @as(u32, @int_cast(val)), target.cpu.arch.endian());
            } else {
                log.debug("write end (got_table[0x{x}] = 0x{x})", .{ end_atom.got_index.? * 8, val });
                mem.write_int(u64, got_table[end_atom.got_index.? * 8 ..][0..8], val, target.cpu.arch.endian());
            }
        }
    }
    var sym_buf = std.ArrayList(u8).init(gpa);
    try self.write_syms(&sym_buf);
    const syms = try sym_buf.to_owned_slice();
    defer gpa.free(syms);
    assert(2 + self.atom_count() - self.extern_count() == iovecs_i); // we didn't write all the decls
    ...
    self.hdr = .{
        .magic = self.magic,
        .text = @as(u32, @int_cast(text_i)),
        .data = @as(u32, @int_cast(data_i)),
        .syms = @as(u32, @int_cast(syms.len)),
        .bss = 0,
        .spsz = 0,
        .pcsz = @as(u32, @int_cast(linecountinfo.items.len)),
        .entry = @as(u32, @int_cast(self.entry_val.?)),
    };
    @memcpy(hdr_slice, self.hdr.to_u8s()[0..hdr_size]);
    // write the fat header for 64 bit entry points
    if (self.sixtyfour_bit) {
        mem.write_int(u64, hdr_buf[32..40], self.entry_val.?, .big);
    }
    // perform the relocs
    {
        ...
        while (it.next()) |kv| {
            ...
            const source_atom = self.get_atom(source_atom_index);
            const source_atom_symbol = self.syms.items[source_atom.sym_index.?];
            const code = source_atom.code.get_code(self);
            ...
            for (kv.value_ptr.items) |reloc| {
                ...
                if (reloc.type == .pcrel or reloc.type == .nonpcrel) {
                    ...
                    const target_atom = self.get_atom_ptr(target_atom_index);
                    const target_symbol = self.syms.items[target_atom.sym_index.?];
                    const target_offset = target_atom.offset.?;

                    switch (reloc.type) {
                        .pcrel => {
                            const disp = @as(i32, @int_cast(target_offset)) - @as(i32, @int_cast(source_atom.offset.?)) - 4 - @as(i32, @int_cast(offset));
                            mem.write_int(i32, code[@as(usize, @int_cast(offset))..][0..4], @as(i32, @int_cast(disp)), endian);
                        },
                        .nonpcrel => {
                            if (!self.sixtyfour_bit) {
                                mem.write_int(u32, code[@int_cast(offset)..][0..4], @as(u32, @int_cast(target_offset + addend)), endian);
                            } else {
                                mem.write_int(u64, code[@int_cast(offset)..][0..8], target_offset + addend, endian);
                            }
                        },
                        else => unreachable,
                    }
                    ...
                    const addr = switch (reloc.type) {
                        .special_etext => self.syms.items[self.get_atom(self.etext_edata_end_atom_indices[0].?).sym_index.?].value,
                        .special_edata => self.syms.items[self.get_atom(self.etext_edata_end_atom_indices[1].?).sym_index.?].value,
                        .special_end => self.syms.items[self.get_atom(self.etext_edata_end_atom_indices[2].?).sym_index.?].value,
                        else => unreachable,
                    };
                    if (!self.sixtyfour_bit) {
                        mem.write_int(u32, code[@int_cast(offset)..][0..4], @as(u32, @int_cast(addr + addend)), endian);
                    } else {
                        mem.write_int(u64, code[@int_cast(offset)..][0..8], addr + addend, endian);
                    }
                    log.debug("relocating the address of '{s}' + {d} into '{s}' + {d} (({s}[{d}] = 0x{x} + 0x{x})", .{ @tag_name(reloc.type), addend, source_atom_symbol.name, offset, source_atom_symbol.name, offset, addr, addend });
                }
            }
        }
    }
    // write it all!
    try file.pwritev_all(iovecs, 0);
}
fn add_decl_exports(
    self: *Plan9,
    mod: *Module,
    ...
    const gpa = self.base.comp.gpa;
    const metadata = self.decls.get_ptr(decl_index).?;
    const atom = self.get_atom(metadata.index);

    for (exports) |exp| {
        const exp_name = exp.opts.name.to_slice(&mod.intern_pool);
        // plan9 does not support custom sections
        if (exp.opts.section.unwrap()) |section_name| {
            if (!section_name.eql_slice(".text", &mod.intern_pool) and
                !section_name.eql_slice(".data", &mod.intern_pool))
            {
                try mod.failed_exports.put(mod.gpa, exp, try Module.ErrorMsg.create(
                    gpa,
                    mod.decl_ptr(decl_index).src_loc(mod),
        ...
        const sym = .{
            .value = atom.offset.?,
            .type = atom.type.to_global(),
            .name = try gpa.dupe(u8, exp_name),
        };

        if (metadata.get_export(self, exp_name)) |i| {
...
pub fn free_decl(self: *Plan9, decl_index: InternPool.DeclIndex) void {
    const gpa = self.base.comp.gpa;
    // TODO audit the lifetimes of decls table entries. It's possible to get
    // free_decl without any update_decl in between.
    // However that is planned to change, see the TODO comment in Module.zig
    // in the deleteUnusedDecl function.
    const mod = self.base.comp.module.?;
    const decl = mod.decl_ptr(decl_index);
    const is_fn = decl.val.is_func_body(mod);
    if (is_fn) {
        const symidx_and_submap = self.fn_decl_table.get(decl.get_file_scope(mod)).?;
        var submap = symidx_and_submap.functions;
        if (submap.fetch_swap_remove(decl_index)) |removed_entry| {
        ...
        if (self.data_decl_table.fetch_swap_remove(decl_index)) |removed_entry| {
            gpa.free(removed_entry.value);
        }
    }
    if (self.decls.fetch_remove(decl_index)) |const_kv| {
        var kv = const_kv;
        const atom = self.get_atom(kv.value.index);
        if (atom.got_index) |i| {
            // TODO: if this catch {} is triggered, an assertion in flush_module will be triggered, because got_index_free_list will have the wrong length
    ...
    self.free_unnamed_consts(decl_index);
    {
        ...
        const relocs = self.relocs.get_ptr(atom_index) orelse return;
        relocs.clear_and_free(gpa);
        assert(self.relocs.remove(atom_index));
    }
}
fn free_unnamed_consts(self: *Plan9, decl_index: InternPool.DeclIndex) void {
    const gpa = self.base.comp.gpa;
    const unnamed_consts = self.unnamed_const_atoms.get_ptr(decl_index) orelse return;
    for (unnamed_consts.items) |atom_idx| {
        const atom = self.get_atom(atom_idx);
...
fn create_atom(self: *Plan9) !Atom.Index {
    const gpa = self.base.comp.gpa;
...
pub fn see_decl(self: *Plan9, decl_index: InternPool.DeclIndex) !Atom.Index {
    const gpa = self.base.comp.gpa;
    const gop = try self.decls.get_or_put(gpa, decl_index);
    if (!gop.found_existing) {
        const index = try self.create_atom();
        self.get_atom_ptr(index).got_index = self.allocate_got_index();
        gop.value_ptr.* = .{
            .index = index,
            .exports = .{},
        };
    ...
    // handle externs here because they might not get update_decl called on them
    const mod = self.base.comp.module.?;
    const decl = mod.decl_ptr(decl_index);
    if (decl.is_extern(mod)) {
        // this is a "phantom atom" - it is never actually written to disk, just convenient for us to store stuff about externs
        if (decl.name.eql_slice("etext", &mod.intern_pool)) {
            self.etext_edata_end_atom_indices[0] = atom_idx;
        } else if (decl.name.eql_slice("edata", &mod.intern_pool)) {
            self.etext_edata_end_atom_indices[1] = atom_idx;
        } else if (decl.name.eql_slice("end", &mod.intern_pool)) {
            self.etext_edata_end_atom_indices[2] = atom_idx;
        }
        try self.update_finish(decl_index);
        log.debug("see_decl(extern) for {} (got_addr=0x{x})", .{
            decl.name.fmt(&mod.intern_pool),
            self.get_atom(atom_idx).get_offset_table_address(self),
        });
    } else log.debug("see_decl for {}", .{decl.name.fmt(&mod.intern_pool)});
    return atom_idx;
}
...
pub fn update_exports(
    ...
    switch (exported) {
        .value => @panic("TODO: plan9 update_exports handling values"),
        .decl_index => |decl_index| _ = try self.see_decl(decl_index),
    }
...
pub fn get_or_create_atom_for_lazy_symbol(self: *Plan9, sym: File.LazySymbol) !Atom.Index {
    const gpa = self.base.comp.gpa;
    const gop = try self.lazy_syms.get_or_put(gpa, sym.get_decl(self.base.comp.module.?));
    ...
    _ = try self.get_atom_ptr(atom).get_or_create_symbol_table_entry(self);
    _ = self.get_atom_ptr(atom).get_or_create_offset_table_entry(self);
    // anyerror needs to be deferred until flush_module
    if (sym.get_decl(self.base.comp.module.?) != .none) {
        try self.update_lazy_symbol_atom(sym, atom);
    }
    return atom;
}

fn update_lazy_symbol_atom(self: *Plan9, sym: File.LazySymbol, atom_index: Atom.Index) !void {
    const gpa = self.base.comp.gpa;
    const mod = self.base.comp.module.?;
    ...
    self.syms.items[self.get_atom_ptr(atom_index).sym_index.?] = symbol;
    ...
    const res = try codegen.generate_lazy_symbol(
        ...
        &code_buffer,
        .none,
        .{ .parent_atom_index = @as(Atom.Index, @int_cast(atom_index)) },
    );
    const code = switch (res) {
        .ok => code_buffer.items,
        .fail => |em| {
    ...
    const duped_code = try gpa.dupe(u8, code);
    errdefer gpa.free(duped_code);
    self.get_atom_ptr(atom_index).code = .{
...
pub fn deinit(self: *Plan9) void {
    const gpa = self.base.comp.gpa;
    {
        var it = self.relocs.value_iterator();
    ...
    while (it_unc.next()) |kv| {
        self.free_unnamed_consts(kv.key_ptr.*);
    }
    self.unnamed_const_atoms.deinit(gpa);
    var it_lzc = self.lazy_syms.iterator();
    while (it_lzc.next()) |kv| {
        if (kv.value_ptr.text_state != .unused)
            gpa.free(self.syms.items[self.get_atom(kv.value_ptr.text_atom).sym_index.?].name);
        if (kv.value_ptr.rodata_state != .unused)
            gpa.free(self.syms.items[self.get_atom(kv.value_ptr.rodata_atom).sym_index.?].name);
    }
    ...
    while (it_anon.next()) |entry| {
        const sym_index = self.get_atom(entry.value_ptr.*).sym_index.?;
...
pub fn open(
    ...
    const self = try create_empty(arena, comp, emit, options);
    errdefer self.base.destroy();

    const file = try emit.directory.handle.create_file(emit.sub_path, .{
        .read = true,
        .mode = link.File.determine_mode(
            use_lld,
    ...
    self.bases = default_base_addrs(target.cpu.arch);

    const gpa = comp.gpa;

    try self.syms.append_slice(gpa, &.{
        // we include the global offset table to make it easier for debugging
        .{
            .value = self.get_addr(0, .d), // the global offset table starts at 0
            .type = .d,
            .name = "__GOT",
        },
    });
...
pub fn write_sym(self: *Plan9, w: anytype, sym: aout.Sym) !void {
    // log.debug("write sym{{name: {s}, value: {x}}}", .{ sym.name, sym.value });
    if (sym.type == .bad) return; // we don't want to write free'd symbols
    if (!self.sixtyfour_bit) {
        try w.write_int(u32, @as(u32, @int_cast(sym.value)), .big);
    } else {
        try w.write_int(u64, sym.value, .big);
    }
    try w.write_byte(@int_from_enum(sym.type));
    try w.write_all(sym.name);
    try w.write_byte(0);
}

pub fn write_syms(self: *Plan9, buf: *std.ArrayList(u8)) !void {
    const mod = self.base.comp.module.?;
    const ip = &mod.intern_pool;
    const writer = buf.writer();
    // write __GOT
    try self.write_sym(writer, self.syms.items[0]);
    // write the f symbols
    {
        var it = self.file_segments.iterator();
        while (it.next()) |entry| {
            try self.write_sym(writer, .{
                .type = .f,
    ...
    // write the data symbols
    {
        ...
        while (it.next()) |entry| {
            ...
            const atom = self.get_atom(decl_metadata.index);
            const sym = self.syms.items[atom.sym_index.?];
            try self.write_sym(writer, sym);
            if (self.base.comp.module.?.decl_exports.get(decl_index)) |exports| {
                for (exports.items) |e| if (decl_metadata.get_export(self, e.opts.name.to_slice(ip))) |exp_i| {
                    try self.write_sym(writer, self.syms.items[exp_i]);
                };
            }
        }
    }
    ...
    // unnamed consts
    {
        ...
        while (it.next()) |kv| {
            ...
            for (consts.items) |atom_index| {
                const sym = self.syms.items[self.get_atom(atom_index).sym_index.?];
                try self.write_sym(writer, sym);
            }
        }
    }
    ...
    // so we have to write everything in the right order
    {
        ...
        while (it_file.next()) |fentry| {
            ...
            // write the z symbols
            try self.write_sym(writer, self.syms.items[symidx_and_submap.sym_index - 1]);
            try self.write_sym(writer, self.syms.items[symidx_and_submap.sym_index]);
...
pub fn update_decl_line_number(self: *Plan9, mod: *Module, decl_index: InternPool.DeclIndex) !void {
    _ = self;
    _ = mod;
    _ = decl_index;
}
...
pub fn get_decl_vaddr(
    ...
    const ip = &mod.intern_pool;
    const decl = mod.decl_ptr(decl_index);
    log.debug("get_decl_vaddr for {}", .{decl.name.fmt(ip)});
    if (decl.is_extern(mod)) {
        if (decl.name.eql_slice("etext", ip)) {
            try self.add_reloc(reloc_info.parent_atom_index, .{
                .target = undefined,
                .offset = reloc_info.offset,
                .addend = reloc_info.addend,
                .type = .special_etext,
            });
        } else if (decl.name.eql_slice("edata", ip)) {
            try self.add_reloc(reloc_info.parent_atom_index, .{
                .target = undefined,
                .offset = reloc_info.offset,
                .addend = reloc_info.addend,
        ...
        } else if (decl.name.eql_slice("end", ip)) {
            try self.add_reloc(reloc_info.parent_atom_index, .{
                .target = undefined,
                .offset = reloc_info.offset,
                .addend = reloc_info.addend,
    ...
    const atom_index = try self.see_decl(decl_index);
...
pub fn lower_anon_decl(
    ...
    // This is basically the same as lower_unnamed_const.
    // example:
    // const ty = mod.intern_pool.type_of(decl_val).to_type();
    // const val = decl_val.to_value();
    // The symbol name can be something like `__anon_{d}` with `@int_from_enum(decl_val)`.
    ...
    // ...
    const gpa = self.base.comp.gpa;
    const gop = try self.anon_decls.get_or_put(gpa, decl_val);
    if (!gop.found_existing) {
        const val = Value.from_interned(decl_val);
        const name = try std.fmt.alloc_print(gpa, "__anon_{d}", .{@int_from_enum(decl_val)});

        const index = try self.create_atom();
        const got_index = self.allocate_got_index();
        gop.value_ptr.* = index;
        // we need to free name latex
        var code_buffer = std.ArrayList(u8).init(gpa);
        const res = try codegen.generate_symbol(&self.base, src_loc, val, &code_buffer, .{ .none = {} }, .{ .parent_atom_index = index });
        const code = switch (res) {
            .ok => code_buffer.items,
            .fail => |em| return .{ .fail = em },
        };
        const atom_ptr = self.get_atom_ptr(index);
        atom_ptr.* = .{
            .type = .d,
            .offset = undefined,
            .sym_index = null,
            .got_index = got_index,
            .code = Atom.CodePtr.from_slice(code),
        };
        _ = try atom_ptr.get_or_create_symbol_table_entry(self);
...
pub fn get_anon_decl_vaddr(self: *Plan9, decl_val: InternPool.Index, reloc_info: link.File.RelocInfo) !u64 {
...
pub fn add_reloc(self: *Plan9, parent_index: Atom.Index, reloc: Reloc) !void {
    const gpa = self.base.comp.gpa;
    const gop = try self.relocs.get_or_put(gpa, parent_index);
    if (!gop.found_existing) {
        gop.value_ptr.* = .{};
    }
...
pub fn get_atom(self: *const Plan9, index: Atom.Index) Atom {
    return self.atoms.items[index];
}

fn get_atom_ptr(self: *Plan9, index: Atom.Index) *Atom {
    return &self.atoms.items[index];
}
