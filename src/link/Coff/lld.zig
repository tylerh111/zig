...
const alloc_print = std.fmt.alloc_print;
...
pub fn link_with_lld(self: *Coff, arena: Allocator, prog_node: std.Progress.Node) !void {
    ...
    if (!self.base.disable_lld_caching) {
        ...
        man.hash.add_optional_bytes(entry_name);
        ...
        if (comp.config.link_libc) {
            ...
            if (comp.libc_installation) |libc_installation| {
                ...
                if (target.abi == .msvc) {
                    man.hash.add_bytes(libc_installation.msvc_lib_dir.?);
                    man.hash.add_bytes(libc_installation.kernel32_lib_dir.?);
                }
            }
        }
        try link.hash_add_system_libs(&man, comp.system_libs);
        man.hash.add_list_of_bytes(comp.force_undefined_symbols.keys());
        man.hash.add_optional(self.subsystem);
        ...
        try man.add_optional_file(self.module_definition_file);
        ...
        const prev_digest: []u8 = Cache.read_small_file(
            ...
            log.debug("COFF LLD new_digest={s} error: {s}", .{ std.fmt.fmt_slice_hex_lower(&digest), @errorName(err) });
            // Handle this as a cache miss.
            break :blk prev_digest_buf[0..0];
        };
        if (mem.eql(u8, prev_digest, &digest)) {
            log.debug("COFF LLD digest={s} match - skipping invocation", .{std.fmt.fmt_slice_hex_lower(&digest)});
            // Hot diggity dog! The output binary is already there.
            self.base.lock = man.to_owned_lock();
            return;
        }
        log.debug("COFF LLD prev_digest={s} new_digest={s}", .{ std.fmt.fmt_slice_hex_lower(prev_digest), std.fmt.fmt_slice_hex_lower(&digest) });
    ...
    if (comp.config.output_mode == .Obj) {
        ...
        if (comp.config.debug_format != .strip) {
            ...
            const out_pdb = self.pdb_out_path orelse try alloc_print(arena, "{s}.pdb", .{
                full_out_path[0 .. full_out_path.len - out_ext.len],
            });
            const out_pdb_basename = std.fs.path.basename(out_pdb);

            try argv.append(try alloc_print(arena, "-PDB:{s}", .{out_pdb}));
            try argv.append(try alloc_print(arena, "-PDBALTPATH:{s}", .{out_pdb_basename}));
        }
        if (comp.version) |version| {
            try argv.append(try alloc_print(arena, "-VERSION:{}.{}", .{ version.major, version.minor }));
        }
        if (comp.config.lto) {
        ...
        if (comp.config.output_mode == .Exe) {
            try argv.append(try alloc_print(arena, "-STACK:{d}", .{self.base.stack_size}));
        }
        try argv.append(try std.fmt.alloc_print(arena, "-BASE:{d}", .{self.image_base}));
        ...
        for (comp.force_undefined_symbols.keys()) |symbol| {
            try argv.append(try alloc_print(arena, "-INCLUDE:{s}", .{symbol}));
        }
        ...
        if (entry_name) |name| {
            try argv.append(try alloc_print(arena, "-ENTRY:{s}", .{name}));
        }
        ...
        try argv.append(try alloc_print(arena, "-OUT:{s}", .{full_out_path}));

        if (comp.implib_emit) |emit| {
            const implib_out_path = try emit.directory.join(arena, &[_][]const u8{emit.sub_path});
            try argv.append(try alloc_print(arena, "-IMPLIB:{s}", .{implib_out_path}));
        }

        if (comp.config.link_libc) {
            if (comp.libc_installation) |libc_installation| {
                try argv.append(try alloc_print(arena, "-LIBPATH:{s}", .{libc_installation.crt_dir.?}));

                if (target.abi == .msvc) {
                    try argv.append(try alloc_print(arena, "-LIBPATH:{s}", .{libc_installation.msvc_lib_dir.?}));
                    try argv.append(try alloc_print(arena, "-LIBPATH:{s}", .{libc_installation.kernel32_lib_dir.?}));
                }
            }
        }

        for (self.lib_dirs) |lib_dir| {
            try argv.append(try alloc_print(arena, "-LIBPATH:{s}", .{lib_dir}));
        }

        try argv.ensure_unused_capacity(comp.objects.len);
        for (comp.objects) |obj| {
            if (obj.must_link) {
                argv.append_assume_capacity(try alloc_print(arena, "-WHOLEARCHIVE:{s}", .{obj.path}));
            } else {
                argv.append_assume_capacity(obj.path);
            }
        }
        ...
        if (self.module_definition_file) |def| {
            try argv.append(try alloc_print(arena, "-DEF:{s}", .{def}));
        }
        ...
        const mode: Mode = mode: {
            if (resolved_subsystem) |subsystem| {
                const subsystem_suffix = try alloc_print(arena, ",{d}.{d}", .{
                    self.major_subsystem_version, self.minor_subsystem_version,
                });

                switch (subsystem) {
                    .Console => {
                        try argv.append(try alloc_print(arena, "-SUBSYSTEM:console{s}", .{
                            subsystem_suffix,
                        }));
                        break :mode .win32;
                    },
                    .EfiApplication => {
                        try argv.append(try alloc_print(arena, "-SUBSYSTEM:efi_application{s}", .{
                            subsystem_suffix,
                        }));
                        break :mode .uefi;
                    },
                    .EfiBootServiceDriver => {
                        try argv.append(try alloc_print(arena, "-SUBSYSTEM:efi_boot_service_driver{s}", .{
                            subsystem_suffix,
                        }));
                        break :mode .uefi;
                    },
                    .EfiRom => {
                        try argv.append(try alloc_print(arena, "-SUBSYSTEM:efi_rom{s}", .{
                            subsystem_suffix,
                        }));
                        break :mode .uefi;
                    },
                    .EfiRuntimeDriver => {
                        try argv.append(try alloc_print(arena, "-SUBSYSTEM:efi_runtime_driver{s}", .{
                            subsystem_suffix,
                        }));
                        break :mode .uefi;
                    },
                    .Native => {
                        try argv.append(try alloc_print(arena, "-SUBSYSTEM:native{s}", .{
                            subsystem_suffix,
                        }));
                        break :mode .win32;
                    },
                    .Posix => {
                        try argv.append(try alloc_print(arena, "-SUBSYSTEM:posix{s}", .{
                            subsystem_suffix,
                        }));
                        break :mode .win32;
                    },
                    .Windows => {
                        try argv.append(try alloc_print(arena, "-SUBSYSTEM:windows{s}", .{
                            subsystem_suffix,
                        }));
                        break :mode .win32;
                    },
        ...
        switch (mode) {
            .uefi => try argv.append_slice(&[_][]const u8{
                "-BASE:0",
                "-ENTRY:EfiMain",
                "-OPT:REF",
                "-SAFESEH:NO",
                ...
                if (link_in_crt) {
                    if (target.abi.is_gnu()) {
                        ...
                        switch (comp.config.link_mode) {
                            .static => try argv.append(try alloc_print(arena, "libcmt{s}.lib", .{d_str})),
                            .dynamic => try argv.append(try alloc_print(arena, "msvcrt{s}.lib", .{d_str})),
                        }

                        try argv.append(try alloc_print(arena, "{s}vcruntime{s}.lib", .{ lib_str, d_str }));
                        try argv.append(try alloc_print(arena, "{s}ucrt{s}.lib", .{ lib_str, d_str }));
                    ...
                    if (!is_lib and entry_name == null) {
                        if (comp.module) |module| {
                            if (module.stage1_flags.have_winmain_crt_startup) {
                                ...
                                try argv.append("-ENTRY:w_win_main_crtstartup");
                            }
                        } else {
                            try argv.append("-ENTRY:w_win_main_crtstartup");
                        }
        ...
        try argv.ensure_unused_capacity(comp.system_libs.count());
        for (comp.system_libs.keys()) |key| {
            const lib_basename = try alloc_print(arena, "{s}.lib", .{key});
            if (comp.crt_files.get(lib_basename)) |crt_file| {
                argv.append_assume_capacity(crt_file.full_object_path);
                continue;
            }
            if (try find_lib(arena, lib_basename, self.lib_dirs)) |full_path| {
                argv.append_assume_capacity(full_path);
                continue;
            }
            if (target.abi.is_gnu()) {
                const fallback_name = try alloc_print(arena, "lib{s}.dll.a", .{key});
                if (try find_lib(arena, fallback_name, self.lib_dirs)) |full_path| {
                    argv.append_assume_capacity(full_path);
                    continue;
                }
            }
            if (target.abi == .msvc) {
                argv.append_assume_capacity(lib_basename);
                continue;
            }
