...
pub fn append(opts: @This(), buffer: *std.ArrayList(u8)) Allocator.Error!void {
    ...
    const generic_arch_name = target.cpu.arch.generic_name();
    ...
    try buffer.writer().print(
        ...
        \\/// feature detection (i.e. with `@hasDecl` or `@has_field`) over version checks.
        ...
        \\    .features = std.Target.{p_}.feature_set(&[_]std.Target.{p_}.Feature{{
        \\
    , .{
        build_options.version,
        std.zig.fmt_id(@tag_name(zig_backend)),
        std.zig.fmt_id(@tag_name(opts.output_mode)),
        std.zig.fmt_id(@tag_name(opts.link_mode)),
        opts.is_test,
        opts.single_threaded,
        std.zig.fmt_id(@tag_name(target.abi)),
        std.zig.fmt_id(@tag_name(target.cpu.arch)),
        std.zig.fmt_id(generic_arch_name),
        std.zig.fmt_id(target.cpu.model.name),
        std.zig.fmt_id(generic_arch_name),
        std.zig.fmt_id(generic_arch_name),
    });
    ...
    try buffer.writer().print(
        ...
        \\    .version_range = .{{
    ,
        .{std.zig.fmt_id(@tag_name(target.os.tag))},
    );

    switch (target.os.get_version_range()) {
        .none => try buffer.append_slice(" .none = {} },\n"),
    ...
    try buffer.writer().print(
        ...
        \\
    , .{
        std.zig.fmt_id(@tag_name(target.ofmt)),
        std.zig.fmt_id(@tag_name(opts.optimize_mode)),
        link_libc,
        opts.link_libcpp,
        opts.error_tracing,
        opts.valgrind,
        ...
        opts.pie,
        opts.strip,
        std.zig.fmt_id(@tag_name(opts.code_model)),
    ...
    if (target.os.tag == .wasi) {
        try buffer.writer().print(
            ...
            \\
        , .{std.zig.fmt_id(@tag_name(opts.wasi_exec_model))});
    }
...
pub fn populate_file(comp: *Compilation, mod: *Module, file: *File) !void {
    ...
    if (mod.root.stat_file(mod.root_src_path)) |stat| {
        ...
        error.FileNotFound => try write_file(file, mod),
    ...
    assert(!file.zir.has_compile_errors()); // builtin.zig must not have astgen errors
...
fn write_file(file: *File, mod: *Module) !void {
    ...
    var af = try mod.root.atomic_file(mod.root_src_path, .{ .make_path = true }, &buf);
    defer af.deinit();
    try af.file.write_all(file.source);
