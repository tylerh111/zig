...
export fn verify(argc: c_int, argv: [*]const [*:0]const u16) c_int {
    const argv_slice = argv[0..@int_cast(argc)];
    test_argv(argv_slice) catch |err| switch (err) {
...
fn test_argv(expected_args: []const [*:0]const u16) !void {
    ...
    for (expected_args[0..min_len], args[0..min_len], 0..) |expected_arg, arg_wtf8, i| {
        wtf8_buf.clear_retaining_capacity();
        try std.unicode.wtf16_le_to_wtf8_array_list(&wtf8_buf, std.mem.span(expected_arg));
        if (!std.mem.eql(u8, wtf8_buf.items, arg_wtf8)) {
            std.debug.print("{}: expected: \"{}\"\n", .{ i, std.zig.fmt_escapes(wtf8_buf.items) });
            std.debug.print("{}:   actual: \"{}\"\n", .{ i, std.zig.fmt_escapes(arg_wtf8) });
            eql = false;
        }
    }
    if (!eql) {
        for (expected_args[min_len..], min_len..) |arg, i| {
            wtf8_buf.clear_retaining_capacity();
            try std.unicode.wtf16_le_to_wtf8_array_list(&wtf8_buf, std.mem.span(arg));
            std.debug.print("{}: expected: \"{}\"\n", .{ i, std.zig.fmt_escapes(wtf8_buf.items) });
        }
        for (args[min_len..], min_len..) |arg, i| {
            std.debug.print("{}:   actual: \"{}\"\n", .{ i, std.zig.fmt_escapes(arg) });
        }
        const peb = std.os.windows.peb();
        const lpCmdLine: [*:0]u16 = @ptr_cast(peb.ProcessParameters.CommandLine.Buffer);
        wtf8_buf.clear_retaining_capacity();
        try std.unicode.wtf16_le_to_wtf8_array_list(&wtf8_buf, std.mem.span(lpCmdLine));
        std.debug.print("command line: \"{}\"\n", .{std.zig.fmt_escapes(wtf8_buf.items)});
