...
pub fn add_case(self: *RunTranslatedCContext, case: *const TestCase) void {
    ...
    const annotated_case_name = fmt.alloc_print(self.b.allocator, "run-translated-c {s}", .{case.name}) catch unreachable;
    ...
    const exe = translate_c.add_executable(.{});
    ...
    run.expect_std_out_equal(case.expected_stdout);
