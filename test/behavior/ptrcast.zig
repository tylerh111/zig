...
test "reinterpret bytes as integer with nonzero offset" {
    ...
    try test_reinterpret_bytes_as_integer();
    try comptime test_reinterpret_bytes_as_integer();
}
...
fn test_reinterpret_bytes_as_integer() !void {
    ...
    try expect(@as(*align(1) const u32, @ptr_cast(bytes[1..5])).* == expected);
}
...
test "reinterpret an array over multiple elements, with no well-defined layout" {
    ...
    try test_reinterpret_with_offset_and_no_well_defined_layout();
    try comptime test_reinterpret_with_offset_and_no_well_defined_layout();
}

fn test_reinterpret_with_offset_and_no_well_defined_layout() !void {
    const bytes: ?[5]?u8 = [5]?u8{ 0x12, 0x34, 0x56, 0x78, 0x9a };
    const ptr = &bytes.?[1];
    const copy: [4]?u8 = @as(*const [4]?u8, @ptr_cast(ptr)).*;
    _ = copy;
    //try expect(@ptr_cast(*align(1)?u8, bytes[1..5]).* == );
}

test "reinterpret bytes inside auto-layout struct as integer with nonzero offset" {
    if (builtin.zig_backend == .stage2_sparc64) return error.SkipZigTest; // TODO

    try test_reinterpret_struct_wrapped_bytes_as_integer();
    try comptime test_reinterpret_struct_wrapped_bytes_as_integer();
}
...
fn test_reinterpret_struct_wrapped_bytes_as_integer() !void {
    ...
    try expect(@as(*align(1) const u32, @ptr_cast(obj.bytes[1..5])).* == expected);
}
...
test "reinterpret bytes of an array into an extern struct" {
    ...
    try test_reinterpret_bytes_as_extern_struct();
    try comptime test_reinterpret_bytes_as_extern_struct();
}
...
fn test_reinterpret_bytes_as_extern_struct() !void {
    ...
    const ptr: *const S = @ptr_cast(&bytes);
    const val = ptr.c;
    try expect(val == 5);
}
...
test "reinterpret bytes of an extern struct (with under-aligned fields) into another" {
    ...
    try test_reinterpret_extern_struct_as_extern_struct();
    try comptime test_reinterpret_extern_struct_as_extern_struct();
}
...
fn test_reinterpret_extern_struct_as_extern_struct() !void {
    ...
    const ptr: *const S2 = @ptr_cast(&bytes);
    const val = ptr.c;
    try expect(val == 5);
}
...
test "reinterpret bytes of an extern struct into another" {
    ...
    try test_reinterpret_over_aligned_extern_struct_as_extern_struct();
    try comptime test_reinterpret_over_aligned_extern_struct_as_extern_struct();
}
...
test "lower reinterpreted comptime field ptr (with under-aligned fields)" {
    ...
    comptime var ptr = @as(*const S, @ptr_cast(&bytes));
    const val = &ptr.c;
    try expect(val.* == 5);

    // Test lowering an elem ptr
    comptime var src_value = S{ .a = 15, .c = 5 };
    comptime var ptr2 = @as(*[@size_of(S)]u8, @ptr_cast(&src_value));
    const val2 = &ptr2[4];
    try expect(val2.* == 5);
}
...
const Bytes = struct {
    ...
    pub fn init(v: u32) Bytes {
        ...
        @as(*align(1) u32, @ptr_cast(&res.bytes)).* = v;
...
test "ptrcast of const integer has the correct object size" {
    ...
    const is_value = ~@as(isize, @int_cast(std.math.min_int(isize)));
    const is_bytes = @as([*]const u8, @ptr_cast(&is_value))[0..@size_of(isize)];
    if (@size_of(isize) == 8) {
...
test "implicit optional pointer to optional anyopaque pointer" {
    ...
    const x: ?[*]u8 = &buf;
    const y: ?*anyopaque = x;
    const z: *[4]u8 = @ptr_cast(y);
    try expect(std.mem.eql(u8, z, "aoeu"));
}

test "@ptr_cast slice to slice" {
    if (builtin.zig_backend == .stage2_arm) return error.SkipZigTest; // TODO
    if (builtin.zig_backend == .stage2_aarch64) return error.SkipZigTest; // TODO
    if (builtin.zig_backend == .stage2_sparc64) return error.SkipZigTest; // TODO
    if (builtin.zig_backend == .stage2_riscv64) return error.SkipZigTest;
...
test "comptime @ptr_cast a subset of an array, then write through it" {
    if (builtin.zig_backend == .stage2_arm) return error.SkipZigTest; // TODO
    if (builtin.zig_backend == .stage2_aarch64) return error.SkipZigTest; // TODO
    if (builtin.zig_backend == .stage2_sparc64) return error.SkipZigTest; // TODO
    ...
    comptime {
        ...
        const len_bytes = @as(*u32, @ptr_cast(&buff));
        len_bytes.* = 16;
        const source = "abcdef";
...
test "@ptr_cast undefined value at comptime" {
    const S = struct {
        fn transmute(comptime T: type, comptime U: type, value: T) U {
            return @as(*const U, @ptr_cast(&value)).*;
        }
    };
    comptime {
...
test "comptime @ptr_cast with packed struct leaves value unmodified" {
    if (builtin.zig_backend == .stage2_spirv64) return error.SkipZigTest;

    const S = packed struct { three: u3 };
    const st: S = .{ .three = 6 };
    try expect(st.three == 6);
    const p: *const [1]u3 = @ptr_cast(&st);
    try expect(p.*[0] == 6);
    try expect(st.three == 6);
}

test "@ptr_cast restructures comptime-only array" {
    {
        ...
        const a2a3: *const [2][3]comptime_int = @ptr_cast(&a3a2);
...

    {
        ...
        const a1a2a3: *const [1][2][3]comptime_int = @ptr_cast(&a6a1);
...

    {
        ...
        const raw: *const comptime_int = @ptr_cast(&a1);
        comptime assert(raw.* == 123);
    }

    {
        const raw: comptime_int = 123;
        const a1: *const [1]comptime_int = @ptr_cast(&raw);
        comptime assert(a1[0] == 123);
    }
}

test "@ptr_cast restructures sliced comptime-only array" {
    ...
    const sub: *const [4]comptime_int = @ptr_cast(a3a2[1..]);
