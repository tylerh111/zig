...
const math = std.math;
const qnan128: f128 = @bit_cast(@as(u128, 0x7fff800000000000) << 64);
const inf128: f128 = @bit_cast(@as(u128, 0x7fff000000000000) << 64);
...
fn test__multf3(a: f128, b: f128, expected_hi: u64, expected_lo: u64) !void {
    ...
    if (compare_result_ld(x, expected_hi, expected_lo))
        return;
...
test "multf3" {
    ...
    // NaN * any = NaN
    const a = make_na_n128(0x800030000000);
    ...
    // any * any
    try test__multf3(
        @as(f128, @bit_cast(@as(u128, 0x40042eab345678439abcdefea5678234))),
        @as(f128, @bit_cast(@as(u128, 0x3ffeedcb34a235253948765432134675))),
        0x400423e7f9e3c9fc,
        0xd906c2c2a85777c4,
    );

    try test__multf3(
        @as(f128, @bit_cast(@as(u128, 0x3fcd353e45674d89abacc3a2ebf3ff50))),
        @as(f128, @bit_cast(@as(u128, 0x3ff6ed8764648369535adf4be3214568))),
        0x3fc52a163c6223fc,
        0xc94c4bf0430768b4,
    );
    ...
    try test__multf3(
        @as(f128, @bit_cast(@as(u128, 0x3f154356473c82a9fabf2d22ace345df))),
        @as(f128, @bit_cast(@as(u128, 0x3e38eda98765476743ab21da23d45679))),
        0x3d4f37c1a3137cae,
        0xfc6807048bc2836a,
    );
    ...
    try test__multf3(2.0, math.float_true_min(f128), 0x0000_0000_0000_0000, 0x0000_0000_0000_0002);
}

const qnan80: f80 = @bit_cast(@as(u80, @bit_cast(math.nan(f80))) | (1 << (math.float_fractional_bits(f80) - 1)));

fn test__mulxf3(a: f80, b: f80, expected: u80) !void {
    const x = __mulxf3(a, b);
    const rep: u80 = @bit_cast(x);

    if (rep == expected)
        return;

    if (math.is_nan(@as(f80, @bit_cast(expected))) and math.is_nan(x))
...
test "mulxf3" {
    // NaN * any = NaN
    try test__mulxf3(qnan80, 0x1.23456789abcdefp+5, @as(u80, @bit_cast(qnan80)));
    try test__mulxf3(@as(f80, @bit_cast(@as(u80, 0x7fff_8000_8000_3000_0000))), 0x1.23456789abcdefp+5, @as(u80, @bit_cast(qnan80)));

    // any * NaN = NaN
    try test__mulxf3(0x1.23456789abcdefp+5, qnan80, @as(u80, @bit_cast(qnan80)));
    try test__mulxf3(0x1.23456789abcdefp+5, @as(f80, @bit_cast(@as(u80, 0x7fff_8000_8000_3000_0000))), @as(u80, @bit_cast(qnan80)));

    // NaN * inf = NaN
    try test__mulxf3(qnan80, math.inf(f80), @as(u80, @bit_cast(qnan80)));

    // inf * NaN = NaN
    try test__mulxf3(math.inf(f80), qnan80, @as(u80, @bit_cast(qnan80)));

    // inf * inf = inf
    try test__mulxf3(math.inf(f80), math.inf(f80), @as(u80, @bit_cast(math.inf(f80))));
    ...
    // inf * any = inf
    try test__mulxf3(math.inf(f80), 0x1.2335653452436234723489432abcdefp+5, @as(u80, @bit_cast(math.inf(f80))));

    // any * inf = inf
    try test__mulxf3(0x1.2335653452436234723489432abcdefp+5, math.inf(f80), @as(u80, @bit_cast(math.inf(f80))));
