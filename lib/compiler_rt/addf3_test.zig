...
test "addtf3" {
    ...
    // NaN + any = NaN
    try test__addtf3(@as(f128, @bit_cast((@as(u128, 0x7fff000000000000) << 64) | @as(u128, 0x800030000000))), 0x1.23456789abcdefp+5, 0x7fff800000000000, 0x0);
...
test "subtf3" {
    ...
    // NaN + any = NaN
    try test__subtf3(@as(f128, @bit_cast((@as(u128, 0x7fff000000000000) << 64) | @as(u128, 0x800030000000))), 0x1.23456789abcdefp+5, 0x7fff800000000000, 0x0);
...
test "addxf3" {
    // NaN + any = NaN
    try test__addxf3(qnan80, 0x1.23456789abcdefp+5, @as(u80, @bit_cast(qnan80)));
    try test__addxf3(@as(f80, @bit_cast(@as(u80, 0x7fff_8000_8000_3000_0000))), 0x1.23456789abcdefp+5, @as(u80, @bit_cast(qnan80)));

    // any + NaN = NaN
    try test__addxf3(0x1.23456789abcdefp+5, qnan80, @as(u80, @bit_cast(qnan80)));
    try test__addxf3(0x1.23456789abcdefp+5, @as(f80, @bit_cast(@as(u80, 0x7fff_8000_8000_3000_0000))), @as(u80, @bit_cast(qnan80)));

    // NaN + inf = NaN
    try test__addxf3(qnan80, math.inf(f80), @as(u80, @bit_cast(qnan80)));

    // inf + NaN = NaN
    try test__addxf3(math.inf(f80), qnan80, @as(u80, @bit_cast(qnan80)));

    // inf + inf = inf
    try test__addxf3(math.inf(f80), math.inf(f80), @as(u80, @bit_cast(math.inf(f80))));

    // inf + -inf = NaN
    try test__addxf3(math.inf(f80), -math.inf(f80), @as(u80, @bit_cast(qnan80)));

    // -inf + inf = NaN
    try test__addxf3(-math.inf(f80), math.inf(f80), @as(u80, @bit_cast(qnan80)));

    // inf + any = inf
    try test__addxf3(math.inf(f80), 0x1.2335653452436234723489432abcdefp+5, @as(u80, @bit_cast(math.inf(f80))));

    // any + inf = inf
    try test__addxf3(0x1.2335653452436234723489432abcdefp+5, math.inf(f80), @as(u80, @bit_cast(math.inf(f80))));
