...
// - mulo_xi4_generic_small as default
// - mulo_xi4_generic_fast for 2*bitsize <= usize

inline fn mulo_xi4_generic_small(comptime ST: type, a: ST, b: ST, overflow: *c_int) ST {
    overflow.* = 0;
    const min = math.min_int(ST);
    const res: ST = a *% b;
    ...
    if ((a < 0 and b == min) or (a != 0 and @div_trunc(res, a) != b))
        overflow.* = 1;
    return res;
}
...
pub fn __mulosi4(a: i32, b: i32, overflow: *c_int) callconv(.C) i32 {
    if (2 * @bitSizeOf(i32) <= @bitSizeOf(usize)) {
        return mulo_xi4_generic_fast(i32, a, b, overflow);
    } else {
        return mulo_xi4_generic_small(i32, a, b, overflow);
    }
}

pub fn __mulodi4(a: i64, b: i64, overflow: *c_int) callconv(.C) i64 {
    if (2 * @bitSizeOf(i64) <= @bitSizeOf(usize)) {
        return mulo_xi4_generic_fast(i64, a, b, overflow);
    } else {
        return mulo_xi4_generic_small(i64, a, b, overflow);
    }
}

pub fn __muloti4(a: i128, b: i128, overflow: *c_int) callconv(.C) i128 {
    if (2 * @bitSizeOf(i128) <= @bitSizeOf(usize)) {
        return mulo_xi4_generic_fast(i128, a, b, overflow);
    } else {
        return mulo_xi4_generic_small(i128, a, b, overflow);
    }
}
