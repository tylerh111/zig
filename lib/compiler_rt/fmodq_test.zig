...
fn test_fmodq_nans() !void {
    try testing.expect(std.math.is_nan(fmod.fmodq(1.0, std.math.nan(f128))));
    try testing.expect(std.math.is_nan(fmod.fmodq(1.0, -std.math.nan(f128))));
    try testing.expect(std.math.is_nan(fmod.fmodq(std.math.nan(f128), 1.0)));
    try testing.expect(std.math.is_nan(fmod.fmodq(-std.math.nan(f128), 1.0)));
}
...
fn test_fmodq_infs() !void {
    ...
    try testing.expect(fmod.fmodq(1.0, -std.math.inf(f128)) == 1.0);
    try testing.expect(std.math.is_nan(fmod.fmodq(std.math.inf(f128), 1.0)));
    try testing.expect(std.math.is_nan(fmod.fmodq(-std.math.inf(f128), 1.0)));
}
