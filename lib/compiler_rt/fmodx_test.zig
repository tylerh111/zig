...
fn test_fmodx_nans() !void {
    try testing.expect(std.math.is_nan(fmod.__fmodx(1.0, std.math.nan(f80))));
    try testing.expect(std.math.is_nan(fmod.__fmodx(1.0, -std.math.nan(f80))));
    try testing.expect(std.math.is_nan(fmod.__fmodx(std.math.nan(f80), 1.0)));
    try testing.expect(std.math.is_nan(fmod.__fmodx(-std.math.nan(f80), 1.0)));
}
...
fn test_fmodx_infs() !void {
    ...
    try testing.expect(fmod.__fmodx(1.0, -std.math.inf(f80)) == 1.0);
    try testing.expect(std.math.is_nan(fmod.__fmodx(std.math.inf(f80), 1.0)));
    try testing.expect(std.math.is_nan(fmod.__fmodx(-std.math.inf(f80), 1.0)));
}
