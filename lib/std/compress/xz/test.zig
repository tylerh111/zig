...
fn decompress(data: []const u8) ![]u8 {
    ...
    return xz_stream.reader().read_all_alloc(testing.allocator, std.math.max_int(usize));
}
...
test "compressed data" {
    try test_reader(@embed_file("testdata/good-0-empty.xz"), "");
    ...
    inline for ([_][]const u8{
        ...
        try test_reader(@embed_file("testdata/" ++ filename),
            \\Hello
            \\World!
            \\
        );
    ...
    try test_reader(@embed_file("testdata/good-1-lzma2-5.xz"), "");
}
...
test "unsupported" {
    inline for ([_][]const u8{
        ...
        try testing.expect_error(
            error.Unsupported,
            decompress(@embed_file("testdata/" ++ filename)),
        );
    }
}
...
test "size fields: integer overflow avoidance" {
    ...
    try test_dont_panic(backward_size_overflow);
}
