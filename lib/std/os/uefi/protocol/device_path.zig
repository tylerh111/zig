...
pub const DevicePath = extern struct {
    ...
    pub fn next(self: *DevicePath) ?*DevicePath {
        ...
        return @as(*DevicePath, @ptr_cast(@as([*]u8, @ptr_cast(self)) + self.length));
    }
    ...
    pub fn size(self: *DevicePath) usize {
        ...
        return (@int_from_ptr(node) + node.length) - @int_from_ptr(self);
    }
    ...
    pub fn create_file_device_path(self: *DevicePath, allocator: Allocator, path: [:0]align(1) const u16) !*DevicePath {
        ...
        var buf = try allocator.alloc(u8, path_size + 2 * (path.len + 1) + @size_of(DevicePath));

        @memcpy(buf[0..path_size], @as([*]const u8, @ptr_cast(self))[0..path_size]);
        ...
        var new = @as(*uefi.DevicePath.Media.FilePathDevicePath, @ptr_cast(buf.ptr + path_size - 4));

        new.type = .Media;
        new.subtype = .FilePath;
        new.length = @size_of(uefi.DevicePath.Media.FilePathDevicePath) + 2 * (@as(u16, @int_cast(path.len)) + 1);

        // The same as new.get_path(), but not const as we're filling it in.
        var ptr = @as([*:0]align(1) u16, @ptr_cast(@as([*]u8, @ptr_cast(new)) + @size_of(uefi.DevicePath.Media.FilePathDevicePath)));
        ...
        var end = @as(*uefi.DevicePath.End.EndEntireDevicePath, @ptr_cast(@as(*DevicePath, @ptr_cast(new)).next().?));
        end.type = .End;
        end.subtype = .EndEntire;
        end.length = @size_of(uefi.DevicePath.End.EndEntireDevicePath);

        return @as(*DevicePath, @ptr_cast(buf.ptr));
    }

    pub fn get_device_path(self: *const DevicePath) ?uefi.DevicePath {
        inline for (@typeInfo(uefi.DevicePath).Union.fields) |ufield| {
            const enum_value = std.meta.string_to_enum(uefi.DevicePath.Type, ufield.name);

            // Got the associated union type for self.type, now
            // we need to initialize it and its subtype
            if (self.type == enum_value) {
                const subtype = self.init_subtype(ufield.type);
                if (subtype) |sb| {
                    // e.g. return .{ .Hardware = .{ .Pci = @ptr_cast(...) } }
                    return @union_init(uefi.DevicePath, ufield.name, sb);
                }
            }
        }
    ...
    pub fn init_subtype(self: *const DevicePath, comptime TUnion: type) ?TUnion {
        ...
        inline for (type_info.fields) |subtype| {
            ...
            const tag_val: u8 = @int_from_enum(@field(TTag, subtype.name));

            if (self.subtype == tag_val) {
                // e.g. expr = .{ .Pci = @ptr_cast(...) }
                return @union_init(TUnion, subtype.name, @as(subtype.type, @ptr_cast(self)));
            }
        }
...
comptime {
    assert(4 == @size_of(DevicePath));
    assert(1 == @alignOf(DevicePath));

    assert(0 == @offset_of(DevicePath, "type"));
    assert(1 == @offset_of(DevicePath, "subtype"));
    assert(2 == @offset_of(DevicePath, "length"));
}
