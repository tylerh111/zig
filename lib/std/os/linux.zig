...
const is_mips = native_arch.is_mips();
const is_ppc = native_arch.is_ppc();
const is_ppc64 = native_arch.is_ppc64();
const is_sparc = native_arch.is_sparc();
...
pub const SYS = switch (@import("builtin").cpu.arch) {
    ...
    else => @compile_error("The Zig Standard Library is missing syscall definitions for the target CPU architecture"),
};
...
pub const MAP = switch (native_arch) {
    ...
    else => @compile_error("missing std.os.linux.MAP constants for this architecture"),
};
...
pub const O = switch (native_arch) {
    ...
    else => @compile_error("missing std.os.linux.O constants for this architecture"),
};
...
const require_aligned_register_pair =
    builtin.cpu.arch.is_ppc() or
    builtin.cpu.arch.is_mips() or
    builtin.cpu.arch.is_arm() or
    builtin.cpu.arch.is_thumb();

// Split a 64bit value into a {LSB,MSB} pair.
// The LE/BE variants specify the endianness to assume.
fn split_value_le64(val: i64) [2]u32 {
    const u: u64 = @bit_cast(val);
    return [2]u32{
        @as(u32, @truncate(u)),
...
pub fn dup(old: i32) usize {
    return syscall1(.dup, @as(usize, @bit_cast(@as(isize, old))));
}

pub fn dup2(old: i32, new: i32) usize {
    if (@has_field(SYS, "dup2")) {
        return syscall2(.dup2, @as(usize, @bit_cast(@as(isize, old))), @as(usize, @bit_cast(@as(isize, new))));
    } else {
        if (old == new) {
            if (std.debug.runtime_safety) {
                const rc = syscall2(.fcntl, @as(usize, @bit_cast(@as(isize, old))), F.GETFD);
                if (@as(isize, @bit_cast(rc)) < 0) return rc;
            }
            return @as(usize, @int_cast(old));
        } else {
            return syscall3(.dup3, @as(usize, @bit_cast(@as(isize, old))), @as(usize, @bit_cast(@as(isize, new))), 0);
        }
    }
}

pub fn dup3(old: i32, new: i32, flags: u32) usize {
    return syscall3(.dup3, @as(usize, @bit_cast(@as(isize, old))), @as(usize, @bit_cast(@as(isize, new))), flags);
}

pub fn chdir(path: [*:0]const u8) usize {
    return syscall1(.chdir, @int_from_ptr(path));
}

pub fn fchdir(fd: fd_t) usize {
    return syscall1(.fchdir, @as(usize, @bit_cast(@as(isize, fd))));
}

pub fn chroot(path: [*:0]const u8) usize {
    return syscall1(.chroot, @int_from_ptr(path));
}

pub fn execve(path: [*:0]const u8, argv: [*:null]const ?[*:0]const u8, envp: [*:null]const ?[*:0]const u8) usize {
    return syscall3(.execve, @int_from_ptr(path), @int_from_ptr(argv), @int_from_ptr(envp));
}

pub fn fork() usize {
    if (comptime native_arch.is_sparc()) {
        return syscall_fork();
    } else if (@has_field(SYS, "fork")) {
...
pub fn utimensat(dirfd: i32, path: ?[*:0]const u8, times: *const [2]timespec, flags: u32) usize {
    return syscall4(.utimensat, @as(usize, @bit_cast(@as(isize, dirfd))), @int_from_ptr(path), @int_from_ptr(times), flags);
}

pub fn fallocate(fd: i32, mode: i32, offset: i64, length: i64) usize {
    if (usize_bits < 64) {
        const offset_halves = split_value64(offset);
        const length_halves = split_value64(length);
        return syscall6(
            .fallocate,
            @as(usize, @bit_cast(@as(isize, fd))),
            @as(usize, @bit_cast(@as(isize, mode))),
            offset_halves[0],
            offset_halves[1],
            length_halves[0],
            length_halves[1],
        ...
        return syscall4(
            ...
            @as(usize, @bit_cast(@as(isize, fd))),
            @as(usize, @bit_cast(@as(isize, mode))),
            @as(u64, @bit_cast(offset)),
            @as(u64, @bit_cast(length)),
        );
    }
}

pub fn futex_wait(uaddr: *const i32, futex_op: u32, val: i32, timeout: ?*const timespec) usize {
    return syscall4(.futex, @int_from_ptr(uaddr), futex_op, @as(u32, @bit_cast(val)), @int_from_ptr(timeout));
}

pub fn futex_wake(uaddr: *const i32, futex_op: u32, val: i32) usize {
    return syscall3(.futex, @int_from_ptr(uaddr), futex_op, @as(u32, @bit_cast(val)));
}
...
pub fn futex2_waitv(
    ...
    return syscall6(
        .futex_waitv,
        @int_from_ptr(waiters),
        nr_futexes,
        flags,
        @int_from_ptr(timeout),
        @bit_cast(@as(isize, clockid)),
    );
}
...
pub fn futex2_requeue(
    ...
    return syscall4(
        ...
        @int_from_ptr(waiters),
        flags,
        @bit_cast(@as(isize, nr_wake)),
        @bit_cast(@as(isize, nr_requeue)),
    );
}

pub fn getcwd(buf: [*]u8, size: usize) usize {
    return syscall2(.getcwd, @int_from_ptr(buf), size);
}
...
pub fn inotify_add_watch(fd: i32, pathname: [*:0]const u8, mask: u32) usize {
    return syscall3(.inotify_add_watch, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(pathname), mask);
}

pub fn inotify_rm_watch(fd: i32, wd: i32) usize {
    return syscall2(.inotify_rm_watch, @as(usize, @bit_cast(@as(isize, fd))), @as(usize, @bit_cast(@as(isize, wd))));
}
...
pub fn fanotify_mark(fd: i32, flags: u32, mask: u64, dirfd: i32, pathname: ?[*:0]const u8) usize {
    return syscall5(.fanotify_mark, @as(usize, @bit_cast(@as(isize, fd))), flags, mask, @as(usize, @bit_cast(@as(isize, dirfd))), @int_from_ptr(pathname));
}

pub fn readlink(noalias path: [*:0]const u8, noalias buf_ptr: [*]u8, buf_len: usize) usize {
    if (@has_field(SYS, "readlink")) {
        return syscall3(.readlink, @int_from_ptr(path), @int_from_ptr(buf_ptr), buf_len);
    } else {
        return syscall4(.readlinkat, @as(usize, @bit_cast(@as(isize, AT.FDCWD))), @int_from_ptr(path), @int_from_ptr(buf_ptr), buf_len);
    }
}

pub fn readlinkat(dirfd: i32, noalias path: [*:0]const u8, noalias buf_ptr: [*]u8, buf_len: usize) usize {
    return syscall4(.readlinkat, @as(usize, @bit_cast(@as(isize, dirfd))), @int_from_ptr(path), @int_from_ptr(buf_ptr), buf_len);
}

pub fn mkdir(path: [*:0]const u8, mode: u32) usize {
    if (@has_field(SYS, "mkdir")) {
        return syscall2(.mkdir, @int_from_ptr(path), mode);
    } else {
        return syscall3(.mkdirat, @as(usize, @bit_cast(@as(isize, AT.FDCWD))), @int_from_ptr(path), mode);
    }
}

pub fn mkdirat(dirfd: i32, path: [*:0]const u8, mode: u32) usize {
    return syscall3(.mkdirat, @as(usize, @bit_cast(@as(isize, dirfd))), @int_from_ptr(path), mode);
}

pub fn mknod(path: [*:0]const u8, mode: u32, dev: u32) usize {
    if (@has_field(SYS, "mknod")) {
        return syscall3(.mknod, @int_from_ptr(path), mode, dev);
    } else {
...
pub fn mknodat(dirfd: i32, path: [*:0]const u8, mode: u32, dev: u32) usize {
    return syscall4(.mknodat, @as(usize, @bit_cast(@as(isize, dirfd))), @int_from_ptr(path), mode, dev);
}

pub fn mount(special: [*:0]const u8, dir: [*:0]const u8, fstype: ?[*:0]const u8, flags: u32, data: usize) usize {
    return syscall5(.mount, @int_from_ptr(special), @int_from_ptr(dir), @int_from_ptr(fstype), flags, data);
}

pub fn umount(special: [*:0]const u8) usize {
    return syscall2(.umount2, @int_from_ptr(special), 0);
}

pub fn umount2(special: [*:0]const u8, flags: u32) usize {
    return syscall2(.umount2, @int_from_ptr(special), flags);
}
...
pub fn mmap(address: ?[*]u8, length: usize, prot: usize, flags: MAP, fd: i32, offset: i64) usize {
    if (@has_field(SYS, "mmap2")) {
        ...
        if ((offset & (MMAP2_UNIT - 1)) != 0)
            return @bit_cast(-@as(isize, @int_from_enum(E.INVAL)));

        return syscall6(
            .mmap2,
            @int_from_ptr(address),
            length,
            prot,
            @as(u32, @bit_cast(flags)),
            @bit_cast(@as(isize, fd)),
            @truncate(@as(u64, @bit_cast(offset)) / MMAP2_UNIT),
        );
    } else {
        return syscall6(
            .mmap,
...
pub fn mprotect(address: [*]const u8, length: usize, protection: usize) usize {
    return syscall3(.mprotect, @int_from_ptr(address), length, protection);
}
...
pub fn msync(address: [*]const u8, length: usize, flags: i32) usize {
    return syscall3(.msync, @int_from_ptr(address), length, @as(u32, @bit_cast(flags)));
}

pub fn munmap(address: [*]const u8, length: usize) usize {
    return syscall2(.munmap, @int_from_ptr(address), length);
}

pub fn poll(fds: [*]pollfd, n: nfds_t, timeout: i32) usize {
    if (@has_field(SYS, "poll")) {
        return syscall3(.poll, @int_from_ptr(fds), n, @as(u32, @bit_cast(timeout)));
    } else {
        return syscall5(
            .ppoll,
            @int_from_ptr(fds),
            n,
            @int_from_ptr(if (timeout >= 0)
                &timespec{
                    .tv_sec = @div_trunc(timeout, 1000),
...
pub fn ppoll(fds: [*]pollfd, n: nfds_t, timeout: ?*timespec, sigmask: ?*const sigset_t) usize {
    return syscall5(.ppoll, @int_from_ptr(fds), n, @int_from_ptr(timeout), @int_from_ptr(sigmask), NSIG / 8);
}

pub fn read(fd: i32, buf: [*]u8, count: usize) usize {
    return syscall3(.read, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(buf), count);
}

pub fn preadv(fd: i32, iov: [*]const iovec, count: usize, offset: i64) usize {
    const offset_u: u64 = @bit_cast(offset);
    return syscall5(
        .preadv,
        @as(usize, @bit_cast(@as(isize, fd))),
        @int_from_ptr(iov),
...
pub fn readv(fd: i32, iov: [*]const iovec, count: usize) usize {
    return syscall3(.readv, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(iov), count);
}

pub fn writev(fd: i32, iov: [*]const iovec_const, count: usize) usize {
    return syscall3(.writev, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(iov), count);
}
...
pub fn rmdir(path: [*:0]const u8) usize {
    if (@has_field(SYS, "rmdir")) {
        return syscall1(.rmdir, @int_from_ptr(path));
    } else {
        return syscall3(.unlinkat, @as(usize, @bit_cast(@as(isize, AT.FDCWD))), @int_from_ptr(path), AT.REMOVEDIR);
    }
}

pub fn symlink(existing: [*:0]const u8, new: [*:0]const u8) usize {
    if (@has_field(SYS, "symlink")) {
        return syscall2(.symlink, @int_from_ptr(existing), @int_from_ptr(new));
    } else {
        return syscall3(.symlinkat, @int_from_ptr(existing), @as(usize, @bit_cast(@as(isize, AT.FDCWD))), @int_from_ptr(new));
    }
}

pub fn symlinkat(existing: [*:0]const u8, newfd: i32, newpath: [*:0]const u8) usize {
    return syscall3(.symlinkat, @int_from_ptr(existing), @as(usize, @bit_cast(@as(isize, newfd))), @int_from_ptr(newpath));
}

pub fn pread(fd: i32, buf: [*]u8, count: usize, offset: i64) usize {
    if (@has_field(SYS, "pread64") and usize_bits < 64) {
        const offset_halves = split_value64(offset);
        ...
        const syscall_number = if (!@has_field(SYS, "pread") and @has_field(SYS, "pread64"))
            .pread64
        else
            .pread;
        return syscall4(
...
pub fn access(path: [*:0]const u8, mode: u32) usize {
    if (@has_field(SYS, "access")) {
        return syscall2(.access, @int_from_ptr(path), mode);
    } else {
        return syscall4(.faccessat, @as(usize, @bit_cast(@as(isize, AT.FDCWD))), @int_from_ptr(path), mode, 0);
    }
}

pub fn faccessat(dirfd: i32, path: [*:0]const u8, mode: u32, flags: u32) usize {
    return syscall4(.faccessat, @as(usize, @bit_cast(@as(isize, dirfd))), @int_from_ptr(path), mode, flags);
}

pub fn pipe(fd: *[2]i32) usize {
    if (comptime (native_arch.is_mips() or native_arch.is_sparc())) {
        return syscall_pipe(fd);
    } else if (@has_field(SYS, "pipe")) {
        return syscall1(.pipe, @int_from_ptr(fd));
    } else {
        return syscall2(.pipe2, @int_from_ptr(fd), 0);
    }
}

pub fn pipe2(fd: *[2]i32, flags: O) usize {
    return syscall2(.pipe2, @int_from_ptr(fd), @as(u32, @bit_cast(flags)));
}

pub fn write(fd: i32, buf: [*]const u8, count: usize) usize {
    return syscall3(.write, @bit_cast(@as(isize, fd)), @int_from_ptr(buf), count);
}

pub fn ftruncate(fd: i32, length: i64) usize {
    if (@has_field(SYS, "ftruncate64") and usize_bits < 64) {
        const length_halves = split_value64(length);
        if (require_aligned_register_pair) {
            return syscall4(
                .ftruncate64,
        ...
        return syscall2(
            .ftruncate,
            @as(usize, @bit_cast(@as(isize, fd))),
            @as(usize, @bit_cast(length)),
        );
    }
}

pub fn pwrite(fd: i32, buf: [*]const u8, count: usize, offset: i64) usize {
    if (@has_field(SYS, "pwrite64") and usize_bits < 64) {
        const offset_halves = split_value64(offset);
        ...
        const syscall_number = if (!@has_field(SYS, "pwrite") and @has_field(SYS, "pwrite64"))
            .pwrite64
        else
            .pwrite;
        return syscall4(
...
pub fn rename(old: [*:0]const u8, new: [*:0]const u8) usize {
    if (@has_field(SYS, "rename")) {
        return syscall2(.rename, @int_from_ptr(old), @int_from_ptr(new));
    } else if (@has_field(SYS, "renameat")) {
        return syscall4(.renameat, @as(usize, @bit_cast(@as(isize, AT.FDCWD))), @int_from_ptr(old), @as(usize, @bit_cast(@as(isize, AT.FDCWD))), @int_from_ptr(new));
    } else {
        return syscall5(.renameat2, @as(usize, @bit_cast(@as(isize, AT.FDCWD))), @int_from_ptr(old), @as(usize, @bit_cast(@as(isize, AT.FDCWD))), @int_from_ptr(new), 0);
    }
}

pub fn renameat(oldfd: i32, oldpath: [*:0]const u8, newfd: i32, newpath: [*:0]const u8) usize {
    if (@has_field(SYS, "renameat")) {
        return syscall4(
            .renameat,
            @as(usize, @bit_cast(@as(isize, oldfd))),
            @int_from_ptr(oldpath),
            @as(usize, @bit_cast(@as(isize, newfd))),
            @int_from_ptr(newpath),
        );
    } else {
        return syscall5(
            .renameat2,
...
pub fn open(path: [*:0]const u8, flags: O, perm: mode_t) usize {
    if (@has_field(SYS, "open")) {
        return syscall3(.open, @int_from_ptr(path), @as(u32, @bit_cast(flags)), perm);
    } else {
        return syscall4(
            .openat,
            @bit_cast(@as(isize, AT.FDCWD)),
            @int_from_ptr(path),
            @as(u32, @bit_cast(flags)),
            perm,
        );
...
pub fn create(path: [*:0]const u8, perm: mode_t) usize {
    return syscall2(.creat, @int_from_ptr(path), perm);
}

pub fn openat(dirfd: i32, path: [*:0]const u8, flags: O, mode: mode_t) usize {
    // dirfd could be negative, for example AT.FDCWD is -100
    return syscall4(.openat, @bit_cast(@as(isize, dirfd)), @int_from_ptr(path), @as(u32, @bit_cast(flags)), mode);
}

/// See also `clone` (from the arch-specific include)
pub fn clone5(flags: usize, child_stack_ptr: usize, parent_tid: *i32, child_tid: *i32, newtls: usize) usize {
    return syscall5(.clone, flags, child_stack_ptr, @int_from_ptr(parent_tid), @int_from_ptr(child_tid), newtls);
}
...
pub fn close(fd: i32) usize {
    return syscall1(.close, @as(usize, @bit_cast(@as(isize, fd))));
}

pub fn fchmod(fd: i32, mode: mode_t) usize {
    return syscall2(.fchmod, @as(usize, @bit_cast(@as(isize, fd))), mode);
}

pub fn chmod(path: [*:0]const u8, mode: mode_t) usize {
    if (@has_field(SYS, "chmod")) {
        return syscall2(.chmod, @int_from_ptr(path), mode);
    } else {
...
pub fn fchown(fd: i32, owner: uid_t, group: gid_t) usize {
    if (@has_field(SYS, "fchown32")) {
        return syscall3(.fchown32, @as(usize, @bit_cast(@as(isize, fd))), owner, group);
    } else {
        return syscall3(.fchown, @as(usize, @bit_cast(@as(isize, fd))), owner, group);
    }
}

pub fn fchmodat(fd: i32, path: [*:0]const u8, mode: mode_t, _: u32) usize {
    return syscall3(.fchmodat, @bit_cast(@as(isize, fd)), @int_from_ptr(path), mode);
}

pub fn fchmodat2(fd: i32, path: [*:0]const u8, mode: mode_t, flags: u32) usize {
    return syscall4(.fchmodat2, @bit_cast(@as(isize, fd)), @int_from_ptr(path), mode, flags);
}
...
pub fn lseek(fd: i32, offset: i64, whence: usize) usize {
    return syscall3(.lseek, @as(usize, @bit_cast(@as(isize, fd))), @as(usize, @bit_cast(offset)), whence);
}

pub fn exit(status: i32) noreturn {
    _ = syscall1(.exit, @as(usize, @bit_cast(@as(isize, status))));
    unreachable;
}

pub fn exit_group(status: i32) noreturn {
    _ = syscall1(.exit_group, @as(usize, @bit_cast(@as(isize, status))));
    unreachable;
}
...
pub fn getrandom(buf: [*]u8, count: usize, flags: u32) usize {
    return syscall3(.getrandom, @int_from_ptr(buf), count, flags);
}

pub fn kill(pid: pid_t, sig: i32) usize {
    return syscall2(.kill, @as(usize, @bit_cast(@as(isize, pid))), @as(usize, @bit_cast(@as(isize, sig))));
}

pub fn tkill(tid: pid_t, sig: i32) usize {
    return syscall2(.tkill, @as(usize, @bit_cast(@as(isize, tid))), @as(usize, @bit_cast(@as(isize, sig))));
}

pub fn tgkill(tgid: pid_t, tid: pid_t, sig: i32) usize {
    return syscall3(.tgkill, @as(usize, @bit_cast(@as(isize, tgid))), @as(usize, @bit_cast(@as(isize, tid))), @as(usize, @bit_cast(@as(isize, sig))));
}

pub fn link(oldpath: [*:0]const u8, newpath: [*:0]const u8, flags: i32) usize {
    if (@has_field(SYS, "link")) {
        return syscall3(
            .link,
            @int_from_ptr(oldpath),
            @int_from_ptr(newpath),
            @as(usize, @bit_cast(@as(isize, flags))),
        );
    } else {
        return syscall5(
            .linkat,
            @as(usize, @bit_cast(@as(isize, AT.FDCWD))),
            @int_from_ptr(oldpath),
            @as(usize, @bit_cast(@as(isize, AT.FDCWD))),
            @int_from_ptr(newpath),
            @as(usize, @bit_cast(@as(isize, flags))),
...
pub fn unlink(path: [*:0]const u8) usize {
    if (@has_field(SYS, "unlink")) {
        return syscall1(.unlink, @int_from_ptr(path));
    } else {
        return syscall3(.unlinkat, @as(usize, @bit_cast(@as(isize, AT.FDCWD))), @int_from_ptr(path), 0);
    }
}

pub fn unlinkat(dirfd: i32, path: [*:0]const u8, flags: u32) usize {
    return syscall3(.unlinkat, @as(usize, @bit_cast(@as(isize, dirfd))), @int_from_ptr(path), flags);
}

pub fn waitpid(pid: pid_t, status: *u32, flags: u32) usize {
    return syscall4(.wait4, @as(usize, @bit_cast(@as(isize, pid))), @int_from_ptr(status), flags, 0);
}

pub fn wait4(pid: pid_t, status: *u32, flags: u32, usage: ?*rusage) usize {
    return syscall4(
        .wait4,
        @as(usize, @bit_cast(@as(isize, pid))),
        @int_from_ptr(status),
        flags,
        @int_from_ptr(usage),
    );
...
pub fn waitid(id_type: P, id: i32, infop: *siginfo_t, flags: u32) usize {
    return syscall5(.waitid, @int_from_enum(id_type), @as(usize, @bit_cast(@as(isize, id))), @int_from_ptr(infop), flags, 0);
}

pub fn fcntl(fd: fd_t, cmd: i32, arg: usize) usize {
    return syscall3(.fcntl, @as(usize, @bit_cast(@as(isize, fd))), @as(usize, @bit_cast(@as(isize, cmd))), arg);
}

pub fn flock(fd: fd_t, operation: i32) usize {
    return syscall2(.flock, @as(usize, @bit_cast(@as(isize, fd))), @as(usize, @bit_cast(@as(isize, operation))));
}
...
pub fn clock_gettime(clk_id: i32, tp: *timespec) usize {
    if (@hasDecl(VDSO, "CGT_SYM")) {
        ...
        if (ptr) |f| {
            ...
            switch (rc) {
                0, @as(usize, @bit_cast(-@as(isize, @int_from_enum(E.INVAL)))) => return rc,
                else => {},
            }
        }
    }
    return syscall2(.clock_gettime, @as(usize, @bit_cast(@as(isize, clk_id))), @int_from_ptr(tp));
}
...
fn init_vdso_clock_gettime(clk: i32, ts: *timespec) callconv(.C) usize {
    ...
    return @as(usize, @bit_cast(-@as(isize, @int_from_enum(E.NOSYS))));
}

pub fn clock_getres(clk_id: i32, tp: *timespec) usize {
    return syscall2(.clock_getres, @as(usize, @bit_cast(@as(isize, clk_id))), @int_from_ptr(tp));
}

pub fn clock_settime(clk_id: i32, tp: *const timespec) usize {
    return syscall2(.clock_settime, @as(usize, @bit_cast(@as(isize, clk_id))), @int_from_ptr(tp));
}

pub fn gettimeofday(tv: ?*timeval, tz: ?*timezone) usize {
    return syscall2(.gettimeofday, @int_from_ptr(tv), @int_from_ptr(tz));
}

pub fn settimeofday(tv: *const timeval, tz: *const timezone) usize {
    return syscall2(.settimeofday, @int_from_ptr(tv), @int_from_ptr(tz));
}

pub fn nanosleep(req: *const timespec, rem: ?*timespec) usize {
    return syscall2(.nanosleep, @int_from_ptr(req), @int_from_ptr(rem));
}

pub fn pause() usize {
    if (@has_field(SYS, "pause")) {
...
pub fn getuid() uid_t {
    if (@has_field(SYS, "getuid32")) {
        return @as(uid_t, @int_cast(syscall0(.getuid32)));
    } else {
        return @as(uid_t, @int_cast(syscall0(.getuid)));
    }
}

pub fn getgid() gid_t {
    if (@has_field(SYS, "getgid32")) {
        return @as(gid_t, @int_cast(syscall0(.getgid32)));
    } else {
        return @as(gid_t, @int_cast(syscall0(.getgid)));
    }
}

pub fn geteuid() uid_t {
    if (@has_field(SYS, "geteuid32")) {
        return @as(uid_t, @int_cast(syscall0(.geteuid32)));
    } else {
        return @as(uid_t, @int_cast(syscall0(.geteuid)));
    }
}

pub fn getegid() gid_t {
    if (@has_field(SYS, "getegid32")) {
        return @as(gid_t, @int_cast(syscall0(.getegid32)));
    } else {
        return @as(gid_t, @int_cast(syscall0(.getegid)));
    }
}
...
pub fn seteuid(euid: uid_t) usize {
    ...
    return setresuid(std.math.max_int(uid_t), euid, std.math.max_int(uid_t));
}
...
pub fn setegid(egid: gid_t) usize {
    ...
    // max value in C.
    comptime assert(@typeInfo(uid_t) == .Int and @typeInfo(uid_t).Int.signedness == .unsigned);
    return setresgid(std.math.max_int(gid_t), egid, std.math.max_int(gid_t));
}

pub fn getresuid(ruid: *uid_t, euid: *uid_t, suid: *uid_t) usize {
    if (@has_field(SYS, "getresuid32")) {
        return syscall3(.getresuid32, @int_from_ptr(ruid), @int_from_ptr(euid), @int_from_ptr(suid));
    } else {
        return syscall3(.getresuid, @int_from_ptr(ruid), @int_from_ptr(euid), @int_from_ptr(suid));
    }
}

pub fn getresgid(rgid: *gid_t, egid: *gid_t, sgid: *gid_t) usize {
    if (@has_field(SYS, "getresgid32")) {
        return syscall3(.getresgid32, @int_from_ptr(rgid), @int_from_ptr(egid), @int_from_ptr(sgid));
    } else {
        return syscall3(.getresgid, @int_from_ptr(rgid), @int_from_ptr(egid), @int_from_ptr(sgid));
    }
}

pub fn setresuid(ruid: uid_t, euid: uid_t, suid: uid_t) usize {
    if (@has_field(SYS, "setresuid32")) {
...
pub fn getgroups(size: usize, list: *gid_t) usize {
    if (@has_field(SYS, "getgroups32")) {
        return syscall2(.getgroups32, size, @int_from_ptr(list));
    } else {
        return syscall2(.getgroups, size, @int_from_ptr(list));
    }
}

pub fn setgroups(size: usize, list: [*]const gid_t) usize {
    if (@has_field(SYS, "setgroups32")) {
        return syscall2(.setgroups32, size, @int_from_ptr(list));
    } else {
        return syscall2(.setgroups, size, @int_from_ptr(list));
    }
}

pub fn setsid() pid_t {
    return @as(pid_t, @bit_cast(@as(u32, @truncate(syscall0(.setsid)))));
}

pub fn getpid() pid_t {
    return @as(pid_t, @bit_cast(@as(u32, @truncate(syscall0(.getpid)))));
}

pub fn gettid() pid_t {
    return @as(pid_t, @bit_cast(@as(u32, @truncate(syscall0(.gettid)))));
}

pub fn sigprocmask(flags: u32, noalias set: ?*const sigset_t, noalias oldset: ?*sigset_t) usize {
    return syscall4(.rt_sigprocmask, flags, @int_from_ptr(set), @int_from_ptr(oldset), NSIG / 8);
}
...
pub fn sigaction(sig: u6, noalias act: ?*const Sigaction, noalias oact: ?*Sigaction) usize {
    ...
    const mask_size = @size_of(@TypeOf(ksa.mask));
    ...
    if (act) |new| {
        ...
        ksa = k_sigaction{
            ...
            .mask = undefined,
            .restorer = @ptr_cast(restorer_fn),
        };
        @memcpy(@as([*]u8, @ptr_cast(&ksa.mask))[0..mask_size], @as([*]const u8, @ptr_cast(&new.mask)));
    }

    const ksa_arg = if (act != null) @int_from_ptr(&ksa) else 0;
    const oldksa_arg = if (oact != null) @int_from_ptr(&oldksa) else 0;
    ...
    const result = switch (native_arch) {
        ...
        .sparc, .sparc64 => syscall5(.rt_sigaction, sig, ksa_arg, oldksa_arg, @int_from_ptr(ksa.restorer), mask_size),
    ...
    if (oact) |old| {
        ...
        @memcpy(@as([*]u8, @ptr_cast(&old.mask))[0..mask_size], @as([*]const u8, @ptr_cast(&oldksa.mask)));
    }
...
pub fn sigaddset(set: *sigset_t, sig: u6) void {
    ...
    const shift = @as(u5, @int_cast(s & (usize_bits - 1)));
    const val = @as(u32, @int_cast(1)) << shift;
    (set.*)[@as(usize, @int_cast(s)) / usize_bits] |= val;
}

pub fn sigismember(set: *const sigset_t, sig: u6) bool {
    const s = sig - 1;
    return ((set.*)[@as(usize, @int_cast(s)) / usize_bits] & (@as(usize, @int_cast(1)) << (s & (usize_bits - 1)))) != 0;
}

pub fn getsockname(fd: i32, noalias addr: *sockaddr, noalias len: *socklen_t) usize {
    if (native_arch == .x86) {
        return socketcall(SC.getsockname, &[3]usize{ @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(addr), @int_from_ptr(len) });
    }
    return syscall3(.getsockname, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(addr), @int_from_ptr(len));
}

pub fn getpeername(fd: i32, noalias addr: *sockaddr, noalias len: *socklen_t) usize {
    if (native_arch == .x86) {
        return socketcall(SC.getpeername, &[3]usize{ @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(addr), @int_from_ptr(len) });
    }
    return syscall3(.getpeername, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(addr), @int_from_ptr(len));
}
...
pub fn setsockopt(fd: i32, level: i32, optname: u32, optval: [*]const u8, optlen: socklen_t) usize {
    if (native_arch == .x86) {
        return socketcall(SC.setsockopt, &[5]usize{ @as(usize, @bit_cast(@as(isize, fd))), @as(usize, @bit_cast(@as(isize, level))), optname, @int_from_ptr(optval), @as(usize, @int_cast(optlen)) });
    }
    return syscall5(.setsockopt, @as(usize, @bit_cast(@as(isize, fd))), @as(usize, @bit_cast(@as(isize, level))), optname, @int_from_ptr(optval), @as(usize, @int_cast(optlen)));
}

pub fn getsockopt(fd: i32, level: i32, optname: u32, noalias optval: [*]u8, noalias optlen: *socklen_t) usize {
    if (native_arch == .x86) {
        return socketcall(SC.getsockopt, &[5]usize{ @as(usize, @bit_cast(@as(isize, fd))), @as(usize, @bit_cast(@as(isize, level))), optname, @int_from_ptr(optval), @int_from_ptr(optlen) });
    }
    return syscall5(.getsockopt, @as(usize, @bit_cast(@as(isize, fd))), @as(usize, @bit_cast(@as(isize, level))), optname, @int_from_ptr(optval), @int_from_ptr(optlen));
}

pub fn sendmsg(fd: i32, msg: *const msghdr_const, flags: u32) usize {
    const fd_usize = @as(usize, @bit_cast(@as(isize, fd)));
    const msg_usize = @int_from_ptr(msg);
    if (native_arch == .x86) {
...
pub fn sendmmsg(fd: i32, msgvec: [*]mmsghdr_const, vlen: u32, flags: u32) usize {
    if (@typeInfo(usize).Int.bits > @typeInfo(@typeInfo(mmsghdr).Struct.fields[1].type).Int.bits) {
        ...
        for (msgvec[0..kvlen], 0..) |*msg, i| {
            var size: i32 = 0;
            const msg_iovlen = @as(usize, @int_cast(msg.msg_hdr.msg_iovlen)); // kernel side this is treated as unsigned
            for (msg.msg_hdr.msg_iov[0..msg_iovlen]) |iov| {
                if (iov.len > std.math.max_int(i32) or @add_with_overflow(size, @as(i32, @int_cast(iov.len)))[1] != 0) {
                    // batch-send all messages up to the current message
                    if (next_unsent < i) {
                        const batch_size = i - next_unsent;
                        const r = syscall4(.sendmmsg, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(&msgvec[next_unsent]), batch_size, flags);
    ...
    return syscall4(.sendmmsg, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(msgvec), vlen, flags);
}

pub fn connect(fd: i32, addr: *const anyopaque, len: socklen_t) usize {
    const fd_usize = @as(usize, @bit_cast(@as(isize, fd)));
    const addr_usize = @int_from_ptr(addr);
    if (native_arch == .x86) {
...
pub fn recvfrom(
    ...
    const fd_usize = @as(usize, @bit_cast(@as(isize, fd)));
    const buf_usize = @int_from_ptr(buf);
    const addr_usize = @int_from_ptr(addr);
    const alen_usize = @int_from_ptr(alen);
    if (native_arch == .x86) {
...
pub fn shutdown(fd: i32, how: i32) usize {
    if (native_arch == .x86) {
        return socketcall(SC.shutdown, &[2]usize{ @as(usize, @bit_cast(@as(isize, fd))), @as(usize, @bit_cast(@as(isize, how))) });
    }
    return syscall2(.shutdown, @as(usize, @bit_cast(@as(isize, fd))), @as(usize, @bit_cast(@as(isize, how))));
}

pub fn bind(fd: i32, addr: *const sockaddr, len: socklen_t) usize {
    if (native_arch == .x86) {
        return socketcall(SC.bind, &[3]usize{ @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(addr), @as(usize, @int_cast(len)) });
    }
    return syscall3(.bind, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(addr), @as(usize, @int_cast(len)));
}

pub fn listen(fd: i32, backlog: u32) usize {
    if (native_arch == .x86) {
        return socketcall(SC.listen, &[2]usize{ @as(usize, @bit_cast(@as(isize, fd))), backlog });
    }
    return syscall2(.listen, @as(usize, @bit_cast(@as(isize, fd))), backlog);
}

pub fn sendto(fd: i32, buf: [*]const u8, len: usize, flags: u32, addr: ?*const sockaddr, alen: socklen_t) usize {
    if (native_arch == .x86) {
        return socketcall(SC.sendto, &[6]usize{ @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(buf), len, flags, @int_from_ptr(addr), @as(usize, @int_cast(alen)) });
    }
    return syscall6(.sendto, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(buf), len, flags, @int_from_ptr(addr), @as(usize, @int_cast(alen)));
}

pub fn sendfile(outfd: i32, infd: i32, offset: ?*i64, count: usize) usize {
    if (@has_field(SYS, "sendfile64")) {
        return syscall4(
            .sendfile64,
            @as(usize, @bit_cast(@as(isize, outfd))),
            @as(usize, @bit_cast(@as(isize, infd))),
            @int_from_ptr(offset),
            count,
        );
    } else {
...
pub fn socketpair(domain: i32, socket_type: i32, protocol: i32, fd: *[2]i32) usize {
    if (native_arch == .x86) {
        return socketcall(SC.socketpair, &[4]usize{ @as(usize, @int_cast(domain)), @as(usize, @int_cast(socket_type)), @as(usize, @int_cast(protocol)), @int_from_ptr(fd) });
    }
    return syscall4(.socketpair, @as(usize, @int_cast(domain)), @as(usize, @int_cast(socket_type)), @as(usize, @int_cast(protocol)), @int_from_ptr(fd));
}
...
pub fn accept4(fd: i32, noalias addr: ?*sockaddr, noalias len: ?*socklen_t, flags: u32) usize {
    if (native_arch == .x86) {
        return socketcall(SC.accept4, &[4]usize{ @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(addr), @int_from_ptr(len), flags });
    }
    return syscall4(.accept4, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(addr), @int_from_ptr(len), flags);
}

pub fn fstat(fd: i32, stat_buf: *Stat) usize {
    if (@has_field(SYS, "fstat64")) {
        return syscall2(.fstat64, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(stat_buf));
    } else {
        return syscall2(.fstat, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(stat_buf));
    }
}

pub fn stat(pathname: [*:0]const u8, statbuf: *Stat) usize {
    if (@has_field(SYS, "stat64")) {
        return syscall2(.stat64, @int_from_ptr(pathname), @int_from_ptr(statbuf));
    } else {
        return syscall2(.stat, @int_from_ptr(pathname), @int_from_ptr(statbuf));
    }
}

pub fn lstat(pathname: [*:0]const u8, statbuf: *Stat) usize {
    if (@has_field(SYS, "lstat64")) {
        return syscall2(.lstat64, @int_from_ptr(pathname), @int_from_ptr(statbuf));
    } else {
        return syscall2(.lstat, @int_from_ptr(pathname), @int_from_ptr(statbuf));
    }
}

pub fn fstatat(dirfd: i32, path: [*:0]const u8, stat_buf: *Stat, flags: u32) usize {
    if (@has_field(SYS, "fstatat64")) {
        return syscall4(.fstatat64, @as(usize, @bit_cast(@as(isize, dirfd))), @int_from_ptr(path), @int_from_ptr(stat_buf), flags);
    } else {
        return syscall4(.fstatat, @as(usize, @bit_cast(@as(isize, dirfd))), @int_from_ptr(path), @int_from_ptr(stat_buf), flags);
    }
}

pub fn statx(dirfd: i32, path: [*:0]const u8, flags: u32, mask: u32, statx_buf: *Statx) usize {
    if (@has_field(SYS, "statx")) {
        return syscall5(
            .statx,
            @as(usize, @bit_cast(@as(isize, dirfd))),
            @int_from_ptr(path),
            flags,
            mask,
            @int_from_ptr(statx_buf),
        );
    }
    return @as(usize, @bit_cast(-@as(isize, @int_from_enum(E.NOSYS))));
}
...
pub fn listxattr(path: [*:0]const u8, list: [*]u8, size: usize) usize {
    return syscall3(.listxattr, @int_from_ptr(path), @int_from_ptr(list), size);
}

pub fn llistxattr(path: [*:0]const u8, list: [*]u8, size: usize) usize {
    return syscall3(.llistxattr, @int_from_ptr(path), @int_from_ptr(list), size);
}

pub fn flistxattr(fd: usize, list: [*]u8, size: usize) usize {
    return syscall3(.flistxattr, fd, @int_from_ptr(list), size);
}

pub fn getxattr(path: [*:0]const u8, name: [*:0]const u8, value: [*]u8, size: usize) usize {
    return syscall4(.getxattr, @int_from_ptr(path), @int_from_ptr(name), @int_from_ptr(value), size);
}

pub fn lgetxattr(path: [*:0]const u8, name: [*:0]const u8, value: [*]u8, size: usize) usize {
    return syscall4(.lgetxattr, @int_from_ptr(path), @int_from_ptr(name), @int_from_ptr(value), size);
}

pub fn fgetxattr(fd: usize, name: [*:0]const u8, value: [*]u8, size: usize) usize {
    return syscall4(.lgetxattr, fd, @int_from_ptr(name), @int_from_ptr(value), size);
}

pub fn setxattr(path: [*:0]const u8, name: [*:0]const u8, value: *const void, size: usize, flags: usize) usize {
    return syscall5(.setxattr, @int_from_ptr(path), @int_from_ptr(name), @int_from_ptr(value), size, flags);
}

pub fn lsetxattr(path: [*:0]const u8, name: [*:0]const u8, value: *const void, size: usize, flags: usize) usize {
    return syscall5(.lsetxattr, @int_from_ptr(path), @int_from_ptr(name), @int_from_ptr(value), size, flags);
}

pub fn fsetxattr(fd: usize, name: [*:0]const u8, value: *const void, size: usize, flags: usize) usize {
    return syscall5(.fsetxattr, fd, @int_from_ptr(name), @int_from_ptr(value), size, flags);
}

pub fn removexattr(path: [*:0]const u8, name: [*:0]const u8) usize {
    return syscall2(.removexattr, @int_from_ptr(path), @int_from_ptr(name));
}

pub fn lremovexattr(path: [*:0]const u8, name: [*:0]const u8) usize {
    return syscall2(.lremovexattr, @int_from_ptr(path), @int_from_ptr(name));
}

pub fn fremovexattr(fd: usize, name: [*:0]const u8) usize {
    return syscall2(.fremovexattr, fd, @int_from_ptr(name));
}
...
pub fn sched_getaffinity(pid: pid_t, size: usize, set: *cpu_set_t) usize {
    const rc = syscall3(.sched_getaffinity, @as(usize, @bit_cast(@as(isize, pid))), size, @int_from_ptr(set));
    if (@as(isize, @bit_cast(rc)) < 0) return rc;
    if (rc < size) @memset(@as([*]u8, @ptr_cast(set))[rc..size], 0);
    return 0;
}

pub fn sched_setaffinity(pid: pid_t, set: *const cpu_set_t) !void {
    const size = @size_of(cpu_set_t);
    const rc = syscall3(.sched_setaffinity, @as(usize, @bit_cast(@as(isize, pid))), size, @int_from_ptr(set));
...
pub fn epoll_ctl(epoll_fd: i32, op: u32, fd: i32, ev: ?*epoll_event) usize {
    return syscall4(.epoll_ctl, @as(usize, @bit_cast(@as(isize, epoll_fd))), @as(usize, @int_cast(op)), @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(ev));
}
...
pub fn epoll_pwait(epoll_fd: i32, events: [*]epoll_event, maxevents: u32, timeout: i32, sigmask: ?*const sigset_t) usize {
    return syscall6(
        .epoll_pwait,
        @as(usize, @bit_cast(@as(isize, epoll_fd))),
        @int_from_ptr(events),
        @as(usize, @int_cast(maxevents)),
        @as(usize, @bit_cast(@as(isize, timeout))),
        @int_from_ptr(sigmask),
        @size_of(sigset_t),
    );
}
...
pub fn timerfd_create(clockid: i32, flags: TFD) usize {
    return syscall2(.timerfd_create, @bit_cast(@as(isize, clockid)), @as(u32, @bit_cast(flags)));
}
...
pub fn timerfd_gettime(fd: i32, curr_value: *itimerspec) usize {
    return syscall2(.timerfd_gettime, @bit_cast(@as(isize, fd)), @int_from_ptr(curr_value));
}

pub fn timerfd_settime(fd: i32, flags: TFD.TIMER, new_value: *const itimerspec, old_value: ?*itimerspec) usize {
    return syscall4(.timerfd_settime, @bit_cast(@as(isize, fd)), @as(u32, @bit_cast(flags)), @int_from_ptr(new_value), @int_from_ptr(old_value));
}
...
pub fn getitimer(which: i32, curr_value: *itimerspec) usize {
    return syscall2(.getitimer, @as(usize, @bit_cast(@as(isize, which))), @int_from_ptr(curr_value));
}

pub fn setitimer(which: i32, new_value: *const itimerspec, old_value: ?*itimerspec) usize {
    return syscall3(.setitimer, @as(usize, @bit_cast(@as(isize, which))), @int_from_ptr(new_value), @int_from_ptr(old_value));
}
...
pub fn capget(hdrp: *cap_user_header_t, datap: *cap_user_data_t) usize {
    return syscall2(.capget, @int_from_ptr(hdrp), @int_from_ptr(datap));
}

pub fn capset(hdrp: *cap_user_header_t, datap: *const cap_user_data_t) usize {
    return syscall2(.capset, @int_from_ptr(hdrp), @int_from_ptr(datap));
}

pub fn sigaltstack(ss: ?*stack_t, old_ss: ?*stack_t) usize {
    return syscall2(.sigaltstack, @int_from_ptr(ss), @int_from_ptr(old_ss));
}

pub fn uname(uts: *utsname) usize {
    return syscall1(.uname, @int_from_ptr(uts));
}

pub fn io_uring_setup(entries: u32, p: *io_uring_params) usize {
    return syscall2(.io_uring_setup, entries, @int_from_ptr(p));
}

pub fn io_uring_enter(fd: i32, to_submit: u32, min_complete: u32, flags: u32, sig: ?*sigset_t) usize {
    return syscall6(.io_uring_enter, @as(usize, @bit_cast(@as(isize, fd))), to_submit, min_complete, flags, @int_from_ptr(sig), NSIG / 8);
}

pub fn io_uring_register(fd: i32, opcode: IORING_REGISTER, arg: ?*const anyopaque, nr_args: u32) usize {
    return syscall4(.io_uring_register, @as(usize, @bit_cast(@as(isize, fd))), @int_from_enum(opcode), @int_from_ptr(arg), nr_args);
}

pub fn memfd_create(name: [*:0]const u8, flags: u32) usize {
    return syscall2(.memfd_create, @int_from_ptr(name), flags);
}

pub fn getrusage(who: i32, usage: *rusage) usize {
    return syscall2(.getrusage, @as(usize, @bit_cast(@as(isize, who))), @int_from_ptr(usage));
}

pub fn tcgetattr(fd: fd_t, termios_p: *termios) usize {
    return syscall3(.ioctl, @as(usize, @bit_cast(@as(isize, fd))), T.CGETS, @int_from_ptr(termios_p));
}

pub fn tcsetattr(fd: fd_t, optional_action: TCSA, termios_p: *const termios) usize {
    return syscall3(.ioctl, @as(usize, @bit_cast(@as(isize, fd))), T.CSETS + @int_from_enum(optional_action), @int_from_ptr(termios_p));
}

pub fn tcgetpgrp(fd: fd_t, pgrp: *pid_t) usize {
    return syscall3(.ioctl, @as(usize, @bit_cast(@as(isize, fd))), T.IOCGPGRP, @int_from_ptr(pgrp));
}

pub fn tcsetpgrp(fd: fd_t, pgrp: *const pid_t) usize {
    return syscall3(.ioctl, @as(usize, @bit_cast(@as(isize, fd))), T.IOCSPGRP, @int_from_ptr(pgrp));
}

pub fn tcdrain(fd: fd_t) usize {
    return syscall3(.ioctl, @as(usize, @bit_cast(@as(isize, fd))), T.CSBRK, 1);
}

pub fn ioctl(fd: fd_t, request: u32, arg: usize) usize {
    return syscall3(.ioctl, @as(usize, @bit_cast(@as(isize, fd))), request, arg);
}

pub fn signalfd(fd: fd_t, mask: *const sigset_t, flags: u32) usize {
    return syscall4(.signalfd4, @as(usize, @bit_cast(@as(isize, fd))), @int_from_ptr(mask), NSIG / 8, flags);
}

pub fn copy_file_range(fd_in: fd_t, off_in: ?*i64, fd_out: fd_t, off_out: ?*i64, len: usize, flags: u32) usize {
    return syscall6(
        .copy_file_range,
        @as(usize, @bit_cast(@as(isize, fd_in))),
        @int_from_ptr(off_in),
        @as(usize, @bit_cast(@as(isize, fd_out))),
        @int_from_ptr(off_out),
        len,
        flags,
    );
}
...
pub fn bpf(cmd: BPF.Cmd, attr: *BPF.Attr, size: u32) usize {
    return syscall3(.bpf, @int_from_enum(cmd), @int_from_ptr(attr), size);
}
...
pub fn syncfs(fd: fd_t) usize {
    return syscall1(.syncfs, @as(usize, @bit_cast(@as(isize, fd))));
}

pub fn fsync(fd: fd_t) usize {
    return syscall1(.fsync, @as(usize, @bit_cast(@as(isize, fd))));
}

pub fn fdatasync(fd: fd_t) usize {
    return syscall1(.fdatasync, @as(usize, @bit_cast(@as(isize, fd))));
}

pub fn prctl(option: i32, arg2: usize, arg3: usize, arg4: usize, arg5: usize) usize {
    return syscall5(.prctl, @as(usize, @bit_cast(@as(isize, option))), arg2, arg3, arg4, arg5);
}
...
pub fn prlimit(pid: pid_t, resource: rlimit_resource, new_limit: ?*const rlimit, old_limit: ?*rlimit) usize {
    return syscall4(
        .prlimit64,
        @as(usize, @bit_cast(@as(isize, pid))),
        @as(usize, @bit_cast(@as(isize, @int_from_enum(resource)))),
        @int_from_ptr(new_limit),
        @int_from_ptr(old_limit),
    );
}

pub fn mincore(address: [*]u8, len: usize, vec: [*]u8) usize {
    return syscall3(.mincore, @int_from_ptr(address), len, @int_from_ptr(vec));
}

pub fn madvise(address: [*]u8, len: usize, advice: u32) usize {
    return syscall3(.madvise, @int_from_ptr(address), len, advice);
}

pub fn pidfd_open(pid: pid_t, flags: u32) usize {
    return syscall2(.pidfd_open, @as(usize, @bit_cast(@as(isize, pid))), flags);
}

pub fn pidfd_getfd(pidfd: fd_t, targetfd: fd_t, flags: u32) usize {
    return syscall3(
        .pidfd_getfd,
        @as(usize, @bit_cast(@as(isize, pidfd))),
        @as(usize, @bit_cast(@as(isize, targetfd))),
        flags,
    );
}
...
pub fn pidfd_send_signal(pidfd: fd_t, sig: i32, info: ?*siginfo_t, flags: u32) usize {
    return syscall4(
        .pidfd_send_signal,
        @as(usize, @bit_cast(@as(isize, pidfd))),
        @as(usize, @bit_cast(@as(isize, sig))),
        @int_from_ptr(info),
        flags,
    );
}
...
pub fn process_vm_readv(pid: pid_t, local: []iovec, remote: []const iovec_const, flags: usize) usize {
    return syscall6(
        .process_vm_readv,
        @as(usize, @bit_cast(@as(isize, pid))),
        @int_from_ptr(local.ptr),
        local.len,
        @int_from_ptr(remote.ptr),
        remote.len,
        flags,
    );
}
...
pub fn fadvise(fd: fd_t, offset: i64, len: i64, advice: usize) usize {
    if (comptime builtin.cpu.arch.is_mips()) {
        ...
        const offset_halves = split_value64(offset);
        const length_halves = split_value64(len);
    ...
    } else if (comptime builtin.cpu.arch.is_arm()) {
    ...
    } else if (@has_field(SYS, "fadvise64_64") and usize_bits != 64) {
        ...
        return syscall4(
            .fadvise64,
            @as(usize, @bit_cast(@as(isize, fd))),
            @as(usize, @bit_cast(offset)),
            @as(usize, @bit_cast(len)),
            advice,
        );
    }
}
...
pub fn perf_event_open(
    ...
    return syscall5(
        ...
        @int_from_ptr(attr),
        @as(usize, @bit_cast(@as(isize, pid))),
        @as(usize, @bit_cast(@as(isize, cpu))),
        @as(usize, @bit_cast(@as(isize, group_fd))),
        flags,
    );
}

pub fn seccomp(operation: u32, flags: u32, args: ?*const anyopaque) usize {
    return syscall3(.seccomp, operation, flags, @int_from_ptr(args));
}
...
pub const E = switch (native_arch) {
    .mips, .mipsel => enum(u16) {
        ...
        pub const init = errno_from_syscall;
    },
...
pub const EPOLL = struct {
    pub const CLOEXEC = 1 << @bit_offset_of(O, "CLOEXEC");
...
pub const EFD = struct {
    ...
    pub const CLOEXEC = 1 << @bit_offset_of(O, "CLOEXEC");
    pub const NONBLOCK = 1 << @bit_offset_of(O, "NONBLOCK");
};
...
pub const sockaddr = extern struct {
    ...
    pub const storage = extern struct {
        ...
        padding: [SS_MAXSIZE - @size_of(sa_family_t)]u8 = undefined,
    ...
    pub const vm = extern struct {
        ...
        zero: [3]u8 = [_]u8{0} ** 3,
        comptime {
            std.debug.assert(@size_of(vm) == @size_of(sockaddr));
        }
    };
};
...
pub const XATTR_CAPS_SZ_1 = @size_of(u32) * (1 + 2 * VFS_CAP_U32_1);

pub const VFS_CAP_REVISION_2 = 0x02000000;
pub const VFS_CAP_U32_2 = 2;
pub const XATTR_CAPS_SZ_2 = @size_of(u32) * (1 + 2 * VFS_CAP_U32_2);
...
pub const CAP = struct {
    ...
    pub fn TO_MASK(cap: u8) u32 {
        return @as(u32, 1) << @as(u5, @int_cast(cap & 31));
    }
...
pub const inotify_event = extern struct {
    ...
    pub fn get_name(self: *const inotify_event) ?[:0]const u8 {
        ...
        return std.mem.span(@as([*:0]const u8, @ptr_cast(self)) + @size_of(inotify_event));
    }
};
...
pub const cpu_set_t = [CPU_SETSIZE / @size_of(usize)]usize;
...
pub const SIGSTKSZ = switch (native_arch) {
    ...
    .aarch64 => 16384,
    else => @compile_error("SIGSTKSZ not defined for this architecture"),
};
...
const siginfo_fields_union = extern union {
    pad: [128 - 2 * @size_of(c_int) - @size_of(c_long)]u8,
...
pub const IORING_FILE_INDEX_ALLOC = max_int(u32);
...
/// use fixed fileset
pub const IOSQE_FIXED_FILE = 1 << @int_from_enum(IOSQE_BIT.FIXED_FILE);

/// issue after inflight IO
pub const IOSQE_IO_DRAIN = 1 << @int_from_enum(IOSQE_BIT.IO_DRAIN);

/// links next sqe
pub const IOSQE_IO_LINK = 1 << @int_from_enum(IOSQE_BIT.IO_LINK);

/// like LINK, but stronger
pub const IOSQE_IO_HARDLINK = 1 << @int_from_enum(IOSQE_BIT.IO_HARDLINK);

/// always go async
pub const IOSQE_ASYNC = 1 << @int_from_enum(IOSQE_BIT.ASYNC);

/// select buffer from buf_group
pub const IOSQE_BUFFER_SELECT = 1 << @int_from_enum(IOSQE_BIT.BUFFER_SELECT);

/// don't post CQE if request succeeded
/// Available since Linux 5.17
pub const IOSQE_CQE_SKIP_SUCCESS = 1 << @int_from_enum(IOSQE_BIT.CQE_SKIP_SUCCESS);
...
pub const io_uring_cqe = extern struct {
    ...
    pub fn buffer_id(self: io_uring_cqe) !u16 {
        ...
        return @as(u16, @int_cast(self.flags >> IORING_CQE_BUFFER_SHIFT));
    }
};
...
pub const rlimit_resource = if (native_arch.is_mips() or native_arch.is_sparc())
...
pub const kernel_timespec = if (@size_of(usize) >= 8) timespec else extern struct {
    tv_sec: i64,
    tv_nsec: i64,
};
...
pub const PR = enum(i32) {
    ...
    pub const SET_PTRACER_ANY = std.math.max_int(c_ulong);
...
pub const perf_event_attr = extern struct {
    ...
    size: u32 = @size_of(perf_event_attr),
...
pub const AUDIT = struct {
    pub const ARCH = enum(u32) {
        ...
        pub const current: AUDIT.ARCH = switch (native_arch) {
            ...
            .powerpc64 => .PPC64,
            .powerpc64le => .PPC64LE,
            else => @compile_error("unsupported architecture"),
        };

        AARCH64 = to_audit(.aarch64),
        ARM = to_audit(.arm),
        ARMEB = to_audit(.armeb),
        CSKY = to_audit(.csky),
        HEXAGON = @int_from_enum(std.elf.EM.HEXAGON),
        X86 = to_audit(.x86),
        M68K = to_audit(.m68k),
        MIPS = to_audit(.mips),
        MIPSEL = to_audit(.mips) | LE,
        MIPS64 = to_audit(.mips64),
        MIPSEL64 = to_audit(.mips64) | LE,
        PPC = to_audit(.powerpc),
        PPC64 = to_audit(.powerpc64),
        PPC64LE = to_audit(.powerpc64le),
        RISCV32 = to_audit(.riscv32),
        RISCV64 = to_audit(.riscv64),
        S390X = to_audit(.s390x),
        SPARC = to_audit(.sparc),
        SPARC64 = to_audit(.sparc64),
        X86_64 = to_audit(.x86_64),

        fn to_audit(arch: std.Target.Cpu.Arch) u32 {
            var res: u32 = @int_from_enum(arch.to_elf_machine());
