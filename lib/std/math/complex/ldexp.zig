...
fn frexp_exp32(x: f32, expt: *i32) f32 {
    ...
    const hx = @as(u32, @bit_cast(exp_x));
    // TODO zig should allow this cast implicitly because it should know the value is in range
    expt.* = @as(i32, @int_cast(hx >> 23)) - (0x7f + 127) + k;
    return @as(f32, @bit_cast((hx & 0x7fffff) | ((0x7f + 127) << 23)));
}
...
fn ldexp_cexp32(z: Complex(f32), expt: i32) Complex(f32) {
    ...
    const half_expt1 = @div_trunc(exptf, 2);
    const scale1 = @as(f32, @bit_cast((0x7f + half_expt1) << 23));

    const half_expt2 = exptf - half_expt1;
    const scale2 = @as(f32, @bit_cast((0x7f + half_expt2) << 23));
...
fn frexp_exp64(x: f64, expt: *i32) f64 {
    ...
    const fx = @as(u64, @bit_cast(exp_x));
    const hx = @as(u32, @int_cast(fx >> 32));
    const lx = @as(u32, @truncate(fx));

    expt.* = @as(i32, @int_cast(hx >> 20)) - (0x3ff + 1023) + k;

    const high_word = (hx & 0xfffff) | ((0x3ff + 1023) << 20);
    return @as(f64, @bit_cast((@as(u64, high_word) << 32) | lx));
}
...
fn ldexp_cexp64(z: Complex(f64), expt: i32) Complex(f64) {
    ...
    const half_expt1 = @div_trunc(exptf, 2);
    const scale1 = @as(f64, @bit_cast((0x3ff + half_expt1) << (20 + 32)));

    const half_expt2 = exptf - half_expt1;
    const scale2 = @as(f64, @bit_cast((0x3ff + half_expt2) << (20 + 32)));
