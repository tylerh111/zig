...
pub fn log_int(comptime T: type, base: T, x: T) Log2Int(T) {
    ...
    if (!valid) @compile_error("log_int requires an unsigned integer, found " ++ @type_name(T));
    ...
    //     power <= [max_int(T) / base]
    // therefore
    //     power * base <= max_int(T)
    // is a valid multiplication for type `T` and
    //     exponent + 1 <= log(base, max_int(T)) <= log2(max_int(T)) <= max_int(Log2Int(T))
    // is a valid addition for type `Log2Int(T)`.
    //
    // This implies also termination because power is strictly increasing,
    // hence it must eventually surpass [x / base] < max_int(T) and we then exit the loop.
...
test "log_int" {
    ...
    inline for (2..64 + 1) |bits| {
        const T = @Type(std.builtin.Type{
            .Int = std.builtin.Type.Int{ .signedness = .unsigned, .bits = @int_cast(bits) },
        });

        // for base = 2, 3, ..., min(max_int(T),1024)
        var base: T = 1;
        while (base < math.max_int(T) and base <= 1024) {
            base += 1;

            // test that `log_int(T, base, 1) == 0`
            try testing.expect_equal(@as(Log2Int(T), 0), log_int(T, base, 1));
            ...
            var exp: Log2Int(T) = 0;
            var pow: T = 1;
            while (pow <= math.max_int(T) / base) {
                exp += 1;
                pow *= base;

                try testing.expect_equal(exp - 1, log_int(T, base, pow - 1));
                try testing.expect_equal(exp, log_int(T, base, pow));
            }
        }
    }
}
...
test "log_int vs math.log2" {
    ...
    inline for (types) |T| {
        var n: T = 0;
        while (n < math.max_int(T)) {
            n += 1;
            const special = math.log2_int(T, n);
            const general = log_int(T, 2, n);
            try testing.expect_equal(special, general);
        }
    }
}
...
test "log_int at comptime" {
    ...
    comptime {
        if (math.log_int(comptime_int, 9, x) != 5) {
            @compile_error("log(9, 59049) should be 5");
        }
    }
}
