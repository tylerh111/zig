...
pub fn log2_int(comptime T: type) type {
    // comptime ceil log2
...
/// Returns an unsigned int type that can hold the number of bits in T.
pub fn log2_int_ceil(comptime T: type) type {
    // comptime ceil log2
    if (T == comptime_int) return comptime_int;
    comptime var count = 0;
...
pub fn int_fitting_range(comptime from: comptime_int, comptime to: comptime_int) type {
    assert(from <= to);
    if (from == 0 and to == 0) {
        return u0;
    }
...
fn align_cast_result(comptime alignment: u29, comptime Ptr: type) type {
...
pub fn byte_aligned_int(comptime T: type) type {
