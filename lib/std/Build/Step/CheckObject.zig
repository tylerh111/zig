...
pub fn create(
    ...
    check_object.source.add_step_dependencies(&check_object.step);
    return check_object;
}
...
const SearchPhrase = struct {
    ...
    fn resolve(phrase: SearchPhrase, b: *std.Build, step: *Step) []const u8 {
        ...
        return b.fmt("{s} {s}", .{ phrase.string, lazy_path.get_path2(b, step) });
    }
};
...
const Action = struct {
    ...
    fn extract(
        ...
        var hay_it = mem.tokenize_scalar(u8, hay, ' ');
        var needle_it = mem.tokenize_scalar(u8, phrase, ' ');

        while (needle_it.next()) |needle_tok| {
            const hay_tok = hay_it.next() orelse break;
            if (mem.starts_with(u8, needle_tok, "{")) {
                const closing_brace = mem.index_of(u8, needle_tok, "}") orelse return error.MissingClosingBrace;
                ...
                const value = std.fmt.parse_int(u64, hay_tok, 16) catch return false;
        ...
        for (candidate_vars.items) |cv| try global_vars.put_no_clobber(cv.name, cv.value);
    ...
    fn contains(
        ...
        const hay = mem.trim(u8, haystack, " ");
        const phrase = mem.trim(u8, act.phrase.resolve(b, step), " ");
        return mem.index_of(u8, hay, phrase) != null;
    }
    ...
    fn compute_cmp(act: Action, b: *std.Build, step: *Step, global_vars: anytype) !bool {
        ...
        var it = mem.tokenize_scalar(u8, phrase, ' ');
        while (it.next()) |next| {
            if (mem.eql(u8, next, "+")) {
                ...
                const val = std.fmt.parse_int(u64, next, 0) catch blk: {
...
const ComputeCompareExpected = struct {
    ...
    pub fn format(
        ...
        if (fmt.len != 0) std.fmt.invalid_fmt_error(fmt, value);
        _ = options;
        try writer.print("{s} ", .{@tag_name(value.op)});
        switch (value.value) {
            .variable => |name| try writer.write_all(name),
...
const Check = struct {
    ...
    fn dump_section(allocator: Allocator, name: [:0]const u8) Check {
        ...
        const off: u32 = @int_cast(check.data.items.len);
...
pub fn check_exact(check_object: *CheckObject, phrase: []const u8) void {
    check_object.check_exact_inner(phrase, null);
}

/// Like `check_exact()` but takes an additional argument `LazyPath` which will be
...
pub fn check_exact_path(check_object: *CheckObject, phrase: []const u8, lazy_path: std.Build.LazyPath) void {
    check_object.check_exact_inner(phrase, lazy_path);
}
...
pub fn check_contains(check_object: *CheckObject, phrase: []const u8) void {
    check_object.check_contains_inner(phrase, null);
}

/// Like `check_contains()` but takes an additional argument `lazy_path` which will be
/// resolved to a full search query in `make()`.
pub fn check_contains_path(
    ...
    check_object.check_contains_inner(phrase, lazy_path);
}
...
pub fn check_extract(check_object: *CheckObject, phrase: []const u8) void {
    check_object.check_extract_inner(phrase, null);
}

/// Like `check_extract()` but takes an additional argument `LazyPath` which will be
/// resolved to a full search query in `make()`.
pub fn check_extract_lazy_path(check_object: *CheckObject, phrase: []const u8, lazy_path: std.Build.LazyPath) void {
    check_object.check_extract_inner(phrase, lazy_path);
}
...
pub fn check_in_headers(check_object: *CheckObject) void {
    check_object.check_start(.headers);
}
...
pub fn check_in_symtab(check_object: *CheckObject) void {
    ...
    check_object.check_start(.symtab);
    check_object.check_exact(label);
}
...
pub fn check_in_dyld_rebase(check_object: *CheckObject) void {
    ...
    check_object.check_start(.dyld_rebase);
    check_object.check_exact(label);
}
...
pub fn check_in_dyld_bind(check_object: *CheckObject) void {
    ...
    check_object.check_start(.dyld_bind);
    check_object.check_exact(label);
}
...
pub fn check_in_dyld_weak_bind(check_object: *CheckObject) void {
    ...
    check_object.check_start(.dyld_weak_bind);
    check_object.check_exact(label);
}
...
pub fn check_in_dyld_lazy_bind(check_object: *CheckObject) void {
    ...
    check_object.check_start(.dyld_lazy_bind);
    check_object.check_exact(label);
}
...
pub fn check_in_exports(check_object: *CheckObject) void {
    ...
    check_object.check_start(.exports);
    check_object.check_exact(label);
}
...
pub fn check_in_indirect_symtab(check_object: *CheckObject) void {
    ...
    check_object.check_start(.indirect_symtab);
    check_object.check_exact(label);
}
...
pub fn check_in_dynamic_symtab(check_object: *CheckObject) void {
    ...
    check_object.check_start(.dynamic_symtab);
    check_object.check_exact(label);
}
...
pub fn check_in_dynamic_section(check_object: *CheckObject) void {
    ...
    check_object.check_start(.dynamic_section);
    check_object.check_exact(label);
}
...
pub fn check_in_archive_symtab(check_object: *CheckObject) void {
    ...
    check_object.check_start(.archive_symtab);
    check_object.check_exact(label);
}

pub fn dump_section(check_object: *CheckObject, name: [:0]const u8) void {
    const check = Check.dump_section(check_object.step.owner.allocator, name);
    check_object.checks.append(check) catch @panic("OOM");
}
...
pub fn check_compute_compare(
    ...
    check.compute_cmp(.{ .string = check_object.step.owner.dupe(program) }, expected);
    check_object.checks.append(check) catch @panic("OOM");
}
...
fn make(step: *Step, prog_node: std.Progress.Node) !void {
    ...
    const src_path = check_object.source.get_path2(b, step);
    const contents = fs.cwd().read_file_alloc_options(
        gpa,
        src_path,
    ...
    for (check_object.checks.items) |chk| {
        if (chk.kind == .compute_compare) {
            ...
            const res = act.compute_cmp(b, step, vars) catch |err| switch (err) {
        ...
        const output = switch (check_object.obj_format) {
            .macho => try MachODumper.parse_and_dump(step, chk, contents),
            .elf => try ElfDumper.parse_and_dump(step, chk, contents),
            .coff => return step.fail("TODO coff parser", .{}),
            .wasm => try WasmDumper.parse_and_dump(step, chk, contents),
            else => unreachable,
        };
        ...
        // the failed test.
        const fmt_message_string = struct {
            fn fmt_message_string(kind: Check.Kind, msg: []const u8) std.fmt.Formatter(format_message_string) {
                return .{ .data = .{
                    .kind = kind,
                    .msg = msg,
                } };
            ...
            fn format_message_string(
                ...
                _ = options;
                switch (ctx.kind) {
                    .dump_section => try writer.print("{s}", .{std.fmt.fmt_slice_escape_lower(ctx.msg)}),
                    else => try writer.write_all(ctx.msg),
                }
            }
        }.fmt_message_string;

        var it = mem.tokenize_any(u8, output, "\r\n");
        for (chk.actions.items) |act| {
            switch (act.tag) {
                .exact => {
                    while (it.next()) |line| {
                        ...
                        return step.fail(
                            ...
                            \\{s}
                            \\======================================================
                        , .{ fmt_message_string(chk.kind, act.phrase.resolve(b, step)), fmt_message_string(chk.kind, output) });
                    }
                },
                ...
                .not_present => {
                    while (it.next()) |line| {
                        if (act.not_present(b, step, line)) continue;
                        return step.fail(
                            \\
                ...
                .extract => {
                    while (it.next()) |line| {
                        ...
                        return step.fail(
                            ...
                            \\{s}
                            \\======================================================
                        , .{ act.phrase.resolve(b, step), fmt_message_string(chk.kind, output) });
                    }
                },
...
const MachODumper = struct {
    ...
    fn parse_and_dump(step: *Step, check: Check, bytes: []const u8) ![]const u8 {
        ...
        return parse_and_dump_object(step, check, bytes);
    }
    ...
    const ObjectContext = struct {
        ...
        fn parse(ctx: *ObjectContext) !void {
            var it = ctx.get_load_command_iterator();
            var i: usize = 0;
            while (it.next()) |cmd| {
                switch (cmd.cmd()) {
                    .SEGMENT_64 => {
                        ...
                        try ctx.sections.ensure_unused_capacity(ctx.gpa, seg.nsects);
                        for (cmd.get_sections()) |sect| {
                            ctx.sections.append_assume_capacity(sect);
                        }
                    },
                    .SYMTAB => {
                        const lc = cmd.cast(macho.symtab_command).?;
                        const symtab = @as([*]align(1) const macho.nlist_64, @ptr_cast(ctx.data.ptr + lc.symoff))[0..lc.nsyms];
                        const strtab = ctx.data[lc.stroff..][0..lc.strsize];
                        try ctx.symtab.append_unaligned_slice(ctx.gpa, symtab);
                        try ctx.strtab.append_slice(ctx.gpa, strtab);
                    },
                    .DYSYMTAB => {
                        const lc = cmd.cast(macho.dysymtab_command).?;
                        const indexes = @as([*]align(1) const u32, @ptr_cast(ctx.data.ptr + lc.indirectsymoff))[0..lc.nindirectsyms];
                        try ctx.indsymtab.append_unaligned_slice(ctx.gpa, indexes);
                    },
                    .LOAD_DYLIB,
                    .LOAD_WEAK_DYLIB,
                    .REEXPORT_DYLIB,
                    => {
                        try ctx.imports.append(ctx.gpa, cmd.get_dylib_path_name());
                    },
                    else => {},
                }
        ...
        fn get_string(ctx: ObjectContext, off: u32) [:0]const u8 {
            ...
            return mem.slice_to(@as([*:0]const u8, @ptr_cast(ctx.strtab.items.ptr + off)), 0);
        }

        fn get_load_command_iterator(ctx: ObjectContext) macho.LoadCommandIterator {
            const data = ctx.data[@size_of(macho.mach_header_64)..][0..ctx.header.sizeofcmds];
        ...
        fn get_segment_by_name(ctx: ObjectContext, name: []const u8) ?macho.segment_command_64 {
            for (ctx.segments.items) |seg| {
                if (mem.eql(u8, seg.seg_name(), name)) return seg;
            }
            return null;
        }
        ...
        fn get_section_by_name(ctx: ObjectContext, segname: []const u8, sectname: []const u8) ?macho.section_64 {
            for (ctx.sections.items) |sect| {
                if (mem.eql(u8, sect.seg_name(), segname) and mem.eql(u8, sect.sect_name(), sectname)) return sect;
            }
            return null;
        }
        ...
        fn dump_header(hdr: macho.mach_header_64, writer: anytype) !void {
            ...
            if (hdr.flags > 0) {
                if (hdr.flags & macho.MH_NOUNDEFS != 0) try writer.write_all(" NOUNDEFS");
                if (hdr.flags & macho.MH_INCRLINK != 0) try writer.write_all(" INCRLINK");
                if (hdr.flags & macho.MH_DYLDLINK != 0) try writer.write_all(" DYLDLINK");
                if (hdr.flags & macho.MH_BINDATLOAD != 0) try writer.write_all(" BINDATLOAD");
                if (hdr.flags & macho.MH_PREBOUND != 0) try writer.write_all(" PREBOUND");
                if (hdr.flags & macho.MH_SPLIT_SEGS != 0) try writer.write_all(" SPLIT_SEGS");
                if (hdr.flags & macho.MH_LAZY_INIT != 0) try writer.write_all(" LAZY_INIT");
                if (hdr.flags & macho.MH_TWOLEVEL != 0) try writer.write_all(" TWOLEVEL");
                if (hdr.flags & macho.MH_FORCE_FLAT != 0) try writer.write_all(" FORCE_FLAT");
                if (hdr.flags & macho.MH_NOMULTIDEFS != 0) try writer.write_all(" NOMULTIDEFS");
                if (hdr.flags & macho.MH_NOFIXPREBINDING != 0) try writer.write_all(" NOFIXPREBINDING");
                if (hdr.flags & macho.MH_PREBINDABLE != 0) try writer.write_all(" PREBINDABLE");
                if (hdr.flags & macho.MH_ALLMODSBOUND != 0) try writer.write_all(" ALLMODSBOUND");
                if (hdr.flags & macho.MH_SUBSECTIONS_VIA_SYMBOLS != 0) try writer.write_all(" SUBSECTIONS_VIA_SYMBOLS");
                if (hdr.flags & macho.MH_CANONICAL != 0) try writer.write_all(" CANONICAL");
                if (hdr.flags & macho.MH_WEAK_DEFINES != 0) try writer.write_all(" WEAK_DEFINES");
                if (hdr.flags & macho.MH_BINDS_TO_WEAK != 0) try writer.write_all(" BINDS_TO_WEAK");
                if (hdr.flags & macho.MH_ALLOW_STACK_EXECUTION != 0) try writer.write_all(" ALLOW_STACK_EXECUTION");
                if (hdr.flags & macho.MH_ROOT_SAFE != 0) try writer.write_all(" ROOT_SAFE");
                if (hdr.flags & macho.MH_SETUID_SAFE != 0) try writer.write_all(" SETUID_SAFE");
                if (hdr.flags & macho.MH_NO_REEXPORTED_DYLIBS != 0) try writer.write_all(" NO_REEXPORTED_DYLIBS");
                if (hdr.flags & macho.MH_PIE != 0) try writer.write_all(" PIE");
                if (hdr.flags & macho.MH_DEAD_STRIPPABLE_DYLIB != 0) try writer.write_all(" DEAD_STRIPPABLE_DYLIB");
                if (hdr.flags & macho.MH_HAS_TLV_DESCRIPTORS != 0) try writer.write_all(" HAS_TLV_DESCRIPTORS");
                if (hdr.flags & macho.MH_NO_HEAP_EXECUTION != 0) try writer.write_all(" NO_HEAP_EXECUTION");
                if (hdr.flags & macho.MH_APP_EXTENSION_SAFE != 0) try writer.write_all(" APP_EXTENSION_SAFE");
                if (hdr.flags & macho.MH_NLIST_OUTOFSYNC_WITH_DYLDINFO != 0) try writer.write_all(" NLIST_OUTOFSYNC_WITH_DYLDINFO");
            }
        ...
        fn dump_load_command(lc: macho.LoadCommandIterator.LoadCommand, index: usize, writer: anytype) !void {
            ...
            try writer.print(
                ...
                \\cmd {s}
                \\cmdsize {d}
            , .{ index, @tag_name(lc.cmd()), lc.cmdsize() });
            ...
            switch (lc.cmd()) {
                ...
                .UUID => {
                    ...
                    try writer.write_byte('\n');
                    try writer.print("uuid {x}", .{std.fmt.fmt_slice_hex_lower(&uuid.uuid)});
                },
                ...
                .BUILD_VERSION => {
                    ...
                    for (lc.get_build_version_tools()) |tool| {
                        try writer.write_byte('\n');
                        switch (tool.tool) {
        ...
        fn dump_symtab(ctx: ObjectContext, writer: anytype) !void {
            try writer.write_all(symtab_label ++ "\n");

            for (ctx.symtab.items) |sym| {
                const sym_name = ctx.get_string(sym.n_strx);
                if (sym.stab()) {
                    ...
                    if (sym.n_sect > 0) {
                        ...
                        try writer.print(" ({s},{s})", .{ sect.seg_name(), sect.sect_name() });
                    }
                    ...
                    if (sym.n_desc & macho.REFERENCED_DYNAMICALLY != 0) try writer.write_all(" [referenced dynamically]");
                    if (sym.weak_def()) try writer.write_all(" weak");
                    if (sym.weak_ref()) try writer.write_all(" weakref");
                    if (sym.ext()) {
                        if (sym.pext()) try writer.write_all(" private");
                        try writer.write_all(" external");
                    } else if (sym.pext()) try writer.write_all(" (was private external)");
                    try writer.print(" {s}\n", .{sym_name});
                } else if (sym.tentative()) {
                    const alignment = (sym.n_desc >> 8) & 0x0F;
                    try writer.print("  0x{x:0>16} (common) (alignment 2^{d})", .{ sym.n_value, alignment });
                    if (sym.ext()) try writer.write_all(" external");
                    try writer.print(" {s}\n", .{sym_name});
                } else if (sym.undf()) {
                    const ordinal = @div_floor(@as(i16, @bit_cast(sym.n_desc)), macho.N_SYMBOL_RESOLVER);
                    const import_name = blk: {
                        ...
                        const full_path = ctx.imports.items[@as(u16, @bit_cast(ordinal)) - 1];
                        const basename = fs.path.basename(full_path);
                        assert(basename.len > 0);
                        const ext = mem.last_index_of_scalar(u8, basename, '.') orelse basename.len;
                        break :blk basename[0..ext];
                    };
                    try writer.write_all("(undefined)");
                    if (sym.weak_ref()) try writer.write_all(" weakref");
                    if (sym.ext()) try writer.write_all(" external");
        ...
        fn dump_indirect_symtab(ctx: ObjectContext, writer: anytype) !void {
            try writer.write_all(indirect_symtab_label ++ "\n");

            var sects_buffer: [3]macho.section_64 = undefined;
            const sects = blk: {
                var count: usize = 0;
                if (ctx.get_section_by_name("__TEXT", "__stubs")) |sect| {
                    sects_buffer[count] = sect;
                    count += 1;
                }
                if (ctx.get_section_by_name("__DATA_CONST", "__got")) |sect| {
                    sects_buffer[count] = sect;
                    count += 1;
                }
                if (ctx.get_section_by_name("__DATA", "__la_symbol_ptr")) |sect| {
                    sects_buffer[count] = sect;
                    count += 1;
                }
            ...
            mem.sort(macho.section_64, sects, {}, sort_fn);
            ...
            while (i < sects.len) : (i += 1) {
                ...
                const entry_size = blk: {
                    if (mem.eql(u8, sect.sect_name(), "__stubs")) break :blk sect.reserved2;
                    break :blk @size_of(u64);
                };
                ...
                for (ctx.indsymtab.items[start..end], 0..) |index, j| {
                    ...
                    try writer.print("0x{x} {d} {s}\n", .{ addr, index, ctx.get_string(sym.n_strx) });
                }
            }
        }
        ...
        fn dump_rebase_info(ctx: ObjectContext, data: []const u8, writer: anytype) !void {
            ...
            try ctx.parse_rebase_info(data, &rebases);
        ...
        fn parse_rebase_info(ctx: ObjectContext, data: []const u8, rebases: *std.ArrayList(u64)) !void {
            var stream = std.io.fixed_buffer_stream(data);
            ...
            var offset: u64 = 0;
            while (true) {
                const byte = reader.read_byte() catch break;
                ...
                switch (opc) {
                    ...
                    macho.REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB => {
                        seg_id = imm;
                        offset = try std.leb.read_uleb128(u64, reader);
                    },
                    macho.REBASE_OPCODE_ADD_ADDR_IMM_SCALED => {
                        offset += imm * @size_of(u64);
                    },
                    macho.REBASE_OPCODE_ADD_ADDR_ULEB => {
                        const addend = try std.leb.read_uleb128(u64, reader);
                        offset += addend;
                    },
                    ...
                    => {
                        ...
                        switch (opc) {
                            ...
                            macho.REBASE_OPCODE_DO_REBASE_ULEB_TIMES => {
                                ntimes = try std.leb.read_uleb128(u64, reader);
                            },
                            macho.REBASE_OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB => {
                                ntimes = try std.leb.read_uleb128(u64, reader);
                                skip = try std.leb.read_uleb128(u64, reader);
                            },
                            else => unreachable,
                        }
                        const seg = ctx.segments.items[seg_id.?];
        ...
        fn dump_bind_info(ctx: ObjectContext, data: []const u8, writer: anytype) !void {
            ...
            try ctx.parse_bind_info(data, &bindings);
            mem.sort(Binding, bindings.items, {}, Binding.less_than);
            for (bindings.items) |binding| {
                try writer.print("0x{x} [addend: {d}]", .{ binding.address, binding.addend });
                try writer.write_all(" (");
                switch (binding.tag) {
                    .self => try writer.write_all("self"),
                    .exe => try writer.write_all("main executable"),
                    .flat => try writer.write_all("flat lookup"),
                    .ord => try writer.write_all(std.fs.path.basename(ctx.imports.items[binding.ordinal - 1])),
                }
                try writer.print(") {s}\n", .{binding.name});
            }
        }
            ...
            while (true) {
                ...
                switch (opc) {
                    ...
                    macho.BIND_OPCODE_SET_SYMBOL_TRAILING_FLAGS_IMM => {
                        name_buf.clear_retaining_capacity();
                        try reader.read_until_delimiter_array_list(&name_buf, 0, std.math.max_int(u32));
                        try name_buf.append(0);
                    },
                    macho.BIND_OPCODE_SET_ADDEND_SLEB => {
                        addend = try std.leb.read_ileb128(i64, reader);
                    },
                    macho.BIND_OPCODE_ADD_ADDR_ULEB => {
                        const x = try std.leb.read_uleb128(u64, reader);
                        offset = @int_cast(@as(i64, @int_cast(offset)) + @as(i64, @bit_cast(x)));
                    },
                    ...
                    => {
                        ...
                        switch (opc) {
                            macho.BIND_OPCODE_DO_BIND => {},
                            macho.BIND_OPCODE_DO_BIND_ADD_ADDR_ULEB => {
                                add_addr = try std.leb.read_uleb128(u64, reader);
                            },
                            macho.BIND_OPCODE_DO_BIND_ADD_ADDR_IMM_SCALED => {
                                add_addr = imm * @size_of(u64);
                            },
                            macho.BIND_OPCODE_DO_BIND_ULEB_TIMES_SKIPPING_ULEB => {
                                count = try std.leb.read_uleb128(u64, reader);
                                skip = try std.leb.read_uleb128(u64, reader);
                            },
                            else => unreachable,
                        }
                        ...
                        var i: u64 = 0;
                        while (i < count) : (i += 1) {
                            const addr: u64 = @int_cast(@as(i64, @int_cast(seg.vmaddr + offset)));
                            try bindings.append(.{
                                .address = addr,
                                .addend = addend,
                                .tag = tag,
                            ...
                            offset += skip + @size_of(u64) + add_addr;
                        }
                    },
                    else => break,
                }
        ...
        fn dump_exports_trie(ctx: ObjectContext, data: []const u8, writer: anytype) !void {
            const seg = ctx.get_segment_by_name("__TEXT") orelse return;
            ...
            try parse_trie_node(arena.allocator(), &it, "", &exports);

            mem.sort(Export, exports.items, {}, Export.less_than);
            ...
            for (exports.items) |exp| {
                switch (exp.tag) {
                    .@"export" => {
                        ...
                        switch (info.kind) {
                            .regular => {},
                            .absolute => try writer.write_all("ABS, "),
                            .tlv => try writer.write_all("THREAD_LOCAL, "),
                        }
                        if (info.weak) try writer.write_all("WEAK");
                        if (info.kind != .regular or info.weak) {
                            try writer.write_all("] ");
                        }
        ...
        const Export = struct {
            ...
            fn less_than(ctx: void, lhs: Export, rhs: Export) bool {
                _ = ctx;
                if (lhs.rank_by_tag() == rhs.rank_by_tag()) {
                ...
                return lhs.rank_by_tag() < rhs.rank_by_tag();
            }
        };
        ...
        fn parse_trie_node(
            ...
            if (size > 0) {
                ...
                switch (flags) {
                    macho.EXPORT_SYMBOL_FLAGS_REEXPORT => {
                        const ord = try it.read_uleb128();
                        const name = try arena.dupe(u8, try it.read_string());
                        try exports.append(.{
                    ...
                    macho.EXPORT_SYMBOL_FLAGS_STUB_AND_RESOLVER => {
                        const stub_offset = try it.read_uleb128();
                        const resolver_offset = try it.read_uleb128();
                        try exports.append(.{
                            .name = prefix,
                            .tag = .stub_resolver,
                            .data = .{ .stub_resolver = .{
                        ...
                        const vmoff = try it.read_uleb128();
                        try exports.append(.{
                            .name = prefix,
                            .tag = .@"export",
                            .data = .{ .@"export" = .{
            ...
            for (0..nedges) |_| {
                ...
                const curr = it.pos;
                it.pos = off;
                try parse_trie_node(arena, it, prefix_label, exports);
                it.pos = curr;
            }
        }
    ...
    fn parse_and_dump_object(step: *Step, check: Check, bytes: []const u8) ![]const u8 {
        const gpa = step.owner.allocator;
        const hdr = @as(*align(1) const macho.mach_header_64, @ptr_cast(bytes.ptr)).*;
        ...
        switch (check.kind) {
            .headers => {
                try ObjectContext.dump_header(ctx.header, writer);

                var it = ctx.get_load_command_iterator();
                var i: usize = 0;
                while (it.next()) |cmd| {
                    try ObjectContext.dump_load_command(cmd, i, writer);
                    try writer.write_byte('\n');
            ...
            .dyld_lazy_bind,
            => {
                const cmd = ctx.get_load_command(.DYLD_INFO_ONLY) orelse
                ...
                switch (check.kind) {
                    .dyld_rebase => if (lc.rebase_size > 0) {
                        ...
                        try writer.write_all(dyld_rebase_label ++ "\n");
                        try ctx.dump_rebase_info(data, writer);
                    } else return step.fail("no rebase data found", .{}),

                    .dyld_bind => if (lc.bind_size > 0) {
                        const data = ctx.data[lc.bind_off..][0..lc.bind_size];
                        try writer.write_all(dyld_bind_label ++ "\n");
                        try ctx.dump_bind_info(data, writer);
                    ...
                    .dyld_weak_bind => if (lc.weak_bind_size > 0) {
                        ...
                        try writer.write_all(dyld_weak_bind_label ++ "\n");
                        try ctx.dump_bind_info(data, writer);
                    ...
                    .dyld_lazy_bind => if (lc.lazy_bind_size > 0) {
                        ...
                        try writer.write_all(dyld_lazy_bind_label ++ "\n");
                        try ctx.dump_bind_info(data, writer);
            ...
            .exports => blk: {
                if (ctx.get_load_command(.DYLD_INFO_ONLY)) |cmd| {
                    const lc = cmd.cast(macho.dyld_info_command).?;
                    if (lc.export_size > 0) {
                        const data = ctx.data[lc.export_off..][0..lc.export_size];
                        try writer.write_all(exports_label ++ "\n");
                        try ctx.dump_exports_trie(data, writer);
                        break :blk;
                    }
                }
            ...
            .dump_section => {
                const name = mem.slice_to(@as([*:0]const u8, @ptr_cast(check.data.items.ptr + check.payload.dump_section)), 0);
                const sep_index = mem.index_of_scalar(u8, name, ',') orelse
                    return step.fail("invalid section name: {s}", .{name});
                const segname = name[0..sep_index];
                const sectname = name[sep_index + 1 ..];
                const sect = ctx.get_section_by_name(segname, sectname) orelse
                    return step.fail("section '{s}' not found", .{name});
                try ctx.dump_section(sect, writer);
            },

            else => return step.fail("invalid check kind for MachO file format: {s}", .{@tag_name(check.kind)}),
        }

        return output.to_owned_slice();
    }
};
...
const ElfDumper = struct {
    ...
    fn parse_and_dump(step: *Step, check: Check, bytes: []const u8) ![]const u8 {
        return parse_and_dump_archive(step, check, bytes) catch |err| switch (err) {
            error.InvalidArchiveMagicNumber => try parse_and_dump_object(step, check, bytes),
            else => |e| return e,
        };
    }
    ...
    fn parse_and_dump_archive(step: *Step, check: Check, bytes: []const u8) ![]const u8 {
        ...
        const magic = try reader.read_bytes_no_eof(elf.ARMAG.len);
        ...
        while (true) {
            ...
            if (!mem.is_aligned(stream.pos, 2)) stream.pos += 1;

            const hdr = try reader.read_struct(elf.ar_hdr);

            if (!mem.eql(u8, &hdr.ar_fmag, elf.ARFMAG)) return error.InvalidArchiveHeaderMagicNumber;

            const size = try hdr.size();
            defer {
                _ = stream.seek_by(size) catch {};
            }

            if (hdr.is_symtab()) {
                try ctx.parse_symtab(ctx.data[stream.pos..][0..size], .p32);
                continue;
            }
            if (hdr.is_symtab64()) {
                try ctx.parse_symtab(ctx.data[stream.pos..][0..size], .p64);
                continue;
            }
            if (hdr.is_strtab()) {
            ...
            const name = if (hdr.name()) |name|
                ...
                try gpa.dupe(u8, ctx.get_string(off))
            else
                unreachable;
        ...
        switch (check.kind) {
            .archive_symtab => if (ctx.symtab.items.len > 0) {
                ...
                try ctx.dump_objects(step, check, writer);
    ...
    const ArchiveContext = struct {
        ...
        fn parse_symtab(ctx: *ArchiveContext, raw: []const u8, ptr_width: enum { p32, p64 }) !void {
            var stream = std.io.fixed_buffer_stream(raw);
            const reader = stream.reader();
            const num = switch (ptr_width) {
                .p32 => try reader.read_int(u32, .big),
                .p64 => try reader.read_int(u64, .big),
            };
            ...
            try ctx.symtab.ensure_total_capacity_precise(ctx.gpa, num);
            ...
            for (0..num) |_| {
                ...
                const name = mem.slice_to(@as([*:0]const u8, @ptr_cast(strtab.ptr + stroff)), 0);
                stroff += name.len + 1;
                ctx.symtab.append_assume_capacity(.{ .off = off, .name = name });
            }
        }
        ...
        fn dump_symtab(ctx: ArchiveContext, writer: anytype) !void {
            ...
            defer files.deinit();
            try files.ensure_unused_capacity(@int_cast(ctx.objects.items.len));

            for (ctx.objects.items) |object| {
                files.put_assume_capacity_no_clobber(object.off - @size_of(elf.ar_hdr), object.name);
            }
            ...
            for (ctx.symtab.items) |entry| {
                const gop = try symbols.get_or_put(@int_cast(entry.off));
        ...
        fn dump_objects(ctx: ArchiveContext, step: *Step, check: Check, writer: anytype) !void {
            for (ctx.objects.items) |object| {
                ...
                const output = try parse_and_dump_object(step, check, ctx.data[object.off..][0..object.len]);
        ...
        fn get_string(ctx: ArchiveContext, off: u32) []const u8 {
            ...
            const name = mem.slice_to(@as([*:'\n']const u8, @ptr_cast(ctx.strtab.ptr + off)), 0);
    ...
    fn parse_and_dump_object(step: *Step, check: Check, bytes: []const u8) ![]const u8 {
        ...
        const hdr = try reader.read_struct(elf.Elf64_Ehdr);
        if (!mem.eql(u8, hdr.e_ident[0..4], "\x7fELF")) {
            return error.InvalidMagicNumber;
        }

        const shdrs = @as([*]align(1) const elf.Elf64_Shdr, @ptr_cast(bytes.ptr + hdr.e_shoff))[0..hdr.e_shnum];
        const phdrs = @as([*]align(1) const elf.Elf64_Phdr, @ptr_cast(bytes.ptr + hdr.e_phoff))[0..hdr.e_phnum];
        ...
        ctx.shstrtab = ctx.get_section_contents(ctx.hdr.e_shstrndx);

        for (ctx.shdrs, 0..) |shdr, i| switch (shdr.sh_type) {
            elf.SHT_SYMTAB, elf.SHT_DYNSYM => {
                const raw = ctx.get_section_contents(i);
                const nsyms = @div_exact(raw.len, @size_of(elf.Elf64_Sym));
                const symbols = @as([*]align(1) const elf.Elf64_Sym, @ptr_cast(raw.ptr))[0..nsyms];
                const strings = ctx.get_section_contents(shdr.sh_link);
        ...
        switch (check.kind) {
            .headers => {
                try ctx.dump_header(writer);
                try ctx.dump_shdrs(writer);
                try ctx.dump_phdrs(writer);
            },

            .symtab => if (ctx.symtab.symbols.len > 0) {
                try ctx.dump_symtab(.symtab, writer);
            } else return step.fail("no symbol table found", .{}),

            .dynamic_symtab => if (ctx.dysymtab.symbols.len > 0) {
                try ctx.dump_symtab(.dysymtab, writer);
            } else return step.fail("no dynamic symbol table found", .{}),

            .dynamic_section => if (ctx.get_section_by_name(".dynamic")) |shndx| {
                try ctx.dump_dynamic_section(shndx, writer);
            } else return step.fail("no .dynamic section found", .{}),

            .dump_section => {
                const name = mem.slice_to(@as([*:0]const u8, @ptr_cast(check.data.items.ptr + check.payload.dump_section)), 0);
                const shndx = ctx.get_section_by_name(name) orelse return step.fail("no '{s}' section found", .{name});
                try ctx.dump_section(shndx, writer);
            },

            else => return step.fail("invalid check kind for ELF file format: {s}", .{@tag_name(check.kind)}),
        }
    ...
    const ObjectContext = struct {
        ...
        fn dump_phdrs(ctx: ObjectContext, writer: anytype) !void {
            ...
            try writer.write_all("program headers\n");

            for (ctx.phdrs, 0..) |phdr, phndx| {
                try writer.print("phdr {d}\n", .{phndx});
                try writer.print("type {s}\n", .{fmt_ph_type(phdr.p_type)});
...

                {
                    ...
                    try writer.write_all("flags");
                    if (flags > 0) try writer.write_byte(' ');
                    if (flags & elf.PF_R != 0) {
                        try writer.write_byte('R');
                    }
                    if (flags & elf.PF_W != 0) {
                        try writer.write_byte('W');
                    }
                    if (flags & elf.PF_X != 0) {
                        try writer.write_byte('E');
                    }
                    if (flags & elf.PF_MASKOS != 0) {
                        try writer.write_all("OS");
                    }
                    if (flags & elf.PF_MASKPROC != 0) {
                        try writer.write_all("PROC");
                    }
                    try writer.write_byte('\n');
                }
            }
        ...
        fn dump_shdrs(ctx: ObjectContext, writer: anytype) !void {
            ...
            try writer.write_all("section headers\n");

            for (ctx.shdrs, 0..) |shdr, shndx| {
                try writer.print("shdr {d}\n", .{shndx});
                try writer.print("name {s}\n", .{ctx.get_section_name(shndx)});
                try writer.print("type {s}\n", .{fmt_sh_type(shdr.sh_type)});
        ...
        fn dump_dynamic_section(ctx: ObjectContext, shndx: usize, writer: anytype) !void {
            ...
            const strtab = ctx.get_section_contents(shdr.sh_link);
            const data = ctx.get_section_contents(shndx);
            const nentries = @div_exact(data.len, @size_of(elf.Elf64_Dyn));
            const entries = @as([*]align(1) const elf.Elf64_Dyn, @ptr_cast(data.ptr))[0..nentries];

            try writer.write_all(ElfDumper.dynamic_section_label ++ "\n");

            for (entries) |entry| {
                const key = @as(u64, @bit_cast(entry.d_tag));
                ...
                switch (key) {
                    ...
                    elf.DT_RUNPATH,
                    => {
                        const name = get_string(strtab, @int_cast(value));
                    ...
                    elf.DT_PLTREL => try writer.write_all(switch (value) {
                        elf.DT_REL => " REL",
                        elf.DT_RELA => " RELA",
                        else => " UNKNOWN",
                    }),
                    ...
                    elf.DT_FLAGS => if (value > 0) {
                        if (value & elf.DF_ORIGIN != 0) try writer.write_all(" ORIGIN");
                        if (value & elf.DF_SYMBOLIC != 0) try writer.write_all(" SYMBOLIC");
                        if (value & elf.DF_TEXTREL != 0) try writer.write_all(" TEXTREL");
                        if (value & elf.DF_BIND_NOW != 0) try writer.write_all(" BIND_NOW");
                        if (value & elf.DF_STATIC_TLS != 0) try writer.write_all(" STATIC_TLS");
                    },

                    elf.DT_FLAGS_1 => if (value > 0) {
                        if (value & elf.DF_1_NOW != 0) try writer.write_all(" NOW");
                        if (value & elf.DF_1_GLOBAL != 0) try writer.write_all(" GLOBAL");
                        if (value & elf.DF_1_GROUP != 0) try writer.write_all(" GROUP");
                        if (value & elf.DF_1_NODELETE != 0) try writer.write_all(" NODELETE");
                        if (value & elf.DF_1_LOADFLTR != 0) try writer.write_all(" LOADFLTR");
                        if (value & elf.DF_1_INITFIRST != 0) try writer.write_all(" INITFIRST");
                        if (value & elf.DF_1_NOOPEN != 0) try writer.write_all(" NOOPEN");
                        if (value & elf.DF_1_ORIGIN != 0) try writer.write_all(" ORIGIN");
                        if (value & elf.DF_1_DIRECT != 0) try writer.write_all(" DIRECT");
                        if (value & elf.DF_1_TRANS != 0) try writer.write_all(" TRANS");
                        if (value & elf.DF_1_INTERPOSE != 0) try writer.write_all(" INTERPOSE");
                        if (value & elf.DF_1_NODEFLIB != 0) try writer.write_all(" NODEFLIB");
                        if (value & elf.DF_1_NODUMP != 0) try writer.write_all(" NODUMP");
                        if (value & elf.DF_1_CONFALT != 0) try writer.write_all(" CONFALT");
                        if (value & elf.DF_1_ENDFILTEE != 0) try writer.write_all(" ENDFILTEE");
                        if (value & elf.DF_1_DISPRELDNE != 0) try writer.write_all(" DISPRELDNE");
                        if (value & elf.DF_1_DISPRELPND != 0) try writer.write_all(" DISPRELPND");
                        if (value & elf.DF_1_NODIRECT != 0) try writer.write_all(" NODIRECT");
                        if (value & elf.DF_1_IGNMULDEF != 0) try writer.write_all(" IGNMULDEF");
                        if (value & elf.DF_1_NOKSYMS != 0) try writer.write_all(" NOKSYMS");
                        if (value & elf.DF_1_NOHDR != 0) try writer.write_all(" NOHDR");
                        if (value & elf.DF_1_EDITED != 0) try writer.write_all(" EDITED");
                        if (value & elf.DF_1_NORELOC != 0) try writer.write_all(" NORELOC");
                        if (value & elf.DF_1_SYMINTPOSE != 0) try writer.write_all(" SYMINTPOSE");
                        if (value & elf.DF_1_GLOBAUDIT != 0) try writer.write_all(" GLOBAUDIT");
                        if (value & elf.DF_1_SINGLETON != 0) try writer.write_all(" SINGLETON");
                        if (value & elf.DF_1_STUB != 0) try writer.write_all(" STUB");
                        if (value & elf.DF_1_PIE != 0) try writer.write_all(" PIE");
                    },
        ...
        fn dump_symtab(ctx: ObjectContext, comptime @"type": enum { symtab, dysymtab }, writer: anytype) !void {
            ...
            try writer.write_all(switch (@"type") {
                .symtab => symtab_label,
            ...
            for (symtab.symbols, 0..) |sym, index| {
                ...
                try writer.print(" {s}", .{@tag_name(sym_vis)});

                const sym_name = switch (sym.st_type()) {
                    elf.STT_SECTION => ctx.get_section_name(sym.st_shndx),
                    else => symtab.get_name(index).?,
                };
                try writer.print(" {s}\n", .{sym_name});
            }
        }
        ...
        inline fn get_section_name(ctx: ObjectContext, shndx: usize) []const u8 {
            const shdr = ctx.shdrs[shndx];
            return get_string(ctx.shstrtab, shdr.sh_name);
        }
        ...
        fn get_section_by_name(ctx: ObjectContext, name: []const u8) ?usize {
            for (0..ctx.shdrs.len) |shndx| {
                if (mem.eql(u8, ctx.get_section_name(shndx), name)) return shndx;
            } else return null;
        }
    };
    ...
    const Symtab = struct {
        ...
        fn get_name(st: Symtab, index: usize) ?[]const u8 {
            ...
            return get_string(st.strings, sym.st_name);
        }
    };

    fn get_string(strtab: []const u8, off: u32) []const u8 {
        assert(off < strtab.len);
        return mem.slice_to(@as([*:0]const u8, @ptr_cast(strtab.ptr + off)), 0);
    }

    fn fmt_sh_type(sh_type: u32) std.fmt.Formatter(format_sh_type) {
        return .{ .data = sh_type };
    }
    ...
    fn fmt_ph_type(ph_type: u32) std.fmt.Formatter(format_ph_type) {
        return .{ .data = ph_type };
    }
...
const WasmDumper = struct {
    ...
    fn parse_and_dump(step: *Step, check: Check, bytes: []const u8) ![]const u8 {
        const gpa = step.owner.allocator;
        var fbs = std.io.fixed_buffer_stream(bytes);
        const reader = fbs.reader();

        const buf = try reader.read_bytes_no_eof(8);
        ...
        switch (check.kind) {
            .headers => {
                while (reader.read_byte()) |current_byte| {
                    const section = std.meta.int_to_enum(std.wasm.Section, current_byte) catch {
                        return step.fail("Found invalid section id '{d}'", .{current_byte});
                    };

                    const section_length = try std.leb.read_uleb128(u32, reader);
                    try parse_and_dump_section(step, section, bytes[fbs.pos..][0..section_length], writer);
                    fbs.pos += section_length;
                } else |_| {} // reached end of stream
            },

            else => return step.fail("invalid check kind for Wasm file format: {s}", .{@tag_name(check.kind)}),
        }
    ...
    fn parse_and_dump_section(
        ...
        switch (section) {
            ...
            .data,
            => {
                const entries = try std.leb.read_uleb128(u32, reader);
                try writer.print("\nentries {d}\n", .{entries});
                try parse_section(step, section, data[fbs.pos..], entries, writer);
            },
            .custom => {
                const name_length = try std.leb.read_uleb128(u32, reader);
                const name = data[fbs.pos..][0..name_length];
                fbs.pos += name_length;
                try writer.print("\nname {s}\n", .{name});

                if (mem.eql(u8, name, "name")) {
                    try parse_dump_names(step, reader, writer, data);
                } else if (mem.eql(u8, name, "producers")) {
                    try parse_dump_producers(reader, writer, data);
                } else if (mem.eql(u8, name, "target_features")) {
                    try parse_dump_features(reader, writer, data);
                }
                // TODO: Implement parsing and dumping other custom sections (such as relocations)
            },
            .start => {
                const start = try std.leb.read_uleb128(u32, reader);
                try writer.print("\nstart {d}\n", .{start});
            },
            .data_count => {
                const count = try std.leb.read_uleb128(u32, reader);
                try writer.print("\ncount {d}\n", .{count});
            },
    ...
    fn parse_section(step: *Step, section: std.wasm.Section, data: []const u8, entries: u32, writer: anytype) !void {
        ...
        switch (section) {
            .type => {
                var i: u32 = 0;
                while (i < entries) : (i += 1) {
                    const func_type = try reader.read_byte();
                    if (func_type != std.wasm.function_type) {
                        return step.fail("expected function type, found byte '{d}'", .{func_type});
                    }
                    const params = try std.leb.read_uleb128(u32, reader);
                    try writer.print("params {d}\n", .{params});
                    var index: u32 = 0;
                    while (index < params) : (index += 1) {
                        _ = try parse_dump_type(step, std.wasm.Valtype, reader, writer);
                    } else index = 0;
                    const returns = try std.leb.read_uleb128(u32, reader);
            ...
            .import => {
                var i: u32 = 0;
                while (i < entries) : (i += 1) {
                    const module_name_len = try std.leb.read_uleb128(u32, reader);
                    const module_name = data[fbs.pos..][0..module_name_len];
                    fbs.pos += module_name_len;
                    const name_len = try std.leb.read_uleb128(u32, reader);
                    const name = data[fbs.pos..][0..name_len];
                    fbs.pos += name_len;

                    const kind = std.meta.int_to_enum(std.wasm.ExternalKind, try reader.read_byte()) catch {
                    ...
                    try writer.print(
                        ...
                        \\name {s}
                        \\kind {s}
                    , .{ module_name, name, @tag_name(kind) });
                    try writer.write_byte('\n');
                    switch (kind) {
                        .function => {
                            try writer.print("index {d}\n", .{try std.leb.read_uleb128(u32, reader)});
                        },
                        .memory => {
                            try parse_dump_limits(reader, writer);
                        },
                        .global => {
                            _ = try parse_dump_type(step, std.wasm.Valtype, reader, writer);
                            try writer.print("mutable {}\n", .{0x01 == try std.leb.read_uleb128(u32, reader)});
                        },
                        .table => {
                            _ = try parse_dump_type(step, std.wasm.RefType, reader, writer);
                            try parse_dump_limits(reader, writer);
                        },
                    }
                }
            ...
            .global => {
                ...
                while (i < entries) : (i += 1) {
                    _ = try parse_dump_type(step, std.wasm.Valtype, reader, writer);
                    try writer.print("mutable {}\n", .{0x01 == try std.leb.read_uleb128(u1, reader)});
                    try parse_dump_init(step, reader, writer);
                }
            },
            .@"export" => {
                var i: u32 = 0;
                while (i < entries) : (i += 1) {
                    ...
                    const name = data[fbs.pos..][0..name_len];
                    fbs.pos += name_len;
                    const kind_byte = try std.leb.read_uleb128(u8, reader);
                    const kind = std.meta.int_to_enum(std.wasm.ExternalKind, kind_byte) catch {
                        return step.fail("invalid export kind value '{d}'", .{kind_byte});
                    };
                    const index = try std.leb.read_uleb128(u32, reader);
                    try writer.print(
                        \\name {s}
                        \\kind {s}
                        \\index {d}
                    , .{ name, @tag_name(kind), index });
                    try writer.write_byte('\n');
                }
            },
            .element => {
                ...
                while (i < entries) : (i += 1) {
                    ...
                    const function_indexes = try std.leb.read_uleb128(u32, reader);
            ...
            .data => {
                var i: u32 = 0;
                while (i < entries) : (i += 1) {
                    const flags = try std.leb.read_uleb128(u32, reader);
                    ...
                    const size = try std.leb.read_uleb128(u32, reader);
                    try writer.print("size {d}\n", .{size});
                    try reader.skip_bytes(size, .{}); // we do not care about the content of the segments
                }
            },
            else => unreachable,
        }
    ...
    fn parse_dump_type(step: *Step, comptime E: type, reader: anytype, writer: anytype) !E {
        const byte = try reader.read_byte();
        const tag = std.meta.int_to_enum(E, byte) catch {
    ...
    fn parse_dump_limits(reader: anytype, writer: anytype) !void {
        const flags = try std.leb.read_uleb128(u8, reader);
        const min = try std.leb.read_uleb128(u32, reader);
    ...
    fn parse_dump_init(step: *Step, reader: anytype, writer: anytype) !void {
        const byte = try reader.read_byte();
        const opcode = std.meta.int_to_enum(std.wasm.Opcode, byte) catch {
            return step.fail("invalid wasm opcode '{d}'", .{byte});
        };
        switch (opcode) {
            .i32_const => try writer.print("i32.const {x}\n", .{try std.leb.read_ileb128(i32, reader)}),
            .i64_const => try writer.print("i64.const {x}\n", .{try std.leb.read_ileb128(i64, reader)}),
            .f32_const => try writer.print("f32.const {x}\n", .{@as(f32, @bit_cast(try reader.read_int(u32, .little)))}),
            .f64_const => try writer.print("f64.const {x}\n", .{@as(f64, @bit_cast(try reader.read_int(u64, .little)))}),
            .global_get => try writer.print("global.get {x}\n", .{try std.leb.read_uleb128(u32, reader)}),
            else => unreachable,
        }
        const end_opcode = try std.leb.read_uleb128(u8, reader);
    ...
    fn parse_dump_names(step: *Step, reader: anytype, writer: anytype, data: []const u8) !void {
        while (reader.context.pos < data.len) {
            switch (try parse_dump_type(step, std.wasm.NameSubsection, reader, writer)) {
                ...
                // function indices.
                .local => {
                    return step.fail("TODO implement parse_dump_names for local subsections", .{});
                },

                else => |t| return step.fail("invalid subsection type: {s}", .{@tag_name(t)}),
            }
        }
    }

    fn parse_dump_producers(reader: anytype, writer: anytype, data: []const u8) !void {
        const field_count = try std.leb.read_uleb128(u32, reader);
        try writer.print("fields {d}\n", .{field_count});
        var current_field: u32 = 0;
        while (current_field < field_count) : (current_field += 1) {
            const field_name_length = try std.leb.read_uleb128(u32, reader);
            const field_name = data[reader.context.pos..][0..field_name_length];
            reader.context.pos += field_name_length;

            const value_count = try std.leb.read_uleb128(u32, reader);
            try writer.print(
                \\field_name {s}
                \\values {d}
            ...
            while (current_value < value_count) : (current_value += 1) {
                const value_length = try std.leb.read_uleb128(u32, reader);
                const value = data[reader.context.pos..][0..value_length];
                reader.context.pos += value_length;

                const version_length = try std.leb.read_uleb128(u32, reader);
    ...
    fn parse_dump_features(reader: anytype, writer: anytype, data: []const u8) !void {
        const feature_count = try std.leb.read_uleb128(u32, reader);
        try writer.print("features {d}\n", .{feature_count});

        var index: u32 = 0;
        while (index < feature_count) : (index += 1) {
            const prefix_byte = try std.leb.read_uleb128(u8, reader);
            const name_length = try std.leb.read_uleb128(u32, reader);
