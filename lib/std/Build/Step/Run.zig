...
/// See also add_arg and add_args to modifying this directly
argv: std.ArrayListUnmanaged(Arg),

/// Use `set_cwd` to set the initial current working directory
cwd: ?Build.LazyPath,

/// Override this field to modify the environment, or use set_environment_variable
env_map: ?*EnvMap,
...
/// It should be only set using `set_std_in`.
stdin: StdIn,

/// Deprecated: use `add_file_input`
...
pub const StdIo = union(enum) {
    ...
    /// of the args is an output file (added with `add_output_file_arg`).
...
pub fn set_name(run: *Run, name: []const u8) void {
    run.step.name = name;
    run.rename_step_with_output_arg = false;
}

pub fn enable_test_runner_mode(run: *Run) void {
    run.stdio = .zig_test;
    run.add_args(&.{"--listen=-"});
}

pub fn add_artifact_arg(run: *Run, artifact: *Step.Compile) void {
    const b = run.step.owner;
    const bin_file = artifact.get_emitted_bin();
    bin_file.add_step_dependencies(&run.step);
...
///
/// Related:
/// * `add_prefixed_output_file_arg` - same thing but prepends a string to the argument
/// * `add_file_arg` - for input files given to the child process
pub fn add_output_file_arg(run: *Run, basename: []const u8) std.Build.LazyPath {
    return run.add_prefixed_output_file_arg("", basename);
}
...
///
/// Related:
/// * `add_output_file_arg` - same thing but without the prefix
/// * `add_file_arg` - for input files given to the child process
pub fn add_prefixed_output_file_arg(
    run: *Run,
    prefix: []const u8,
    basename: []const u8,
) std.Build.LazyPath {
    ...
    if (run.rename_step_with_output_arg) {
        run.set_name(b.fmt("{s} ({s})", .{ run.step.name, basename }));
    }
...
///
/// Related:
/// * `add_prefixed_file_arg` - same thing but prepends a string to the argument
/// * `add_output_file_arg` - for files generated by the child process
pub fn add_file_arg(run: *Run, lp: std.Build.LazyPath) void {
    run.add_prefixed_file_arg("", lp);
}
...
///
/// Related:
/// * `add_file_arg` - same thing but without the prefix
/// * `add_output_file_arg` - for files generated by the child process
pub fn add_prefixed_file_arg(run: *Run, prefix: []const u8, lp: std.Build.LazyPath) void {
    const b = run.step.owner;
    ...
    lp.add_step_dependencies(&run.step);
}
...
///
/// Related:
/// * `add_prefixed_output_directory_arg` - same thing but prepends a string to the argument
/// * `add_directory_arg` - for input directories given to the child process
pub fn add_output_directory_arg(run: *Run, basename: []const u8) std.Build.LazyPath {
    return run.add_prefixed_output_directory_arg("", basename);
}
...
///
/// Related:
/// * `add_output_directory_arg` - same thing but without the prefix
/// * `add_directory_arg` - for input directories given to the child process
pub fn add_prefixed_output_directory_arg(
    run: *Run,
    prefix: []const u8,
    basename: []const u8,
) std.Build.LazyPath {
...
/// deprecated: use `add_directory_arg`
pub const addDirectorySourceArg = add_directory_arg;

pub fn add_directory_arg(run: *Run, directory_source: std.Build.LazyPath) void {
    run.add_prefixed_directory_arg("", directory_source);
}

// deprecated: use `add_prefixed_directory_arg`
pub const addPrefixedDirectorySourceArg = add_prefixed_directory_arg;

pub fn add_prefixed_directory_arg(run: *Run, prefix: []const u8, directory_source: std.Build.LazyPath) void {
    const b = run.step.owner;
    ...
    directory_source.add_step_dependencies(&run.step);
}
...
pub fn add_dep_file_output_arg(run: *Run, basename: []const u8) std.Build.LazyPath {
    return run.add_prefixed_dep_file_output_arg("", basename);
}

/// Add a prefixed path argument to a dep file (.d) for the child process to
/// write its discovered additional dependencies.
/// Only one dep file argument is allowed by instance.
pub fn add_prefixed_dep_file_output_arg(run: *Run, prefix: []const u8, basename: []const u8) std.Build.LazyPath {
    const b = run.step.owner;
...
pub fn add_arg(run: *Run, arg: []const u8) void {
    const b = run.step.owner;
    run.argv.append(b.allocator, .{ .bytes = b.dupe(arg) }) catch @panic("OOM");
}

pub fn add_args(run: *Run, args: []const []const u8) void {
    for (args) |arg| run.add_arg(arg);
}

pub fn set_std_in(run: *Run, stdin: StdIn) void {
    switch (stdin) {
        .lazy_path => |lazy_path| lazy_path.add_step_dependencies(&run.step),
        .bytes, .none => {},
    }
    run.stdin = stdin;
}

pub fn set_cwd(run: *Run, cwd: Build.LazyPath) void {
    cwd.add_step_dependencies(&run.step);
    run.cwd = cwd.dupe(run.step.owner);
}

pub fn clear_environment(run: *Run) void {
    const b = run.step.owner;
...
pub fn add_path_dir(run: *Run, search_path: []const u8) void {
    const b = run.step.owner;
    const env_map = get_env_map_internal(run);
    ...
    if (prev_path) |pp| {
        ...
        env_map.put(key, b.dupe_path(search_path)) catch @panic("OOM");
    }
}

pub fn get_env_map(run: *Run) *EnvMap {
    return get_env_map_internal(run);
}

fn get_env_map_internal(run: *Run) *EnvMap {
    const arena = run.step.owner.allocator;
    return run.env_map orelse {
        const env_map = arena.create(EnvMap) catch @panic("OOM");
        env_map.* = process.get_env_map(arena) catch @panic("unhandled error");
        run.env_map = env_map;
        return env_map;
    };
}

pub fn set_environment_variable(run: *Run, key: []const u8, value: []const u8) void {
    const b = run.step.owner;
    const env_map = run.get_env_map();
    env_map.put(b.dupe(key), b.dupe(value)) catch @panic("unhandled error");
}

pub fn remove_environment_variable(run: *Run, key: []const u8) void {
    run.get_env_map().remove(key);
}

/// Adds a check for exact stderr match. Does not add any other checks.
pub fn expect_std_err_equal(run: *Run, bytes: []const u8) void {
    const new_check: StdIo.Check = .{ .expect_stderr_exact = run.step.owner.dupe(bytes) };
    run.add_check(new_check);
}

/// Adds a check for exact stdout match as well as a check for exit code 0, if
/// there is not already an expected termination check.
pub fn expect_std_out_equal(run: *Run, bytes: []const u8) void {
    const new_check: StdIo.Check = .{ .expect_stdout_exact = run.step.owner.dupe(bytes) };
    run.add_check(new_check);
    if (!run.has_term_check()) {
        run.expect_exit_code(0);
    }
}

pub fn expect_exit_code(run: *Run, code: u8) void {
    const new_check: StdIo.Check = .{ .expect_term = .{ .Exited = code } };
    run.add_check(new_check);
}

pub fn has_term_check(run: Run) bool {
...
pub fn add_check(run: *Run, new_check: StdIo.Check) void {
    const b = run.step.owner;
    ...
    switch (run.stdio) {
        ...
        else => @panic("illegal call to add_check: conflicting helper method calls. Suggest to directly set stdio field of Run instead"),
    }
}

pub fn capture_std_err(run: *Run) std.Build.LazyPath {
...
pub fn capture_std_out(run: *Run) std.Build.LazyPath {
    assert(run.stdio != .inherit);
...
pub fn add_file_input(self: *Run, file_input: std.Build.LazyPath) void {
    file_input.add_step_dependencies(&self.step);
...
fn has_side_effects(run: Run) bool {
    if (run.has_side_effects) return true;
    return switch (run.stdio) {
        .infer_from_args => !run.has_any_output_args(),
        .inherit => true,
        .check => false,
        .zig_test => false,
    };
...
fn has_any_output_args(run: Run) bool {
...
fn checks_contain_stdout(checks: []const StdIo.Check) bool {
...
fn checks_contain_stderr(checks: []const StdIo.Check) bool {
    for (checks) |check| switch (check) {
        .expect_stdout_exact,
        .expect_stdout_match,
        .expect_term,
...
fn make(step: *Step, prog_node: std.Progress.Node) !void {
    ...
    const has_side_effects = run.has_side_effects();
    ...
    for (run.argv.items) |arg| {
        switch (arg) {
            ...
            .lazy_path => |file| {
                const file_path = file.lazy_path.get_path2(b, step);
                try argv_list.append(b.fmt("{s}{s}", .{ file.prefix, file_path }));
                man.hash.add_bytes(file.prefix);
                _ = try man.add_file(file_path, null);
            },
            .directory_source => |file| {
                const file_path = file.lazy_path.get_path2(b, step);
                try argv_list.append(b.fmt("{s}{s}", .{ file.prefix, file_path }));
                man.hash.add_bytes(file.prefix);
                man.hash.add_bytes(file_path);
            },
            .artifact => |artifact| {
                if (artifact.root_module_target().os.tag == .windows) {
                    // On Windows we don't have rpaths so we have to add .dll search paths to PATH
                    run.add_path_for_dyn_libs(artifact);
                }
            ...
            .output_file, .output_directory => |output| {
                man.hash.add_bytes(output.prefix);
                man.hash.add_bytes(output.basename);
    ...
    switch (run.stdin) {
        ...
        .lazy_path => |lazy_path| {
            const file_path = lazy_path.get_path2(b, step);
            _ = try man.add_file(file_path, null);
        },
        .none => {},
    }
    ...
    hash_std_io(&man.hash, run.stdio);

    for (run.extra_file_dependencies) |file_path| {
        _ = try man.add_file(b.path_from_root(file_path), null);
    }
    for (run.file_inputs.items) |lazy_path| {
        _ = try man.add_file(lazy_path.get_path2(b, step), null);
    }

    if (!has_side_effects and try step.cache_hit(&man)) {
        // cache hit, skip running command
        const digest = man.final();

        try populate_generated_paths(
            arena,
    ...
    const dep_output_file = run.dep_output_file orelse {
        ...
        for (output_placeholders.items) |placeholder| {
            const output_sub_path = b.path_join(&.{ output_dir_path, placeholder.output.basename });
            ...
            b.cache_root.handle.make_path(output_sub_dir_path) catch |err| {
        ...
        try run_command(run, argv_list.items, has_side_effects, output_dir_path, prog_node);
        if (!has_side_effects) try step.write_manifest(&man);
        return;
    };
    ...
    for (output_placeholders.items) |placeholder| {
        ...
        const output_sub_path = b.path_join(&output_components);
        const output_sub_dir_path = switch (placeholder.tag) {
            .output_file => fs.path.dirname(output_sub_path).?,
            .output_directory => output_sub_path,
            else => unreachable,
    ...
    try run_command(run, argv_list.items, has_side_effects, tmp_dir_path, prog_node);

    const dep_file_dir = std.fs.cwd();
    const dep_file_basename = dep_output_file.generated_file.get_path();
    if (has_side_effects)
        try man.add_dep_file(dep_file_dir, dep_file_basename)
    else
        try man.add_dep_file_post(dep_file_dir, dep_file_basename);
    ...
    if (any_output) {
        ...
        b.cache_root.handle.rename(tmp_dir_path, o_sub_path) catch |err| {
            if (err == error.PathAlreadyExists) {
                b.cache_root.handle.delete_tree(o_sub_path) catch |del_err| {
...
fn format_term(
    ...
    if (term) |t| switch (t) {
        ...
        try writer.write_all("exited with any code");
    }
}
fn fmt_term(term: ?std.process.Child.Term) std.fmt.Formatter(format_term) {
    return .{ .data = term };
}

fn term_matches(expected: ?std.process.Child.Term, actual: std.process.Child.Term) bool {
...
fn run_command(
    ...
    const cwd: ?[]const u8 = if (run.cwd) |lazy_cwd| lazy_cwd.get_path2(b, step) else null;

    try step.handle_child_proc_unsupported(cwd, argv);
    try Step.handle_verbose2(step.owner, cwd, run.env_map, argv);
    ...
    const result = spawn_child_and_collect(run, argv, has_side_effects, prog_node) catch |err| term: {
        ...
        if (err == error.InvalidExe or err == error.FileNotFound) interpret: {
            ...
            const need_cross_glibc = exe.root_module_target().is_gnu_lib_c() and
                exe.is_linking_libc;
            const other_target = exe.root_module.resolved_target.?.result;
            switch (std.zig.system.get_external_executor(b.host.result, &other_target, .{
                ...
                .wine => |bin_name| {
                    if (b.enable_wine) {
                        ...
                        try interp_argv.append_slice(argv);
                    } else {
                        return fail_foreign(run, "-fwine", argv[0], exe);
                    }
                },
                .qemu => |bin_name| {
                    if (b.enable_qemu) {
                        const glibc_dir_arg = if (need_cross_glibc)
                            b.glibc_runtimes_dir orelse
                                return fail_foreign(run, "--glibc-runtimes", argv[0], exe)
                        else
                            null;
                        ...
                        if (glibc_dir_arg) |dir| {
                            // TODO look into making this a call to `linux_triple`. This
                            // needs the directory to be called "i686" rather than
                            // "x86" which is why we do it manually here.
                            const fmt_str = "{s}" ++ fs.path.sep_str ++ "{s}-{s}-{s}";
                            const cpu_arch = exe.root_module_target().cpu.arch;
                            const os_tag = exe.root_module_target().os.tag;
                            const abi = exe.root_module_target().abi;
                            const cpu_arch_name: []const u8 = if (cpu_arch == .x86)
                                "i686"
                            else
                                @tag_name(cpu_arch);
                            const full_dir = try std.fmt.alloc_print(b.allocator, fmt_str, .{
                                dir, cpu_arch_name, @tag_name(os_tag), @tag_name(abi),
                            });
                        ...
                        return fail_foreign(run, "-fqemu", argv[0], exe);
                    }
                },
                .darling => |bin_name| {
                    if (b.enable_darling) {
                        ...
                        return fail_foreign(run, "-fdarling", argv[0], exe);
                    }
                },
                .wasmtime => |bin_name| {
                    if (b.enable_wasmtime) {
                        ...
                        try interp_argv.append_slice(argv[1..]);
                    } else {
                        return fail_foreign(run, "-fwasmtime", argv[0], exe);
                    }
                },
                .bad_dl => |foreign_dl| {
                    if (allow_skip) return error.MakeSkipped;
                ...
                .bad_os_or_cpu => {
                    ...
                    const host_name = try b.host.result.zig_triple(b.allocator);
                    const foreign_name = try exe.root_module_target().zig_triple(b.allocator);
            ...
            if (exe.root_module_target().os.tag == .windows) {
                // On Windows we don't have rpaths so we have to add .dll search paths to PATH
                run.add_path_for_dyn_libs(exe);
            }

            try Step.handle_verbose2(step.owner, cwd, run.env_map, interp_argv.items);

            break :term spawn_child_and_collect(run, interp_argv.items, has_side_effects, prog_node) catch |e| {
    ...
    for ([_]Stream{
        ...
        if (stream.captured) |output| {
            ...
            const sub_path = b.path_join(&output_components);
            ...
            b.cache_root.handle.write_file(.{ .sub_path = sub_path, .data = stream.bytes.? }) catch |err| {
    ...
    switch (run.stdio) {
        .check => |checks| for (checks.items) |check| switch (check) {
            .expect_stderr_exact => |expected_bytes| {
                if (!mem.eql(u8, expected_bytes, result.stdio.stderr.?)) {
                    ...
                    , .{
                        expected_bytes,
                        result.stdio.stderr.?,
                        try Step.alloc_print_cmd(arena, cwd, final_argv),
                    });
                }
            },
            .expect_stderr_match => |match| {
                if (mem.index_of(u8, result.stdio.stderr.?, match) == null) {
                    return step.fail(
                        \\
            ...
            .expect_stdout_match => |match| {
                if (mem.index_of(u8, result.stdio.stdout.?, match) == null) {
                    return step.fail(
                        \\
            ...
            .expect_term => |expected_term| {
                if (!term_matches(expected_term, result.term)) {
                    return step.fail("the following command {} (expected {}):\n{s}", .{
                        fmt_term(result.term),
                        fmt_term(expected_term),
                        try Step.alloc_print_cmd(arena, cwd, final_argv),
                    });
                }
            },
        ...
        .zig_test => {
            const prefix: []const u8 = p: {
                if (result.stdio.test_metadata) |tm| {
                    if (tm.next_index > 0 and tm.next_index <= tm.names.len) {
                        const name = tm.test_name(tm.next_index - 1);
            ...
            if (!result.stdio.test_results.is_success()) {
                return step.fail(
                    "{s}the following test command failed:\n{s}",
                    .{ prefix, try Step.alloc_print_cmd(arena, cwd, final_argv) },
                );
            }
        },
        else => {
            try step.handle_child_process_term(result.term, cwd, final_argv);
        },
    }
}
...
fn spawn_child_and_collect(
    ...
    if (run.cwd) |lazy_cwd| {
        child.cwd = lazy_cwd.get_path2(b, &run.step);
    } else {
    ...
    child.stdout_behavior = switch (run.stdio) {
        .infer_from_args => if (has_side_effects) .Inherit else .Ignore,
        .inherit => .Inherit,
        .check => |checks| if (checks_contain_stdout(checks.items)) .Pipe else .Ignore,
        .zig_test => .Pipe,
    };
    ...
    const term, const result, const elapsed_ns = t: {
        if (inherit) std.debug.lock_std_err();
        defer if (inherit) std.debug.unlock_std_err();

        try child.spawn();
        var timer = try std.time.Timer.start();

        const result = if (run.stdio == .zig_test)
            eval_zig_test(run, &child, prog_node)
        else
            eval_generic(run, &child);
    ...
    return .{
        ...
        .peak_rss = child.resource_usage_statistics.get_max_rss() orelse 0,
    };
}
...
fn eval_zig_test(
    ...
    try send_message(child.stdin.?, .query_test_metadata);
    ...
    poll: while (true) {
        ...
        switch (header.tag) {
            ...
            .test_metadata => {
                ...
                const tm_hdr = @as(*align(1) const TmHdr, @ptr_cast(body));
                test_count = tm_hdr.tests_len;

                const names_bytes = body[@size_of(TmHdr)..][0 .. test_count * @size_of(u32)];
                const expected_panic_msgs_bytes = body[@size_of(TmHdr) + names_bytes.len ..][0 .. test_count * @size_of(u32)];
                const string_bytes = body[@size_of(TmHdr) + names_bytes.len + expected_panic_msgs_bytes.len ..][0..tm_hdr.string_bytes_len];

                const names = std.mem.bytes_as_slice(u32, names_bytes);
                const expected_panic_msgs = std.mem.bytes_as_slice(u32, expected_panic_msgs_bytes);
                ...
                prog_node.set_estimated_total_items(names.len);
                metadata = .{
                ...
                try request_next_test(child.stdin.?, &metadata.?, &sub_prog_node);
            },
            .test_results => {
                const md = metadata.?;

                const TrHdr = std.zig.Server.Message.TestResults;
                const tr_hdr = @as(*align(1) const TrHdr, @ptr_cast(body));
                fail_count +|= @int_from_bool(tr_hdr.flags.fail);
                skip_count +|= @int_from_bool(tr_hdr.flags.skip);
                leak_count +|= @int_from_bool(tr_hdr.flags.leak);
                log_err_count +|= tr_hdr.flags.log_err_count;

                if (tr_hdr.flags.fail or tr_hdr.flags.leak or tr_hdr.flags.log_err_count > 0) {
                    const name = std.mem.slice_to(md.string_bytes[md.names[tr_hdr.index]..], 0);
                    const orig_msg = stderr.readable_slice(0);
                    ...
                    const label = if (tr_hdr.flags.fail)
                        ...
                        unreachable;
                    if (msg.len > 0) {
                        try run.step.add_error("'{s}' {s}: {s}", .{ name, label, msg });
                    } else {
                        try run.step.add_error("'{s}' {s}", .{ name, label });
                    }
                }
    ...
    if (stderr.readable_length() > 0) {
        const msg = std.mem.trim(u8, try stderr.to_owned_slice(), "\n");
...
const TestMetadata = struct {
    ...
    fn test_name(tm: TestMetadata, index: u32) []const u8 {
        return std.mem.slice_to(tm.string_bytes[tm.names[index]..], 0);
    }
};

fn request_next_test(in: fs.File, metadata: *TestMetadata, sub_prog_node: *?std.Progress.Node) !void {
    while (metadata.next_index < metadata.names.len) {
        ...
        const name = metadata.test_name(i);
        if (sub_prog_node.*) |n| n.end();
        sub_prog_node.* = metadata.prog_node.start(name, 0);

        try send_run_test_message(in, i);
        return;
    } else {
        try send_message(in, .exit);
    }
}
...
fn send_run_test_message(file: std.fs.File, index: u32) !void {
    const header: std.zig.Client.Message.Header = .{
        .tag = .run_test,
        .bytes_len = 4,
    };
    const full_msg = std.mem.as_bytes(&header) ++ std.mem.as_bytes(&index);
    try file.write_all(full_msg);
}

fn eval_generic(run: *Run, child: *std.process.Child) !StdIoResult {
    const b = run.step.owner;
    const arena = b.allocator;

    switch (run.stdin) {
        .bytes => |bytes| {
            child.stdin.?.write_all(bytes) catch |err| {
        ...
        .lazy_path => |lazy_path| {
            const path = lazy_path.get_path2(b, &run.step);
            const file = b.build_root.handle.open_file(path, .{}) catch |err| {
                return run.step.fail("unable to open stdin file: {s}", .{@errorName(err)});
            };
            defer file.close();
            child.stdin.?.write_file_all(file, .{}) catch |err| {
    ...
    if (child.stdout) |stdout| {
        if (child.stderr) |stderr| {
            ...
            stdout_bytes = try poller.fifo(.stdout).to_owned_slice();
            stderr_bytes = try poller.fifo(.stderr).to_owned_slice();
        } else {
            stdout_bytes = try stdout.reader().read_all_alloc(arena, run.max_stdio_size);
        }
    } else if (child.stderr) |stderr| {
        stderr_bytes = try stderr.reader().read_all_alloc(arena, run.max_stdio_size);
    }
    ...
    if (stderr_bytes) |bytes| if (bytes.len > 0) {
        ...
        const stderr_is_diagnostic = run.captured_stderr == null and switch (run.stdio) {
            .check => |checks| !checks_contain_stderr(checks.items),
            else => true,
        };
...
fn add_path_for_dyn_libs(run: *Run, artifact: *Step.Compile) void {
    const b = run.step.owner;
    var it = artifact.root_module.iterate_dependencies(artifact, true);
    while (it.next()) |item| {
        const other = item.compile.?;
        if (item.module == &other.root_module) {
            if (item.module.resolved_target.?.result.os.tag == .windows and
                other.is_dynamic_library())
            {
                add_path_dir(run, fs.path.dirname(other.get_emitted_bin().get_path2(b, &run.step)).?);
            }
        }
    }
}
...
fn hash_std_io(hh: *std.Build.Cache.HashHelper, stdio: StdIo) void {
    switch (stdio) {
