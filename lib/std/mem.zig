...
/// or the allocator.
pub fn validation_allocator(comptime T: type) type {
    return struct {
        const Self = @This();
...
fn slice_to(comptime T: type, comptime end: std.meta.Elem(T)) type {
    switch (@typeInfo(T)) {
        .Optional => |optional_info| {
...
pub inline fn read_int(comptime T: type, buffer: *const [@divExact(@typeInfo(T).Int.bits, 8)]u8, endian: Endian) T {
...
pub inline fn write_int(comptime T: type, buffer: *[@divExact(@typeInfo(T).Int.bits, 8)]u8, value: T, endian: Endian) void {
...
pub fn window_iterator(comptime T: type) type {
    return struct {
        buffer: []const T,
        index: ?usize,
        size: usize,
...
pub fn token_iterator(comptime T: type, comptime delimiter_type: DelimiterType) type {
    return struct {
        buffer: []const T,
...
pub fn split_iterator(comptime T: type, comptime delimiter_type: DelimiterType) type {
    return struct {
        buffer: []const T,
        index: ?usize,
        delimiter: switch (delimiter_type) {
...
pub fn split_backwards_iterator(comptime T: type, comptime delimiter_type: DelimiterType) type {
    return struct {
        buffer: []const T,
        index: ?usize,
        delimiter: switch (delimiter_type) {
...
fn reverse_iterator(comptime T: type) type {
    const Pointer = blk: {
        switch (@typeInfo(T)) {
            .Pointer => |ptr_info| switch (ptr_info.size) {
                .One => switch (@typeInfo(ptr_info.child)) {
...
fn as_bytes_return_type(comptime P: type) type {
...
pub fn to_bytes(value: anytype) [@sizeOf(@TypeOf(value))]u8 {
...
fn bytes_as_value_return_type(comptime T: type, comptime B: type) type {
...
fn bytes_as_slice_return_type(comptime T: type, comptime bytesType: type) type {
...
fn slice_as_bytes_return_type(comptime Slice: type) type {
...
fn aligned_slice(comptime AttributeSource: type, comptime new_alignment: usize) type {
