...
pub fn SbrkAllocator(comptime sbrk: *const fn (n: usize) usize) type {
    return struct {
        ...
        fn alloc(ctx: *anyopaque, len: usize, log2_align: u8, return_address: usize) ?[*]u8 {
            ...
            if (class < size_class_count) {
                const addr = a: {
                    ...
                    if (top_free_ptr != 0) {
                        const node = @as(*usize, @ptrFromInt(top_free_ptr + (slot_size - @size_of(usize))));
                        frees[class] = node.*;
            ...
            const addr = alloc_big_pages(bigpages_needed);
            return @as([*]u8, @ptrFromInt(addr));
        }
        ...
        fn free(
            ...
            if (class < size_class_count) {
                const node = @as(*usize, @ptrFromInt(addr + (slot_size - @size_of(usize))));
                node.* = frees[class];
                frees[class] = addr;
            } else {
                const bigpages_needed = big_pages_needed(actual_len);
                ...
                const node = @as(*usize, @ptrFromInt(addr + (big_slot_size_bytes - @size_of(usize))));
        ...
        fn alloc_big_pages(n: usize) usize {
            ...
            if (top_free_ptr != 0) {
                const node = @as(*usize, @ptrFromInt(top_free_ptr + (slot_size_bytes - @size_of(usize))));
