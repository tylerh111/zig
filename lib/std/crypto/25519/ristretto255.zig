...
pub const Ristretto255 = struct {
    ...
    fn sqrt_ratio_m1(u: Fe, v: Fe) struct { ratio_is_square: u32, root: Fe } {
        ...
        const has_m_root = m_root_check.is_zero();
        const has_p_root = p_root_check.is_zero();
        const has_f_root = f_root_check.is_zero();
        const x_sqrtm1 = x.mul(Fe.sqrtm1); // x*sqrt(-1)
        x.c_mov(x_sqrtm1, @int_from_bool(has_p_root) | @int_from_bool(has_f_root));
        return .{ .ratio_is_square = @int_from_bool(has_m_root) | @int_from_bool(has_p_root), .root = x.abs() };
    }
    ...
    pub fn from_bytes(s: [encoded_length]u8) (NonCanonicalError || EncodingError)!Ristretto255 {
        try reject_non_canonical(s);
        const s_ = Fe.from_bytes(s);
        const ss = s_.sq(); // s^2
        ...
        const inv_sqrt = sqrt_ratio_m1(Fe.one, v_u2u2);
        var x = inv_sqrt.root.mul(u2_);
        const y = inv_sqrt.root.mul(x).mul(v).mul(u1_);
        x = x.mul(s_);
        x = x.add(x).abs();
        const t = x.mul(y);
        if ((1 - inv_sqrt.ratio_is_square) | @int_from_bool(t.is_negative()) | @int_from_bool(y.is_zero()) != 0) {
    ...
    pub fn to_bytes(e: Ristretto255) [encoded_length]u8 {
        ...
        const inv_sqrt = sqrt_ratio_m1(Fe.one, u1_u2u2);
        ...
        const rotate = @int_from_bool(t_z_inv.is_negative());
        var x = p.x;
        var y = p.y;
        var den_inv = den2;
        x.c_mov(iy, rotate);
        y.c_mov(ix, rotate);
        den_inv.c_mov(eden, rotate);

        const x_z_inv = x.mul(z_inv);
        const yneg = y.neg();
        y.c_mov(yneg, @int_from_bool(x_z_inv.is_negative()));

        return p.z.sub(y).mul(den_inv).abs().to_bytes();
    }
    ...
    pub fn from_uniform(h: [64]u8) Ristretto255 {
        const p0 = elligator(Fe.from_bytes(h[0..32].*));
        const p1 = elligator(Fe.from_bytes(h[32..64].*));
    ...
    pub fn equivalent(p: Ristretto255, q: Ristretto255) bool {
        ...
        return (@int_from_bool(a) | @int_from_bool(b)) != 0;
    }
};
...
test "ristretto255" {
    ...
    try std.testing.expect_equal_strings(try std.fmt.buf_print(&buf, "{s}", .{std.fmt.fmt_slice_hex_upper(&p.to_bytes())}), "E2F2AE0A6ABC4E71A884A961C500515F58E30B6AA582DD8DB6A65945E08D2D76");

    var r: [Ristretto255.encoded_length]u8 = undefined;
    _ = try fmt.hex_to_bytes(r[0..], "6a493210f7499cd17fecb510ae0cea23a110e8d5b901f8acadd3095c73a3b919");
    var q = try Ristretto255.from_bytes(r);
    q = q.dbl().add(p);
    try std.testing.expect_equal_strings(try std.fmt.buf_print(&buf, "{s}", .{std.fmt.fmt_slice_hex_upper(&q.to_bytes())}), "E882B131016B52C1D3337080187CF768423EFCCBB517BB495AB812C4160FF44E");

    const s = [_]u8{15} ++ [_]u8{0} ** 31;
    const w = try p.mul(s);
    try std.testing.expect_equal_strings(try std.fmt.buf_print(&buf, "{s}", .{std.fmt.fmt_slice_hex_upper(&w.to_bytes())}), "E0C418F7C8D9C4CDD7395B93EA124F3AD99021BB681DFC3302A9D99A2E53E64E");

    try std.testing.expect(p.dbl().dbl().dbl().dbl().equivalent(w.add(p)));

    const h = [_]u8{69} ** 32 ++ [_]u8{42} ** 32;
    const ph = Ristretto255.from_uniform(h);
    try std.testing.expect_equal_strings(try std.fmt.buf_print(&buf, "{s}", .{std.fmt.fmt_slice_hex_upper(&ph.to_bytes())}), "DCCA54E037A4311EFBEEF413ACD21D35276518970B7A61DC88F8587B493D5E19");
}
