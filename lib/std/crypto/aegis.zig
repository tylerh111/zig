...
const State128L = struct {
    ...
    fn init(key: [16]u8, nonce: [16]u8) State128L {
        const c1 = AesBlock.from_bytes(&[16]u8{ 0xdb, 0x3d, 0x18, 0x55, 0x6d, 0xc2, 0x2f, 0xf1, 0x20, 0x11, 0x31, 0x42, 0x73, 0xb5, 0x28, 0xdd });
        const c2 = AesBlock.from_bytes(&[16]u8{ 0x0, 0x1, 0x01, 0x02, 0x03, 0x05, 0x08, 0x0d, 0x15, 0x22, 0x37, 0x59, 0x90, 0xe9, 0x79, 0x62 });
        const key_block = AesBlock.from_bytes(&key);
        const nonce_block = AesBlock.from_bytes(&nonce);
        const blocks = [8]AesBlock{
            key_block.xor_blocks(nonce_block),
            c1,
            c2,
            c1,
            key_block.xor_blocks(nonce_block),
            key_block.xor_blocks(c2),
            key_block.xor_blocks(c1),
            key_block.xor_blocks(c2),
        };
    ...
    inline fn update(state: *State128L, d1: AesBlock, d2: AesBlock) void {
        ...
        blocks[0] = blocks[0].xor_blocks(d1);
        blocks[4] = blocks[4].xor_blocks(d2);
    }

    fn absorb(state: *State128L, src: *const [32]u8) void {
        const msg0 = AesBlock.from_bytes(src[0..16]);
        const msg1 = AesBlock.from_bytes(src[16..32]);
    ...
    fn enc(state: *State128L, dst: *[32]u8, src: *const [32]u8) void {
        ...
        const msg0 = AesBlock.from_bytes(src[0..16]);
        const msg1 = AesBlock.from_bytes(src[16..32]);
        var tmp0 = msg0.xor_blocks(blocks[6]).xor_blocks(blocks[1]);
        var tmp1 = msg1.xor_blocks(blocks[2]).xor_blocks(blocks[5]);
        tmp0 = tmp0.xor_blocks(blocks[2].and_blocks(blocks[3]));
        tmp1 = tmp1.xor_blocks(blocks[6].and_blocks(blocks[7]));
        dst[0..16].* = tmp0.to_bytes();
        dst[16..32].* = tmp1.to_bytes();
        state.update(msg0, msg1);
    }

    fn dec(state: *State128L, dst: *[32]u8, src: *const [32]u8) void {
        const blocks = &state.blocks;
        var msg0 = AesBlock.from_bytes(src[0..16]).xor_blocks(blocks[6]).xor_blocks(blocks[1]);
        var msg1 = AesBlock.from_bytes(src[16..32]).xor_blocks(blocks[2]).xor_blocks(blocks[5]);
        msg0 = msg0.xor_blocks(blocks[2].and_blocks(blocks[3]));
        msg1 = msg1.xor_blocks(blocks[6].and_blocks(blocks[7]));
        dst[0..16].* = msg0.to_bytes();
        dst[16..32].* = msg1.to_bytes();
        state.update(msg0, msg1);
    }
    ...
    fn mac(state: *State128L, comptime tag_bits: u9, adlen: usize, mlen: usize) [tag_bits / 8]u8 {
        ...
        mem.write_int(u64, sizes[0..8], @as(u64, adlen) * 8, .little);
        mem.write_int(u64, sizes[8..16], @as(u64, mlen) * 8, .little);
        const tmp = AesBlock.from_bytes(&sizes).xor_blocks(blocks[2]);
        var i: usize = 0;
        while (i < 7) : (i += 1) {
            state.update(tmp, tmp);
        }
        return switch (tag_bits) {
            128 => blocks[0].xor_blocks(blocks[1]).xor_blocks(blocks[2]).xor_blocks(blocks[3])
                .xor_blocks(blocks[4]).xor_blocks(blocks[5]).xor_blocks(blocks[6]).to_bytes(),
            256 => tag: {
                const t1 = blocks[0].xor_blocks(blocks[1]).xor_blocks(blocks[2]).xor_blocks(blocks[3]);
                const t2 = blocks[4].xor_blocks(blocks[5]).xor_blocks(blocks[6]).xor_blocks(blocks[7]);
                break :tag t1.to_bytes() ++ t2.to_bytes();
            },
            else => unreachable,
        };
    }
    ...
    return struct {
        ...
        pub fn decrypt(m: []u8, c: []const u8, tag: [tag_length]u8, ad: []const u8, npub: [nonce_length]u8, key: [key_length]u8) AuthenticationError!void {
            ...
            if (m.len % 32 != 0) {
                ...
                const blocks = &state.blocks;
                blocks[0] = blocks[0].xor_blocks(AesBlock.from_bytes(dst[0..16]));
                blocks[4] = blocks[4].xor_blocks(AesBlock.from_bytes(dst[16..32]));
            }
...
const State256 = struct {
    ...
    fn init(key: [32]u8, nonce: [32]u8) State256 {
        const c1 = AesBlock.from_bytes(&[16]u8{ 0xdb, 0x3d, 0x18, 0x55, 0x6d, 0xc2, 0x2f, 0xf1, 0x20, 0x11, 0x31, 0x42, 0x73, 0xb5, 0x28, 0xdd });
        const c2 = AesBlock.from_bytes(&[16]u8{ 0x0, 0x1, 0x01, 0x02, 0x03, 0x05, 0x08, 0x0d, 0x15, 0x22, 0x37, 0x59, 0x90, 0xe9, 0x79, 0x62 });
        const key_block1 = AesBlock.from_bytes(key[0..16]);
        const key_block2 = AesBlock.from_bytes(key[16..32]);
        const nonce_block1 = AesBlock.from_bytes(nonce[0..16]);
        const nonce_block2 = AesBlock.from_bytes(nonce[16..32]);
        const kxn1 = key_block1.xor_blocks(nonce_block1);
        const kxn2 = key_block2.xor_blocks(nonce_block2);
        const blocks = [6]AesBlock{
            kxn1,
            kxn2,
            c1,
            c2,
            key_block1.xor_blocks(c2),
            key_block2.xor_blocks(c1),
        };
    ...
    inline fn update(state: *State256, d: AesBlock) void {
        ...
        blocks[0] = tmp.xor_blocks(d);
    }

    fn absorb(state: *State256, src: *const [16]u8) void {
        const msg = AesBlock.from_bytes(src);
        state.update(msg);
    }
    ...
    fn enc(state: *State256, dst: *[16]u8, src: *const [16]u8) void {
        const blocks = &state.blocks;
        const msg = AesBlock.from_bytes(src);
        var tmp = msg.xor_blocks(blocks[5]).xor_blocks(blocks[4]).xor_blocks(blocks[1]);
        tmp = tmp.xor_blocks(blocks[2].and_blocks(blocks[3]));
        dst.* = tmp.to_bytes();
        state.update(msg);
    }
    ...
    fn dec(state: *State256, dst: *[16]u8, src: *const [16]u8) void {
        const blocks = &state.blocks;
        var msg = AesBlock.from_bytes(src).xor_blocks(blocks[5]).xor_blocks(blocks[4]).xor_blocks(blocks[1]);
        msg = msg.xor_blocks(blocks[2].and_blocks(blocks[3]));
        dst.* = msg.to_bytes();
        state.update(msg);
    }
    ...
    fn mac(state: *State256, comptime tag_bits: u9, adlen: usize, mlen: usize) [tag_bits / 8]u8 {
        ...
        mem.write_int(u64, sizes[0..8], @as(u64, adlen) * 8, .little);
        mem.write_int(u64, sizes[8..16], @as(u64, mlen) * 8, .little);
        const tmp = AesBlock.from_bytes(&sizes).xor_blocks(blocks[3]);
        var i: usize = 0;
        while (i < 7) : (i += 1) {
            state.update(tmp);
        }
        return switch (tag_bits) {
            128 => blocks[0].xor_blocks(blocks[1]).xor_blocks(blocks[2]).xor_blocks(blocks[3])
                .xor_blocks(blocks[4]).xor_blocks(blocks[5]).to_bytes(),
            256 => tag: {
                const t1 = blocks[0].xor_blocks(blocks[1]).xor_blocks(blocks[2]);
                const t2 = blocks[3].xor_blocks(blocks[4]).xor_blocks(blocks[5]);
                break :tag t1.to_bytes() ++ t2.to_bytes();
            },
            else => unreachable,
        };
    ...
    return struct {
        ...
        pub fn decrypt(m: []u8, c: []const u8, tag: [tag_length]u8, ad: []const u8, npub: [nonce_length]u8, key: [key_length]u8) AuthenticationError!void {
            ...
            if (m.len % 16 != 0) {
                ...
                const blocks = &state.blocks;
                blocks[0] = blocks[0].xor_blocks(AesBlock.from_bytes(&dst));
            }
            var computed_tag = state.mac(tag_bits, ad.len, m.len);
            const verify = crypto.utils.timing_safe_eql([tag_length]u8, computed_tag, tag);
            if (!verify) {
...
test "Aegis128L test vector 1" {
    ...
    try testing.expect_equal_slices(u8, &m, &m2);

    try htest.assert_equal("79d94593d8c2119d7e8fd9b8fc77845c5c077a05b2528b6ac54b563aed8efe84", &c);
    try htest.assert_equal("cc6f3372f6aa1bb82388d695c3962d9a", &tag);

    c[0] +%= 1;
    try testing.expect_error(error.AuthenticationFailed, Aegis128L.decrypt(&m2, &c, tag, &ad, nonce, key));
    c[0] -%= 1;
    tag[0] +%= 1;
    try testing.expect_error(error.AuthenticationFailed, Aegis128L.decrypt(&m2, &c, tag, &ad, nonce, key));
}
...
test "Aegis128L test vector 2" {
    ...
    try htest.assert_equal("41de9000a7b5e40e2d68bb64d99ebb19", &c);
    try htest.assert_equal("f4d997cc9b94227ada4fe4165422b1c8", &tag);
}
...
test "Aegis128L test vector 3" {
    ...
    try htest.assert_equal("83cc600dc4e3e7e62d4055826174f149", &tag);
}
...
test "Aegis256 test vector 1" {
    ...
    try htest.assert_equal("f373079ed84b2709faee373584585d60accd191db310ef5d8b11833df9dec711", &c);
    try htest.assert_equal("8d86f91ee606e9ff26a01b64ccbdd91d", &tag);

    c[0] +%= 1;
    try testing.expect_error(error.AuthenticationFailed, Aegis256.decrypt(&m2, &c, tag, &ad, nonce, key));
    c[0] -%= 1;
    tag[0] +%= 1;
    try testing.expect_error(error.AuthenticationFailed, Aegis256.decrypt(&m2, &c, tag, &ad, nonce, key));
}
...
test "Aegis256 test vector 2" {
    ...
    try htest.assert_equal("b98f03a947807713d75a4fff9fc277a6", &c);
    try htest.assert_equal("478f3b50dc478ef7d5cf2d0f7cc13180", &tag);
}
...
test "Aegis256 test vector 3" {
    ...
    try htest.assert_equal("f7a0878f68bd083e8065354071fc27c3", &tag);
}
...
test "Aegis MAC" {
    ...
    st.update(msg[32..]);
    st.final(&tag);
    try htest.assert_equal("f8840849602738d81037cbaa0f584ea95759e2ac60263ce77346bcdc79fe4319", &tag);
    ...
    st.update(msg[0..33]);
    st.final(&tag);
    try htest.assert_equal("c7cf649a844c1a6676cf6d91b1658e0aee54a4da330b0a8d3bc7ea4067551d1b", &tag);
}
