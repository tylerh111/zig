...
pub const Algorithm = enum {
    ...
    pub const map = std.StaticStringMap(Algorithm).init_comptime(.{
    ...
    pub fn Hash(comptime algorithm: Algorithm) type {
        return switch (algorithm) {
            ...
            .md2WithRSAEncryption => @compile_error("unimplemented"),
...
pub const AlgorithmCategory = enum {
    ...
    pub const map = std.StaticStringMap(AlgorithmCategory).init_comptime(.{
...
pub const Attribute = enum {
    ...
    pub const map = std.StaticStringMap(Attribute).init_comptime(.{
        .{ &[_]u8{ 0x55, 0x04, 0x03 }, .common_name },
...
pub const NamedCurve = enum {
    ...
    pub const map = std.StaticStringMap(NamedCurve).init_comptime(.{
    ...
    pub fn Curve(comptime curve: NamedCurve) type {
        return switch (curve) {
            ...
            .secp521r1 => @compile_error("unimplemented"),
        };
    }
};
...
pub const ExtensionId = enum {
    ...
    pub const map = std.StaticStringMap(ExtensionId).init_comptime(.{
        .{ &[_]u8{ 0x55, 0x04, 0x03 }, .common_name },
...
pub const Parsed = struct {
    ...
    pub fn verify(parsed_subject: Parsed, parsed_issuer: Parsed, now_sec: i64) VerifyError!void {
        ...
        switch (parsed_subject.signature_algorithm) {
            ...
            .curveEd25519 => return verify_ed25519(
                parsed_subject.message(),
                parsed_subject.signature(),
                parsed_issuer.pub_key_algo,
                parsed_issuer.pub_key(),
    ...
    pub fn verify_host_name(parsed_subject: Parsed, host_name: []const u8) VerifyHostNameError!void {
        ...
        const subject_alt_name = parsed_subject.subject_alt_name();
        if (subject_alt_name.len == 0) {
            if (check_host_name(host_name, parsed_subject.common_name())) {
                return;
            } else {
        ...
        while (name_i < general_names.slice.end) {
            ...
            switch (@as(GeneralNameTag, @enumFromInt(@int_from_enum(general_name.identifier.tag)))) {
                .dNSName => {
                    const dns_name = subject_alt_name[general_name.slice.start..general_name.slice.end];
                    if (check_host_name(host_name, dns_name)) return;
                },
                else => {},
            }
        }
    ...
    fn check_host_name(host_name: []const u8, dns_name: []const u8) bool {
        ...
        var it_host = std.mem.split_scalar(u8, host_name, '.');
        var it_dns = std.mem.split_scalar(u8, dns_name, '.');
...
test "Parsed.check_host_name" {
    const expect_equal = std.testing.expect_equal;

    try expect_equal(true, Parsed.check_host_name("ziglang.org", "ziglang.org"));
    try expect_equal(true, Parsed.check_host_name("bar.ziglang.org", "*.ziglang.org"));
    try expect_equal(false, Parsed.check_host_name("foo.bar.ziglang.org", "*.ziglang.org"));
    try expect_equal(false, Parsed.check_host_name("ziglang.org", "zig*.org"));
    try expect_equal(false, Parsed.check_host_name("lang.org", "zig*.org"));
}
...
pub fn parse(cert: Certificate) ParseError!Parsed {
    ...
    const version = try parse_version(cert_bytes, version_elem);
    const serial_number = if (@as(u8, @bit_cast(version_elem.identifier)) == 0xa0)
    ...
    const not_before_utc = try parse_time(cert, not_before);
    const not_after = try der.Element.parse(cert_bytes, not_before.slice.end);
    const not_after_utc = try parse_time(cert, not_after);
    ...
    const pub_key_algo_tag = try parse_algorithm_category(cert_bytes, pub_key_algo_elem);
    ...
    switch (pub_key_algo_tag) {
        ...
        .X9_62_id_ecPublicKey => {
            ...
            const named_curve = try parse_named_curve(cert_bytes, params_elem);
    ...
    const pub_key = try parse_bit_string(cert, pub_key_elem);
    ...
    while (name_i < subject.slice.end) {
        ...
        while (rdn_i < rdn.slice.end) {
            ...
            while (atav_i < atav.slice.end) {
                ...
                const ty = parse_attribute(cert_bytes, ty_elem) catch |err| switch (err) {
                    error.CertificateHasUnrecognizedObjectId => continue,
                    else => |e| return e,
                };
                switch (ty) {
                    .common_name => common_name = val.slice,
                    else => {},
                }
            }
    ...
    const signature_algorithm = try parse_algorithm(cert_bytes, algo_elem);
    const sig_elem = try der.Element.parse(cert_bytes, sig_algo.slice.end);
    const signature = try parse_bit_string(cert, sig_elem);
    ...
    ext: {
        ...
        while (ext_i < extensions.slice.end) {
            ...
            const ext_id = parse_extension_id(cert_bytes, oid_elem) catch |err| switch (err) {
                error.CertificateHasUnrecognizedObjectId => continue,
                else => |e| return e,
            };
    ...
    switch (elem.identifier.tag) {
        .utc_time => {
            ...
            return Date.to_seconds(.{
                .year = @as(u16, 2000) + try parse_time_digits(bytes[0..2], 0, 99),
                .month = try parse_time_digits(bytes[2..4], 1, 12),
                .day = try parse_time_digits(bytes[4..6], 1, 31),
                .hour = try parse_time_digits(bytes[6..8], 0, 23),
                .minute = try parse_time_digits(bytes[8..10], 0, 59),
                .second = try parse_time_digits(bytes[10..12], 0, 59),
            });
        },
        .generalized_time => {
            // Examples:
            ...
            if (bytes.len < 15)
                return error.CertificateTimeInvalid;
            return Date.to_seconds(.{
                .year = try parse_year4(bytes[0..4]),
                .month = try parse_time_digits(bytes[4..6], 1, 12),
                .day = try parse_time_digits(bytes[6..8], 1, 31),
                .hour = try parse_time_digits(bytes[8..10], 0, 23),
                .minute = try parse_time_digits(bytes[10..12], 0, 59),
                .second = try parse_time_digits(bytes[12..14], 0, 59),
            });
        },
    ...
    pub fn to_seconds(date: Date) u64 {
...

        {
            ...
            while (year < date.year) : (year += 1) {
                const days: u64 = std.time.epoch.get_days_in_year(year);
                sec += days * std.time.epoch.secs_per_day;
            }
        }

        {
            const is_leap = std.time.epoch.is_leap_year(date.year);
            var month: u4 = 1;
            while (month < date.month) : (month += 1) {
                const days: u64 = std.time.epoch.get_days_in_month(
                    @as(std.time.epoch.YearLeapKind, @enumFromInt(@int_from_bool(is_leap))),
...
pub fn parse_time_digits(text: *const [2]u8, min: u8, max: u8) !u8 {
    ...
    } else std.fmt.parse_int(u8, text, 10) catch return error.CertificateTimeInvalid;
    if (result < min) return error.CertificateTimeInvalid;
    if (result > max) return error.CertificateTimeInvalid;
    return @truncate(result);
}

test parse_time_digits {
    const expect_equal = std.testing.expect_equal;
    try expect_equal(@as(u8, 0), try parse_time_digits("00", 0, 99));
    try expect_equal(@as(u8, 99), try parse_time_digits("99", 0, 99));
    try expect_equal(@as(u8, 42), try parse_time_digits("42", 0, 99));

    const expect_error = std.testing.expect_error;
    try expect_error(error.CertificateTimeInvalid, parse_time_digits("13", 1, 12));
    try expect_error(error.CertificateTimeInvalid, parse_time_digits("00", 1, 12));
    try expect_error(error.CertificateTimeInvalid, parse_time_digits("Di", 0, 99));
}
...
pub fn parse_year4(text: *const [4]u8) !u16 {
    ...
    } else std.fmt.parse_int(u16, text, 10) catch return error.CertificateTimeInvalid;
...
test parse_year4 {
    const expect_equal = std.testing.expect_equal;
    try expect_equal(@as(u16, 0), try parse_year4("0000"));
    try expect_equal(@as(u16, 9999), try parse_year4("9999"));
    try expect_equal(@as(u16, 1988), try parse_year4("1988"));

    const expect_error = std.testing.expect_error;
    try expect_error(error.CertificateTimeInvalid, parse_year4("999b"));
    try expect_error(error.CertificateTimeInvalid, parse_year4("crap"));
    try expect_error(error.CertificateTimeInvalid, parse_year4("r:bQ"));
}

pub fn parse_algorithm(bytes: []const u8, element: der.Element) ParseEnumError!Algorithm {
    return parse_enum(Algorithm, bytes, element);
}

pub fn parse_algorithm_category(bytes: []const u8, element: der.Element) ParseEnumError!AlgorithmCategory {
    return parse_enum(AlgorithmCategory, bytes, element);
}

pub fn parse_attribute(bytes: []const u8, element: der.Element) ParseEnumError!Attribute {
    return parse_enum(Attribute, bytes, element);
}

pub fn parse_named_curve(bytes: []const u8, element: der.Element) ParseEnumError!NamedCurve {
    return parse_enum(NamedCurve, bytes, element);
}

pub fn parse_extension_id(bytes: []const u8, element: der.Element) ParseEnumError!ExtensionId {
    return parse_enum(ExtensionId, bytes, element);
}
...
pub fn parse_version(bytes: []const u8, version_elem: der.Element) ParseVersionError!Version {
    if (@as(u8, @bit_cast(version_elem.identifier)) != 0xa0)
        return .v1;
...
fn verify_rsa(
    ...
    const pk_components = try rsa.PublicKey.parse_der(pub_key);
    ...
    switch (modulus.len) {
        inline 128, 256, 384, 512 => |modulus_len| {
            ...
            const public_key = rsa.PublicKey.from_bytes(exponent, modulus) catch return error.CertificateSignatureInvalid;
...
fn verify_ecdsa(
    ...
    switch (sig_named_curve) {
        ...
        => |curve| {
            ...
            const sig = Ecdsa.Signature.from_der(encoded_sig) catch |err| switch (err) {
                error.InvalidEncoding => return error.CertificateSignatureInvalid,
            };
            const pub_key = Ecdsa.PublicKey.from_sec1(sec1_pub_key) catch |err| switch (err) {
                error.InvalidEncoding => return error.CertificateSignatureInvalid,
...
fn verify_ed25519(
    ...
    const sig = Ed25519.Signature.from_bytes(encoded_sig[0..Ed25519.Signature.encoded_length].*);
    if (encoded_pub_key.len != Ed25519.PublicKey.encoded_length) return error.CertificateSignatureInvalid;
    const pub_key = Ed25519.PublicKey.from_bytes(encoded_pub_key[0..Ed25519.PublicKey.encoded_length].*) catch |err| switch (err) {
        error.NonCanonical => return error.CertificateSignatureInvalid,
    };
    sig.verify(message, pub_key) catch |err| switch (err) {
        error.IdentityElement => return error.CertificateSignatureInvalid,
...
pub const der = struct {
    ...
    pub const Element = struct {
        ...
        pub fn parse(bytes: []const u8, index: u32) ParseElementError!Element {
            var i = index;
            const identifier = @as(Identifier, @bit_cast(bytes[i]));
            i += 1;
...
pub const rsa = struct {
    ...
    pub const PSSSignature = struct {
        pub fn from_bytes(comptime modulus_len: usize, msg: []const u8) [modulus_len]u8 {
            ...
            std.mem.copy_forwards(u8, &result, msg);
            return result;
        }
        ...
        fn EMSA_PSS_VERIFY(msg: []const u8, em: []const u8, emBit: usize, sLen: usize, comptime Hash: type) !void {
            ...
            std.mem.copy_forwards(u8, m_p, &([_]u8{0} ** 8));
            std.mem.copy_forwards(u8, m_p[8..], &mHash);
            std.mem.copy_forwards(u8, m_p[(8 + Hash.digest_length)..], salt);
        ...
        fn MGF1(comptime Hash: type, out: []u8, seed: *const [Hash.digest_length]u8, len: usize) ![]u8 {
            ...
            while (idx < len) {
                c[0] = @as(u8, @int_cast((counter >> 24) & 0xFF));
                c[1] = @as(u8, @int_cast((counter >> 16) & 0xFF));
                c[2] = @as(u8, @int_cast((counter >> 8) & 0xFF));
                c[3] = @as(u8, @int_cast(counter & 0xFF));

                std.mem.copy_forwards(u8, hash[seed.len..], &c);
                Hash.hash(&hash, &hashed, .{});

                std.mem.copy_forwards(u8, out[idx..], &hashed);
                idx += hashed.len;
    ...
    pub const PublicKey = struct {
        ...
        pub fn from_bytes(pub_bytes: []const u8, modulus_bytes: []const u8) !PublicKey {
            ...
            const _n = Modulus.from_bytes(modulus_bytes, .big) catch return error.CertificatePublicKeyInvalid;
            ...
            const _e = Fe.from_bytes(_n, pub_bytes, .big) catch return error.CertificatePublicKeyInvalid;
            if (!_e.is_odd()) return error.CertificatePublicKeyInvalid;
            const e_v = _e.to_primitive(u32) catch return error.CertificatePublicKeyInvalid;
    ...
    fn encrypt(comptime modulus_len: usize, msg: [modulus_len]u8, public_key: PublicKey) ![modulus_len]u8 {
        const m = Fe.from_bytes(public_key.n, &msg, .big) catch return error.MessageTooLong;
        const e = public_key.n.pow_public(m, public_key.e) catch unreachable;
        var res: [modulus_len]u8 = undefined;
        e.to_bytes(&res, .big) catch unreachable;
        return res;
    }
};
