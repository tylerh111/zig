...
const hkdf_expand_label = tls.hkdf_expand_label;
const int2 = tls.int2;
const int3 = tls.int3;
...
pub fn init(stream: anytype, ca_bundle: Certificate.Bundle, host: []const u8) InitError(@TypeOf(stream))!Client {
    const host_len: u16 = @int_cast(host.len);
    ...
    const extensions_payload =
        ...
        .key_share,
        array(1, int2(@int_from_enum(tls.NamedGroup.x25519)) ++
            array(1, x25519_kp.public_key) ++
            int2(@int_from_enum(tls.NamedGroup.secp256r1)) ++
            array(1, secp256r1_kp.public_key.to_uncompressed_sec1()) ++
            int2(@int_from_enum(tls.NamedGroup.x25519_kyber768d00)) ++
            array(1, x25519_kp.public_key ++ kyber768_kp.public_key.to_bytes())),
    ) ++
        int2(@int_from_enum(tls.ExtensionType.server_name)) ++
    ...
    const extensions_header =
        int2(@int_cast(extensions_payload.len + host_len)) ++
        extensions_payload;

    const legacy_compression_methods = 0x0100;

    const client_hello =
        int2(@int_from_enum(tls.ProtocolVersion.tls_1_2)) ++
        hello_rand ++
    ...
    const out_handshake =
        [_]u8{@int_from_enum(tls.HandshakeType.client_hello)} ++
        int3(@int_cast(client_hello.len + host_len)) ++
        client_hello;

    const plaintext_header = [_]u8{
        @int_from_enum(tls.ContentType.handshake),
        0x03, 0x01, // legacy_record_version
    } ++ int2(@int_cast(out_handshake.len + host_len)) ++ out_handshake;
    ...
    {
        try d.read_at_least_our_amt(stream, tls.record_header_len);
        const ct = d.decode(tls.ContentType);
        d.skip(2); // legacy_record_version
        const record_len = d.decode(u16);
        try d.read_at_least(stream, record_len);
        ...
        switch (ct) {
            ...
            .handshake => {
                        ...
                        .key_share => {
                            ...
                            switch (named_group) {
                                ...
                                .secp256r1 => {
                                    ...
                                    const pk = PublicKey.from_sec1(server_pub_key) catch {
                                        return error.TlsDecryptFailure;
                                    };
                                    const mul = pk.p.mul_public(secp256r1_kp.secret_key.bytes, .big) catch {
                                        return error.TlsDecryptFailure;
                                    };
                                    shared_key = &mul.affine_coordinates().x.to_bytes(.big);
                                },
                                else => {
                                    return error.TlsIllegalParameter;
                                },
                ...
                if (tls_version != @int_from_enum(tls.ProtocolVersion.tls_1_3))
                    return error.TlsIllegalParameter;
                ...
                switch (cipher_suite_tag) {
                    ...
                    .AEGIS_128L_SHA256,
                    => |tag| {
                        const P = std.meta.TagPayloadByName(tls.HandshakeCipher, @tag_name(tag));
                        handshake_cipher = @union_init(tls.HandshakeCipher, @tag_name(tag), .{
                        ...
                        const p = &@field(handshake_cipher, @tag_name(tag));
                        ...
                        const empty_hash = tls.empty_hash(P.Hash);
                        const hs_derived_secret = hkdf_expand_label(P.Hkdf, early_secret, "derived", &empty_hash, P.Hash.digest_length);
                        p.handshake_secret = P.Hkdf.extract(&hs_derived_secret, shared_key);
                        const ap_derived_secret = hkdf_expand_label(P.Hkdf, p.handshake_secret, "derived", &empty_hash, P.Hash.digest_length);
                        p.master_secret = P.Hkdf.extract(&ap_derived_secret, &zeroes);
                        const client_secret = hkdf_expand_label(P.Hkdf, p.handshake_secret, "c hs traffic", &hello_hash, P.Hash.digest_length);
                        const server_secret = hkdf_expand_label(P.Hkdf, p.handshake_secret, "s hs traffic", &hello_hash, P.Hash.digest_length);
                        p.client_finished_key = hkdf_expand_label(P.Hkdf, client_secret, "finished", "", P.Hmac.key_length);
                        p.server_finished_key = hkdf_expand_label(P.Hkdf, server_secret, "finished", "", P.Hmac.key_length);
                        p.client_handshake_key = hkdf_expand_label(P.Hkdf, client_secret, "key", "", P.AEAD.key_length);
                        p.server_handshake_key = hkdf_expand_label(P.Hkdf, server_secret, "key", "", P.AEAD.key_length);
                        p.client_handshake_iv = hkdf_expand_label(P.Hkdf, client_secret, "iv", "", P.AEAD.nonce_length);
                        p.server_handshake_iv = hkdf_expand_label(P.Hkdf, server_secret, "iv", "", P.AEAD.nonce_length);
                    },
                    else => {
                        return error.TlsIllegalParameter;
                    },
    ...
    while (true) {
        ...
        switch (ct) {
            ...
            .application_data => {
                ...
                const cleartext = switch (handshake_cipher) {
                    inline else => |*p| c: {
                        ...
                        const nonce = if (builtin.zig_backend == .stage2_x86_64 and
                            P.AEAD.nonce_length > comptime std.simd.suggest_vector_length(u8) orelse 1)
                        nonce: {
                            var nonce = p.server_handshake_iv;
                            const operand = std.mem.read_int(u64, nonce[nonce.len - 8 ..], .big);
                            std.mem.write_int(u64, nonce[nonce.len - 8 ..], operand ^ read_seq, .big);
                            break :nonce nonce;
                        } else nonce: {
                            const V = @Vector(P.AEAD.nonce_length, u8);
                            const pad = [1]u8{0} ** (P.AEAD.nonce_length - 8);
                            const operand: V = pad ++ @as([8]u8, @bit_cast(big(read_seq)));
                ...
                var ctd = tls.Decoder.from_their_slice(cleartext[0 .. cleartext.len - 1]);
                while (true) {
                    try ctd.ensure(4);
                    ...
                    switch (handshake_type) {
                        ...
                        .certificate => cert: {
                            ...
                            while (!certs_decoder.eof()) {
                                ...
                                if (cert_index == 0) {
                                    ...
                                    main_cert_pub_key_len = @int_cast(pub_key.len);
                                } else {
                        ...
                        .certificate_verify => {
                            ...
                            switch (scheme) {
                                ...
                                => |comptime_scheme| {
                                    ...
                                    const sig = try Ecdsa.Signature.from_der(encoded_sig);
                                    const key = try Ecdsa.PublicKey.from_sec1(main_cert_pub_key);
                                ...
                                => |comptime_scheme| {
                                    ...
                                    const components = try rsa.PublicKey.parse_der(main_cert_pub_key);
                                    const exponent = components.exponent;
                                    const modulus = components.modulus;
                                    switch (modulus.len) {
                                        inline 128, 256, 512 => |modulus_len| {
                                            const key = try rsa.PublicKey.from_bytes(exponent, modulus);
                                            const sig = rsa.PSSSignature.from_bytes(modulus_len, encoded_sig);
                                ...
                                inline .ed25519 => |comptime_scheme| {
                                    ...
                                    const sig = Eddsa.Signature.from_bytes(encoded_sig[0..Eddsa.Signature.encoded_length].*);
                                    if (main_cert_pub_key.len != Eddsa.PublicKey.encoded_length) return error.InvalidEncoding;
                                    const key = try Eddsa.PublicKey.from_bytes(main_cert_pub_key[0..Eddsa.PublicKey.encoded_length].*);
                                    try sig.verify(verify_bytes, key);
                                },
                                else => {
                                    return error.TlsBadSignatureScheme;
                        ...
                        .finished => {
                            ...
                            const client_change_cipher_spec_msg = [_]u8{
                                @int_from_enum(tls.ContentType.change_cipher_spec),
                            ...
                            const app_cipher = switch (handshake_cipher) {
                                inline else => |*p, tag| c: {
                                    ...
                                    try stream.writev_all(&both_msgs_vec);

                                    const client_secret = hkdf_expand_label(P.Hkdf, p.master_secret, "c ap traffic", &handshake_hash, P.Hash.digest_length);
                                    const server_secret = hkdf_expand_label(P.Hkdf, p.master_secret, "s ap traffic", &handshake_hash, P.Hash.digest_length);
                                    break :c @union_init(tls.ApplicationCipher, @tag_name(tag), .{
                                        .client_secret = client_secret,
                                        .server_secret = server_secret,
                                        .client_key = hkdf_expand_label(P.Hkdf, client_secret, "key", "", P.AEAD.key_length),
                                        .server_key = hkdf_expand_label(P.Hkdf, server_secret, "key", "", P.AEAD.key_length),
                                        .client_iv = hkdf_expand_label(P.Hkdf, client_secret, "iv", "", P.AEAD.nonce_length),
                                        .server_iv = hkdf_expand_label(P.Hkdf, server_secret, "iv", "", P.AEAD.nonce_length),
                                    });
                                },
                            };
                            ...
                            var client: Client = .{
                                ...
                                .partial_ciphertext_end = @int_cast(leftover.len),
...
pub fn write(c: *Client, stream: anytype, bytes: []const u8) !usize {
    return write_end(c, stream, bytes, false);
}
...
pub fn write_all_end(c: *Client, stream: anytype, bytes: []const u8, end: bool) !void {
    var index: usize = 0;
    while (index < bytes.len) {
        index += try c.write_end(stream, bytes[index..], end);
    }
}
...
pub fn write_end(c: *Client, stream: anytype, bytes: []const u8, end: bool) !usize {
    ...
    var prepared = prepare_ciphertext_record(c, &iovecs_buf, &ciphertext_buf, bytes, .application_data);
    if (end) {
        prepared.iovec_end += prepare_ciphertext_record(
            c,
...
fn prepare_ciphertext_record(
    ...
    switch (c.application_cipher) {
        inline else => |*p| {
            ...
            while (true) {
                const encrypted_content_len: u16 = @int_cast(@min(
                ...
                cleartext_buf[encrypted_content_len] = @int_from_enum(inner_content_type);
                ...
                ad.* =
                    [_]u8{@int_from_enum(tls.ContentType.application_data)} ++
                    int2(@int_from_enum(tls.ProtocolVersion.tls_1_2)) ++
                ...
                nonce: {
                    ...
                    const operand = std.mem.read_int(u64, nonce[nonce.len - 8 ..], .big);
                    std.mem.write_int(u64, nonce[nonce.len - 8 ..], operand ^ c.write_seq, .big);
                    break :nonce nonce;
                } else nonce: {
                    const V = @Vector(P.AEAD.nonce_length, u8);
                    const pad = [1]u8{0} ** (P.AEAD.nonce_length - 8);
                    const operand: V = pad ++ @as([8]u8, @bit_cast(big(c.write_seq)));
...
pub fn read_at_least(c: *Client, stream: anytype, buffer: []u8, len: usize) !usize {
    ...
    return readv_at_least(c, stream, &iovecs, len);
}

/// Receives TLS-encrypted data from `stream`, which must conform to `StreamInterface`.
pub fn read(c: *Client, stream: anytype, buffer: []u8) !usize {
    return read_at_least(c, stream, buffer, 1);
}
...
pub fn read_all(c: *Client, stream: anytype, buffer: []u8) !usize {
    return read_at_least(c, stream, buffer, buffer.len);
}
...
pub fn readv(c: *Client, stream: anytype, iovecs: []std.posix.iovec) !usize {
    return readv_at_least(c, stream, iovecs, 1);
}
...
pub fn readv_at_least(c: *Client, stream: anytype, iovecs: []std.posix.iovec, len: usize) !usize {
    ...
    var vec_i: usize = 0;
    while (true) {
        var amt = try c.readv_advanced(stream, iovecs[vec_i..]);
        off_i += amt;
...
pub fn readv_advanced(c: *Client, stream: anytype, iovecs: []const std.posix.iovec) !usize {
    ...
    if (partial_cleartext.len > 0) {
        const amt: u15 = @int_cast(vp.put(partial_cleartext));
    ...
    limited_overlap_copy(c.partially_read_buffer[0..c.partial_ciphertext_end], c.partial_ciphertext_idx);
    c.partial_ciphertext_end -= c.partial_ciphertext_idx;
    c.partial_ciphertext_idx = 0;
    c.partial_cleartext_idx = 0;
    ...
    const ask_iovecs = limit_vecs(&ask_iovecs_buf, ask_len);
    ...
    while (true) {
        ...
        if (in + tls.record_header_len > frag.len) {
            if (frag.ptr == frag1.ptr)
                return finish_read(c, frag, in, vp.total);

            const first = frag[in..];

            if (frag1.len < tls.record_header_len)
                return finish_read2(c, first, frag1, vp.total);

            // A record straddles the two fragments. Copy into the now-empty first fragment.
            const record_len_byte_0: u16 = straddle_byte(frag, frag1, in + 3);
            const record_len_byte_1: u16 = straddle_byte(frag, frag1, in + 4);
        ...
        in += 1;
        const legacy_version = mem.read_int(u16, frag[in..][0..2], .big);
        in += 2;
        _ = legacy_version;
        const record_len = mem.read_int(u16, frag[in..][0..2], .big);
        if (record_len > max_ciphertext_len) return error.TlsRecordOverflow;
        in += 2;
                ...
                const cleartext = switch (c.application_cipher) {
                    inline else => |*p| c: {
                        ...
                        nonce: {
                            ...
                            const operand = std.mem.read_int(u64, nonce[nonce.len - 8 ..], .big);
                            std.mem.write_int(u64, nonce[nonce.len - 8 ..], operand ^ c.read_seq, .big);
                            break :nonce nonce;
                        } else nonce: {
                            const V = @Vector(P.AEAD.nonce_length, u8);
                            const pad = [1]u8{0} ** (P.AEAD.nonce_length - 8);
                            const operand: V = pad ++ @as([8]u8, @bit_cast(big(c.read_seq)));
                ...
                switch (inner_ct) {
                    ...
                    .handshake => {
                        ...
                        while (true) {
                            ...
                            ct_i += 1;
                            const handshake_len = mem.read_int(u24, cleartext[ct_i..][0..3], .big);
                            ct_i += 3;
                            ...
                            switch (handshake_type) {
                                ...
                                .key_update => {
                                    switch (c.application_cipher) {
                                        inline else => |*p| {
                                            const P = @TypeOf(p.*);
                                            const server_secret = hkdf_expand_label(P.Hkdf, p.server_secret, "traffic upd", "", P.Hash.digest_length);
                                            p.server_secret = server_secret;
                                            p.server_key = hkdf_expand_label(P.Hkdf, server_secret, "key", "", P.AEAD.key_length);
                                            p.server_iv = hkdf_expand_label(P.Hkdf, server_secret, "iv", "", P.AEAD.nonce_length);
                                        },
                                    }
                                    c.read_seq = 0;
                                    ...
                                    switch (@as(tls.KeyUpdateRequest, @enumFromInt(handshake[0]))) {
                                        .update_requested => {
                                            switch (c.application_cipher) {
                                                inline else => |*p| {
                                                    const P = @TypeOf(p.*);
                                                    const client_secret = hkdf_expand_label(P.Hkdf, p.client_secret, "traffic upd", "", P.Hash.digest_length);
                                                    p.client_secret = client_secret;
                                                    p.client_key = hkdf_expand_label(P.Hkdf, client_secret, "key", "", P.AEAD.key_length);
                                                    p.client_iv = hkdf_expand_label(P.Hkdf, client_secret, "iv", "", P.AEAD.nonce_length);
                                                },
                                            }
                                            c.write_seq = 0;
                                        },
                    ...
                    .application_data => {
                        ...
                        if (cleartext.ptr == &cleartext_stack_buffer) {
                            ...
                            if (c.partial_ciphertext_idx > c.partial_cleartext_idx) {
                                ...
                                c.partial_ciphertext_idx = @int_cast(c.partial_ciphertext_idx + msg.len);
                            } else {
                                const amt = vp.put(msg);
                                if (amt < msg.len) {
                                    const rest = msg[amt..];
                                    c.partial_cleartext_idx = 0;
                                    c.partial_ciphertext_idx = @int_cast(rest.len);
...
fn finish_read(c: *Client, frag: []const u8, in: usize, out: usize) usize {
    ...
    if (c.partial_ciphertext_idx > c.partial_cleartext_idx) {
        ...
        c.partial_ciphertext_end = @int_cast(c.partial_ciphertext_idx + saved_buf.len);
        @memcpy(c.partially_read_buffer[c.partial_ciphertext_idx..][0..saved_buf.len], saved_buf);
    } else {
        c.partial_cleartext_idx = 0;
        c.partial_ciphertext_idx = 0;
        c.partial_ciphertext_end = @int_cast(saved_buf.len);
...
fn finish_read2(c: *Client, first: []const u8, frag1: []const u8, out: usize) usize {
    if (c.partial_ciphertext_idx > c.partial_cleartext_idx) {
        // There is cleartext at the beginning already which we need to preserve.
        c.partial_ciphertext_end = @int_cast(c.partial_ciphertext_idx + first.len + frag1.len);
        // TODO: eliminate this call to copy_forwards
        std.mem.copy_forwards(u8, c.partially_read_buffer[c.partial_ciphertext_idx..][0..first.len], first);
        @memcpy(c.partially_read_buffer[c.partial_ciphertext_idx + first.len ..][0..frag1.len], frag1);
    } else {
        c.partial_cleartext_idx = 0;
        c.partial_ciphertext_idx = 0;
        c.partial_ciphertext_end = @int_cast(first.len + frag1.len);
        // TODO: eliminate this call to copy_forwards
        std.mem.copy_forwards(u8, c.partially_read_buffer[0..first.len], first);
...
fn limited_overlap_copy(frag: []u8, in: usize) void {
    ...
    if (first.len <= in) {
        ...
        std.mem.copy_forwards(u8, frag, first);
    }
}
