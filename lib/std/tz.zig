...
pub const Timetype = struct {
    ...
    pub fn name(self: *const Timetype) [:0]const u8 {
        return std.mem.slice_to(self.name_data[0..], 0);
    }
...
pub const Tz = struct {
    ...
    pub fn parse(allocator: std.mem.Allocator, reader: anytype) !Tz {
        var legacy_header = try reader.read_struct(Header);
        ...
        if (builtin.target.cpu.arch.endian() != std.builtin.Endian.big) {
            std.mem.byte_swap_all_fields(@TypeOf(legacy_header.counts), &legacy_header.counts);
        }

        if (legacy_header.version == 0) {
            return parse_block(allocator, reader, legacy_header, true);
        } else {
            ...
            try reader.skip_bytes(skipv, .{});

            var header = try reader.read_struct(Header);
            ...
            if (builtin.target.cpu.arch.endian() != std.builtin.Endian.big) {
                std.mem.byte_swap_all_fields(@TypeOf(header.counts), &header.counts);
            }

            return parse_block(allocator, reader, header, false);
        }
    }
    ...
    fn parse_block(allocator: std.mem.Allocator, reader: anytype, header: Header, legacy: bool) !Tz {
        ...
        while (i < header.counts.timecnt) : (i += 1) {
            transitions[i].ts = if (legacy) try reader.read_int(i32, .big) else try reader.read_int(i64, .big);
        }

        i = 0;
        while (i < header.counts.timecnt) : (i += 1) {
            const tt = try reader.read_byte();
        ...
        try reader.read_no_eof(designators_data[0..header.counts.charcnt]);
        ...
        for (timetypes) |*tt| {
            const name = std.mem.slice_to(designators[tt.name_data[0]..], 0);
        ...
        while (i < header.counts.leapcnt) : (i += 1) {
            const occur: i64 = if (legacy) try reader.read_int(i32, .big) else try reader.read_int(i64, .big);
            ...
            if (occur > std.math.max_int(i48)) return error.Malformed; // Unreasonably far into the future

            const corr = try reader.read_int(i32, .big);
            ...
            if (corr > std.math.max_int(i16)) return error.Malformed; // Unreasonably large correction

            leapseconds[i] = .{
                .occurrence = @as(i48, @int_cast(occur)),
                .correction = @as(i16, @int_cast(corr)),
            };
        }
        ...
        while (i < header.counts.isstdcnt) : (i += 1) {
            const stdtime = try reader.read_byte();
            if (stdtime == 1) {
        ...
        while (i < header.counts.isutcnt) : (i += 1) {
            const ut = try reader.read_byte();
            if (ut == 1) {
                timetypes[i].flags |= 0x04;
                if (!timetypes[i].standard_time_indicator()) return error.Malformed; // rfc8536: standard/wall value MUST be one (1) if the UT/local value is one (1)
            }
        }
        ...
        if (!legacy) {
            ...
            const footer_mem = reader.read_until_delimiter(&footerdata_buf, '\n') catch |err| switch (err) {
...
test "slim" {
    const data = @embed_file("tz/asia_tokyo.tzif");
    var in_stream = std.io.fixed_buffer_stream(data);

    var tz = try std.Tz.parse(std.testing.allocator, in_stream.reader());
    defer tz.deinit();

    try std.testing.expect_equal(tz.transitions.len, 9);
    try std.testing.expect(std.mem.eql(u8, tz.transitions[3].timetype.name(), "JDT"));
    try std.testing.expect_equal(tz.transitions[5].ts, -620298000); // 1950-05-06 15:00:00 UTC
    try std.testing.expect_equal(tz.leapseconds[13].occurrence, 567993613); // 1988-01-01 00:00:00 UTC (+23s in TAI, and +13 in the data since it doesn't store the initial 10 second offset)
}

test "fat" {
    const data = @embed_file("tz/antarctica_davis.tzif");
    var in_stream = std.io.fixed_buffer_stream(data);

    var tz = try std.Tz.parse(std.testing.allocator, in_stream.reader());
    defer tz.deinit();

    try std.testing.expect_equal(tz.transitions.len, 8);
    try std.testing.expect(std.mem.eql(u8, tz.transitions[3].timetype.name(), "+05"));
    try std.testing.expect_equal(tz.transitions[4].ts, 1268251224); // 2010-03-10 20:00:00 UTC
}

test "legacy" {
    // Taken from Slackware 8.0, from 2001
    const data = @embed_file("tz/europe_vatican.tzif");
    var in_stream = std.io.fixed_buffer_stream(data);

    var tz = try std.Tz.parse(std.testing.allocator, in_stream.reader());
    defer tz.deinit();

    try std.testing.expect_equal(tz.transitions.len, 170);
    try std.testing.expect(std.mem.eql(u8, tz.transitions[69].timetype.name(), "CET"));
    try std.testing.expect_equal(tz.transitions[123].ts, 1414285200); // 2014-10-26 01:00:00 UTC
}
