...
pub fn deinit(elf: *Elf) void {
    ...
    {
        var it = elf.sections.value_iterator();
        while (it.next()) |sect| {
            sect.*.data.deinit();
...
pub fn get_section(elf: *Elf, section_kind: Object.Section) !*std.ArrayList(u8) {
    const section_name = section_string(section_kind);
    const section = elf.sections.get(section_name) orelse blk: {
        ...
        try elf.sections.put_no_clobber(elf.arena.child_allocator, section_name, section);
...
pub fn declare_symbol(
    ...
    const name = if (maybe_name) |some| some else blk: {
        ...
        break :blk try std.fmt.alloc_print(elf.arena.allocator(), ".L.{d}", .{elf.unnamed_symbol_mangle});
    };

    const gop = if (linkage == .Internal)
        try elf.local_symbols.get_or_put(elf.arena.child_allocator, name)
    else
        try elf.global_symbols.get_or_put(elf.arena.child_allocator, name);
...
pub fn finish(elf: *Elf, file: std.fs.File) !void {
    var buf_writer = std.io.buffered_writer(file.writer());
    ...
    var sections_len: std.elf.Elf64_Off = 0;
    {
        ...
        while (it.next()) |sect| {
            ...
            relocations_len += sect.*.relocations.items.len * @size_of(std.elf.Elf64_Rela);
            sect.*.index = num_sections;
            num_sections += 1;
            num_sections += @int_from_bool(sect.*.relocations.items.len != 0);
        }
    }
    const symtab_len = (elf.local_symbols.count() + elf.global_symbols.count() + 1) * @size_of(std.elf.Elf64_Sym);

    const symtab_offset = @size_of(std.elf.Elf64_Ehdr) + sections_len;
    const symtab_offset_aligned = std.mem.align_forward(u64, symtab_offset, 8);
    ...
    const sh_offset_aligned = std.mem.align_forward(u64, sh_offset, 16);
    ...
    const elf_header = std.elf.Elf64_Ehdr{
        ...
        .e_machine = elf.obj.target.cpu.arch.to_elf_machine(),
        .e_version = 1,
        .e_entry = 0, // linker will handle this
        .e_phoff = 0, // no program header
        .e_shoff = sh_offset_aligned, // section headers offset
        .e_flags = 0, // no flags
        .e_ehsize = @size_of(std.elf.Elf64_Ehdr),
        .e_phentsize = 0, // no program header
        .e_phnum = 0, // no program header
        .e_shentsize = @size_of(std.elf.Elf64_Shdr),
        .e_shnum = num_sections,
        .e_shstrndx = strtab_index,
    };
    try w.write_struct(elf_header);

    // write contents of sections
    {
        var it = elf.sections.value_iterator();
        while (it.next()) |sect| try w.write_all(sect.*.data.items);
    }

    // pad to 8 bytes
    try w.write_byte_ntimes(0, @int_cast(symtab_offset_aligned - symtab_offset));

    var name_offset: u32 = strtab_default.len;
    // write symbols
    {
        // first symbol must be null
        try w.write_struct(std.mem.zeroes(std.elf.Elf64_Sym));

        var sym_index: u16 = 1;
        var it = elf.local_symbols.iterator();
        while (it.next()) |entry| {
            const sym = entry.value_ptr.*;
            try w.write_struct(std.elf.Elf64_Sym{
                .st_name = name_offset,
                .st_info = sym.info,
                .st_other = 0,
            ...
            sym.index = sym_index;
            sym_index += 1;
            name_offset += @int_cast(entry.key_ptr.len + 1); // +1 for null byte
        }
    ...
    // write relocations
    {
        ...
        while (it.next()) |sect| {
            for (sect.*.relocations.items) |rela| {
                try w.write_struct(std.elf.Elf64_Rela{
    ...
    // write strtab
    try w.write_all(strtab_default);
    {
        var it = elf.local_symbols.key_iterator();
        while (it.next()) |key| try w.print("{s}\x00", .{key.*});
        it = elf.global_symbols.key_iterator();
        while (it.next()) |key| try w.print("{s}\x00", .{key.*});
    }
    {
        var it = elf.sections.iterator();
        while (it.next()) |entry| {
            if (entry.value_ptr.*.relocations.items.len != 0) try w.write_all(".rela");
            try w.print(".{s}\x00", .{entry.key_ptr.*});
        }
    }

    // pad to 16 bytes
    try w.write_byte_ntimes(0, @int_cast(sh_offset_aligned - sh_offset));
    // mandatory null header
    try w.write_struct(std.mem.zeroes(std.elf.Elf64_Shdr));
    ...
    // write strtab section header
    {
        ...
        try w.write_struct(sect_header);
    }
    ...
    // write symtab section header
    {
        const sect_header = std.elf.Elf64_Shdr{
            ...
            .sh_addralign = 8,
            .sh_entsize = @size_of(std.elf.Elf64_Sym),
        };
        try w.write_struct(sect_header);
    }
    ...
    {
        var sect_offset: u64 = @size_of(std.elf.Elf64_Ehdr);
        ...
        while (it.next()) |entry| {
            ...
            try w.write_struct(std.elf.Elf64_Shdr{
            ...
            if (rela_count != 0) {
                const size = rela_count * @size_of(std.elf.Elf64_Rela);
                try w.write_struct(std.elf.Elf64_Shdr{
                    .sh_name = name_offset,
                    ...
                    .sh_size = rela_count * @size_of(std.elf.Elf64_Rela),
                    .sh_link = symtab_index,
                    .sh_info = sect.index,
                    .sh_addralign = 8,
                    .sh_entsize = @size_of(std.elf.Elf64_Rela),
                });
                rela_sect_offset += size;
            }

            sect_offset += sect.data.items.len;
            name_offset += @as(u32, @int_cast(entry.key_ptr.len + ".\x00".len)) + rela_name_offset;
        }
    }
