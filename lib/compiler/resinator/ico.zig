...
pub fn read(allocator: std.mem.Allocator, reader: anytype, max_size: u64) ReadError!IconDir {
    ...
    if (empty_reader_errorset) {
        return read_any_error(allocator, reader, max_size) catch |err| switch (err) {
...
pub fn read_any_error(allocator: std.mem.Allocator, reader: anytype, max_size: u64) !IconDir {
    const reserved = try reader.read_int(u16, .little);
    if (reserved != 0) {
        return error.InvalidHeader;
    }

    const image_type = reader.read_enum(ImageType, .little) catch |err| switch (err) {
        error.InvalidValue => return error.InvalidImageType,
        else => |e| return e,
    };

    const num_images = try reader.read_int(u16, .little);
    ...
    var entries = try std.ArrayList(Entry).init_capacity(allocator, initial_capacity);
    errdefer entries.deinit();

    var i: usize = 0;
    while (i < num_images) : (i += 1) {
        var entry: Entry = undefined;
        entry.width = try reader.read_byte();
        entry.height = try reader.read_byte();
        entry.num_colors = try reader.read_byte();
        entry.reserved = try reader.read_byte();
        switch (image_type) {
            .icon => {
                entry.type_specific_data = .{ .icon = .{
                    .color_planes = try reader.read_int(u16, .little),
                    .bits_per_pixel = try reader.read_int(u16, .little),
                } };
            },
            .cursor => {
                entry.type_specific_data = .{ .cursor = .{
                    .hotspot_x = try reader.read_int(u16, .little),
                    .hotspot_y = try reader.read_int(u16, .little),
                } };
            },
        }
        entry.data_size_in_bytes = try reader.read_int(u32, .little);
        entry.data_offset_from_start_of_file = try reader.read_int(u32, .little);
    ...
    return .{
        ...
        .entries = try entries.to_owned_slice(),
        .allocator = allocator,
    };
}
...
pub const IconDir = struct {
    ...
    pub fn get_res_data_size(self: IconDir) u32 {
        // max_int(u16) * Entry.res_byte_len = 917,490 which is well within the u32 range.
        // Note: self.entries.len is limited to max_int(u16)
        return @int_cast(IconDir.res_header_byte_len + self.entries.len * Entry.res_byte_len);
    }

    pub fn write_res_data(self: IconDir, writer: anytype, first_image_id: u16) !void {
        try writer.write_int(u16, 0, .little);
        try writer.write_int(u16, @int_from_enum(self.image_type), .little);
        // We know that entries.len must fit into a u16
        try writer.write_int(u16, @as(u16, @int_cast(self.entries.len)), .little);

        var image_id = first_image_id;
        for (self.entries) |entry| {
            try entry.write_res_data(writer, image_id);
            image_id += 1;
        }
    }
};
...
pub const Entry = struct {
    ...
    pub fn write_res_data(self: Entry, writer: anytype, id: u16) !void {
        switch (self.type_specific_data) {
            .icon => |icon_data| {
                try writer.write_int(u8, @as(u8, @truncate(self.width)), .little);
                try writer.write_int(u8, @as(u8, @truncate(self.height)), .little);
                try writer.write_int(u8, self.num_colors, .little);
                try writer.write_int(u8, self.reserved, .little);
                try writer.write_int(u16, icon_data.color_planes, .little);
                try writer.write_int(u16, icon_data.bits_per_pixel, .little);
                try writer.write_int(u32, self.data_size_in_bytes, .little);
            },
            .cursor => |cursor_data| {
                try writer.write_int(u16, self.width, .little);
                try writer.write_int(u16, self.height, .little);
                try writer.write_int(u16, cursor_data.hotspot_x, .little);
                try writer.write_int(u16, cursor_data.hotspot_y, .little);
                try writer.write_int(u32, self.data_size_in_bytes + 4, .little);
            },
        }
        try writer.write_int(u16, id, .little);
    }
};
...
test "icon" {
    ...
    try std.testing.expect_equal(ImageType.icon, icon.image_type);
    try std.testing.expect_equal(@as(usize, 1), icon.entries.len);
}
...
test "icon too many images" {
    ...
    var fbs = std.io.fixed_buffer_stream(data);
    try std.testing.expect_error(error.ImpossibleDataSize, read(std.testing.allocator, fbs.reader(), data.len));
}
...
pub const ImageFormat = enum(u2) {
    ...
    const riff_header = std.mem.read_int(u32, "RIFF", native_endian);
    const png_signature = std.mem.read_int(u64, "\x89PNG\r\n\x1a\n", native_endian);
    const ihdr_code = std.mem.read_int(u32, "IHDR", native_endian);
    const acon_form_type = std.mem.read_int(u32, "ACON", native_endian);

    pub fn detect(header_bytes: *const [16]u8) ImageFormat {
        if (std.mem.read_int(u32, header_bytes[0..4], native_endian) == riff_header) return .riff;
        if (std.mem.read_int(u64, header_bytes[0..8], native_endian) == png_signature) return .png;
        return .dib;
    }

    pub fn validate(format: ImageFormat, header_bytes: *const [16]u8) bool {
        return switch (format) {
            .png => std.mem.read_int(u32, header_bytes[12..16], native_endian) == ihdr_code,
            .riff => std.mem.read_int(u32, header_bytes[8..12], native_endian) == acon_form_type,
            .dib => true,
        };
    }
};
