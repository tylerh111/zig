...
pub fn find_transformations(
    ...
    transformations.clear_retaining_capacity();
    ...
    try walk_members(&walk, walk.ast.root_decls());

    const unreferenced_globals = walk.unreferenced_globals.values();
    try transformations.ensure_unused_capacity(unreferenced_globals.len);
    for (unreferenced_globals) |node| {
        transformations.append_assume_capacity(.{ .delete_node = node });
    }
}
...
fn walk_members(w: *Walk, members: []const Ast.Node.Index) Error!void {
    ...
    try scan_decls(w, members, .add);

    for (members) |member| {
        try walk_member(w, member);
    }

    try scan_decls(w, members, .remove);
}
...
fn scan_decls(w: *Walk, members: []const Ast.Node.Index, action: ScanDeclsAction) Error!void {
    ...
    for (members) |member_node| {
        ...
        const name_bytes = ast.token_slice(name_token);

        switch (action) {
            .add => {
                try w.unreferenced_globals.put(gpa, name_bytes, member_node);

                const gop = try w.in_scope_names.get_or_put(gpa, name_bytes);
                if (!gop.found_existing) gop.value_ptr.* = 0;
                gop.value_ptr.* += 1;
            },
            .remove => {
                const entry = w.in_scope_names.get_entry(name_bytes).?;
                if (entry.value_ptr.* <= 1) {
                    assert(w.in_scope_names.swap_remove(name_bytes));
                } else {
                    entry.value_ptr.* -= 1;
                }
            },
...
fn walk_member(w: *Walk, decl: Ast.Node.Index) Error!void {
    ...
    switch (ast.nodes.items(.tag)[decl]) {
        .fn_decl => {
            ...
            try walk_expression(w, fn_proto);
            const body_node = datas[decl].rhs;
            if (!is_fn_body_gutted(ast, body_node)) {
                w.replace_names.clear_retaining_capacity();
                try w.transformations.append(.{ .gut_function = decl });
                try walk_expression(w, body_node);
            }
        },
        .fn_proto_simple,
        .fn_proto_multi,
        ...
        .simple_var_decl,
        .aligned_var_decl,
        => try walk_global_var_decl(w, decl, ast.full_var_decl(decl).?),

        .test_decl => {
            try w.transformations.append(.{ .delete_node = decl });
            try walk_expression(w, datas[decl].rhs);
        },
        ...
        => {
            try w.transformations.append(.{ .delete_node = decl });
            try walk_container_field(w, ast.full_container_field(decl).?);
        },
...
fn walk_expression(w: *Walk, node: Ast.Node.Index) Error!void {
    ...
    switch (node_tags[node]) {
        .identifier => {
            ...
            const name_bytes = ast.token_slice(name_ident);
            _ = w.unreferenced_globals.swap_remove(name_bytes);
        ...
        => {
            ...
            if (datas[node].lhs == 0) {
                return walk_block(w, node, statements[0..0]);
            } else if (datas[node].rhs == 0) {
                return walk_block(w, node, statements[0..1]);
            } else {
                return walk_block(w, node, statements[0..2]);
            }
        },
        .block,
        .block_semicolon,
        => {
            ...
            return walk_block(w, node, statements);
        },

        .@"errdefer" => {
            const expr = datas[node].rhs;
            return walk_expression(w, expr);
        },
        ...
        .@"comptime", .@"nosuspend" => {
            ...
            return walk_expression(w, block);
        },

        .@"suspend" => {
            const body = datas[node].lhs;
            return walk_expression(w, body);
        },

        .@"catch" => {
            try walk_expression(w, datas[node].lhs); // target
            try walk_expression(w, datas[node].rhs); // fallback
        },

        .field_access => {
            const field_access = datas[node];
            try walk_expression(w, field_access.lhs);
        },
        ...
        => {
            ...
            try walk_expression(w, infix.lhs);
            return walk_expression(w, infix.rhs);
        },
        .for_range => {
            const infix = datas[node];
            try walk_expression(w, infix.lhs);
        ...
        => {
            const infix = datas[node];
            try walk_expression(w, infix.lhs);
            try walk_expression(w, infix.rhs);
        },

        .assign_destructure => {
            const full = ast.assign_destructure(node);
            for (full.ast.variables) |variable_node| {
                switch (node_tags[variable_node]) {
                    ...
                    .simple_var_decl,
                    .aligned_var_decl,
                    => try walk_local_var_decl(w, ast.full_var_decl(variable_node).?),

                    else => try walk_expression(w, variable_node),
                }
            }
            return walk_expression(w, full.ast.value_expr);
        },
        ...
        .address_of,
        => {
            return walk_expression(w, datas[node].lhs);
        },
        ...
        => {
            ...
            return walk_array_init(w, ast.full_array_init(&elements, node).?);
        },
        ...
        => {
            ...
            return walk_struct_init(w, node, ast.full_struct_init(&buf, node).?);
        },
        ...
        => {
            ...
            return walk_call(w, ast.full_call(&buf, node).?);
        },

        .array_access => {
            const suffix = datas[node];
            try walk_expression(w, suffix.lhs);
            try walk_expression(w, suffix.rhs);
        },

        .slice_open, .slice, .slice_sentinel => return walk_slice(w, node, ast.full_slice(node).?),

        .deref => {
            try walk_expression(w, datas[node].lhs);
        },
        ...
        .@"break" => {
            ...
            if (label_token == 0 and target == 0) {
                ...
                try walk_expression(w, target);
            } else if (label_token != 0 and target == 0) {
                try walk_identifier(w, label_token);
            } else if (label_token != 0 and target != 0) {
                try walk_expression(w, target);
            }
        },
        ...
        .@"continue" => {
            ...
            if (label != 0) {
                return walk_identifier(w, label); // label
            }
        },
        ...
        => {
            var buf: [2]Ast.Node.Index = undefined;
            return walk_container_decl(w, node, ast.full_container_decl(&buf, node).?);
        },
        ...
        .error_set_decl => {
            ...
            while (i < rbrace) : (i += 1) {
                switch (token_tags[i]) {
                    ...
                    .identifier => try walk_identifier(w, i),
                    .comma => {},
                    else => unreachable,
                }
            }
        ...
        .builtin_call_two, .builtin_call_two_comma => {
            if (datas[node].lhs == 0) {
                return walk_builtin_call(w, node, &.{});
            } else if (datas[node].rhs == 0) {
                return walk_builtin_call(w, node, &.{datas[node].lhs});
            } else {
                return walk_builtin_call(w, node, &.{ datas[node].lhs, datas[node].rhs });
            }
        },
        .builtin_call, .builtin_call_comma => {
            const params = ast.extra_data[datas[node].lhs..datas[node].rhs];
            return walk_builtin_call(w, node, params);
        },
        ...
        => {
            var buf: [1]Ast.Node.Index = undefined;
            return walk_fn_proto(w, ast.full_fn_proto(&buf, node).?);
        },

        .anyframe_type => {
            if (datas[node].rhs != 0) {
                return walk_expression(w, datas[node].rhs);
            }
        },
        ...
        => {
            ...
            const extra = ast.extra_data(datas[node].rhs, Ast.Node.SubRange);
            const cases = ast.extra_data[extra.start..extra.end];

            try walk_expression(w, condition); // condition expression
            try walk_expressions(w, cases);
        },
        ...
        .switch_case,
        .switch_case_inline,
        => return walk_switch_case(w, ast.full_switch_case(node).?),

        .while_simple,
        .while_cont,
        .@"while",
        => return walk_while(w, node, ast.full_while(node).?),

        .for_simple,
        .@"for",
        => return walk_for(w, ast.full_for(node).?),

        .if_simple,
        .@"if",
        => return walk_if(w, node, ast.full_if(node).?),

        .asm_simple,
        .@"asm",
        => return walk_asm(w, ast.full_asm(node).?),

        .enum_literal => {
            return walk_identifier(w, main_tokens[node]); // name
        },
...
fn walk_global_var_decl(w: *Walk, decl_node: Ast.Node.Index, var_decl: Ast.full.VarDecl) Error!void {
    ...
    if (var_decl.ast.type_node != 0) {
        try walk_expression(w, var_decl.ast.type_node);
    }

    if (var_decl.ast.align_node != 0) {
        try walk_expression(w, var_decl.ast.align_node);
    }

    if (var_decl.ast.addrspace_node != 0) {
        try walk_expression(w, var_decl.ast.addrspace_node);
    }

    if (var_decl.ast.section_node != 0) {
        try walk_expression(w, var_decl.ast.section_node);
    }

    if (var_decl.ast.init_node != 0) {
        if (!is_undefined_ident(w.ast, var_decl.ast.init_node)) {
            try w.transformations.append(.{ .replace_with_undef = var_decl.ast.init_node });
        }
        try walk_expression(w, var_decl.ast.init_node);
    }
}

fn walk_local_var_decl(w: *Walk, var_decl: Ast.full.VarDecl) Error!void {
    try walk_identifier_new(w, var_decl.ast.mut_token + 1); // name
...
fn walk_container_field(w: *Walk, field: Ast.full.ContainerField) Error!void {
    if (field.ast.type_expr != 0) {
        try walk_expression(w, field.ast.type_expr); // type
    }
    if (field.ast.align_expr != 0) {
        try walk_expression(w, field.ast.align_expr); // alignment
    }
    if (field.ast.value_expr != 0) {
        try walk_expression(w, field.ast.value_expr); // value
    }
}
...
fn walk_block(
    ...
    for (statements) |stmt| {
        switch (node_tags[stmt]) {
            ...
            .aligned_var_decl,
            => {
                const var_decl = ast.full_var_decl(stmt).?;
                if (var_decl.ast.init_node != 0 and
                    is_undefined_ident(w.ast, var_decl.ast.init_node))
                {
                    ...
                    const name_bytes = ast.token_slice(name_tok);
                    try w.replace_names.put(w.gpa, name_bytes, @int_cast(w.transformations.items.len - 1));
                } else {
                    try walk_local_var_decl(w, var_decl);
                }
            },

            else => {
                switch (categorize_stmt(ast, stmt)) {
...
fn walk_array_type(w: *Walk, array_type: Ast.full.ArrayType) Error!void {
    try walk_expression(w, array_type.ast.elem_count);
    if (array_type.ast.sentinel != 0) {
        try walk_expression(w, array_type.ast.sentinel);
    }
    return walk_expression(w, array_type.ast.elem_type);
}

fn walk_array_init(w: *Walk, array_init: Ast.full.ArrayInit) Error!void {
    if (array_init.ast.type_expr != 0) {
        try walk_expression(w, array_init.ast.type_expr); // T
    }
    for (array_init.ast.elements) |elem_init| {
        try walk_expression(w, elem_init);
    }
}
...
fn walk_struct_init(
    ...
    if (struct_init.ast.type_expr != 0) {
        try walk_expression(w, struct_init.ast.type_expr); // T
    }
    for (struct_init.ast.fields) |field_init| {
        try walk_expression(w, field_init);
    }
}

fn walk_call(w: *Walk, call: Ast.full.Call) Error!void {
    try walk_expression(w, call.ast.fn_expr);
    try walk_param_list(w, call.ast.params);
}
...
fn walk_slice(
    ...
    _ = slice_node;
    try walk_expression(w, slice.ast.sliced);
    try walk_expression(w, slice.ast.start);
    if (slice.ast.end != 0) {
        try walk_expression(w, slice.ast.end);
    }
    if (slice.ast.sentinel != 0) {
        try walk_expression(w, slice.ast.sentinel);
    }
}
...
fn walk_container_decl(
    ...
    if (container_decl.ast.arg != 0) {
        try walk_expression(w, container_decl.ast.arg);
    }
    try walk_members(w, container_decl.ast.members);
}
...
fn walk_builtin_call(
    ...
    const builtin_name = ast.token_slice(builtin_token);
    ...
    switch (info.tag) {
        .import => {
            ...
            if (std.mem.ends_with(u8, token_bytes, ".zig\"")) {
                const imported_string = std.zig.string_literal.parse_alloc(w.arena, token_bytes) catch
                    unreachable;
    ...
    for (params) |param_node| {
        try walk_expression(w, param_node);
    }
}
...

    {
        ...
        while (it.next()) |param| {
            if (param.type_expr != 0) {
                try walk_expression(w, param.type_expr);
            }
        }
    }

    if (fn_proto.ast.align_expr != 0) {
        try walk_expression(w, fn_proto.ast.align_expr);
    }

    if (fn_proto.ast.addrspace_expr != 0) {
        try walk_expression(w, fn_proto.ast.addrspace_expr);
    }

    if (fn_proto.ast.section_expr != 0) {
        try walk_expression(w, fn_proto.ast.section_expr);
    }

    if (fn_proto.ast.callconv_expr != 0) {
        try walk_expression(w, fn_proto.ast.callconv_expr);
    }

    try walk_expression(w, fn_proto.ast.return_type);
}

fn walk_expressions(w: *Walk, expressions: []const Ast.Node.Index) Error!void {
    for (expressions) |expression| {
        try walk_expression(w, expression);
    }
}

fn walk_switch_case(w: *Walk, switch_case: Ast.full.SwitchCase) Error!void {
    for (switch_case.ast.values) |value_expr| {
        try walk_expression(w, value_expr);
    }
    try walk_expression(w, switch_case.ast.target_expr);
}
...
fn walk_while(w: *Walk, node_index: Ast.Node.Index, while_node: Ast.full.While) Error!void {
    ...
    if (!is_true_ident(w.ast, while_node.ast.cond_expr) and
        (while_node.ast.else_expr == 0 or is_empty_block(w.ast, while_node.ast.else_expr)))
    {
        try w.transformations.ensure_unused_capacity(1);
        w.transformations.append_assume_capacity(.{ .replace_with_true = while_node.ast.cond_expr });
    } else if (!is_false_ident(w.ast, while_node.ast.cond_expr) and is_empty_block(w.ast, while_node.ast.then_expr)) {
        try w.transformations.ensure_unused_capacity(1);
        w.transformations.append_assume_capacity(.{ .replace_with_false = while_node.ast.cond_expr });
    } else if (is_true_ident(w.ast, while_node.ast.cond_expr)) {
        try w.transformations.ensure_unused_capacity(1);
        w.transformations.append_assume_capacity(.{ .replace_node = .{
            .to_replace = node_index,
            .replacement = while_node.ast.then_expr,
        } });
    } else if (is_false_ident(w.ast, while_node.ast.cond_expr)) {
        try w.transformations.ensure_unused_capacity(1);
        w.transformations.append_assume_capacity(.{ .replace_node = .{
            .to_replace = node_index,
    ...
    try walk_expression(w, while_node.ast.cond_expr); // condition

    if (while_node.ast.cont_expr != 0) {
        try walk_expression(w, while_node.ast.cont_expr);
    }

    if (while_node.ast.then_expr != 0) {
        try walk_expression(w, while_node.ast.then_expr);
    }
    if (while_node.ast.else_expr != 0) {
        try walk_expression(w, while_node.ast.else_expr);
    }
}

fn walk_for(w: *Walk, for_node: Ast.full.For) Error!void {
    try walk_param_list(w, for_node.ast.inputs);
    if (for_node.ast.then_expr != 0) {
        try walk_expression(w, for_node.ast.then_expr);
    }
    if (for_node.ast.else_expr != 0) {
        try walk_expression(w, for_node.ast.else_expr);
    }
}
...
fn walk_if(w: *Walk, node_index: Ast.Node.Index, if_node: Ast.full.If) Error!void {
    ...
    // 3. If the condition is `if (true)`, replace the `if` expression with the contents of the `then` expression.
    // 4. If the condition is `if (false)`, replace the `if` expression with the contents of the `else` expression.
    if (!is_true_ident(w.ast, if_node.ast.cond_expr) and
        (if_node.ast.else_expr == 0 or is_empty_block(w.ast, if_node.ast.else_expr)))
    {
        try w.transformations.ensure_unused_capacity(1);
        w.transformations.append_assume_capacity(.{ .replace_with_true = if_node.ast.cond_expr });
    } else if (!is_false_ident(w.ast, if_node.ast.cond_expr) and is_empty_block(w.ast, if_node.ast.then_expr)) {
        try w.transformations.ensure_unused_capacity(1);
        w.transformations.append_assume_capacity(.{ .replace_with_false = if_node.ast.cond_expr });
    } else if (is_true_ident(w.ast, if_node.ast.cond_expr)) {
        try w.transformations.ensure_unused_capacity(1);
        w.transformations.append_assume_capacity(.{ .replace_node = .{
            .to_replace = node_index,
            .replacement = if_node.ast.then_expr,
        } });
    } else if (is_false_ident(w.ast, if_node.ast.cond_expr)) {
        try w.transformations.ensure_unused_capacity(1);
        w.transformations.append_assume_capacity(.{ .replace_node = .{
            .to_replace = node_index,
    ...
    try walk_expression(w, if_node.ast.cond_expr); // condition

    if (if_node.ast.then_expr != 0) {
        try walk_expression(w, if_node.ast.then_expr);
    }
    if (if_node.ast.else_expr != 0) {
        try walk_expression(w, if_node.ast.else_expr);
    }
}

fn walk_asm(w: *Walk, asm_node: Ast.full.Asm) Error!void {
    try walk_expression(w, asm_node.ast.template);
    for (asm_node.ast.items) |item| {
        try walk_expression(w, item);
    }
}
...
fn is_fn_body_gutted(ast: *const Ast, body_node: Ast.Node.Index) bool {
    ...
    while (i < statements.len) : (i += 1) {
        switch (categorize_stmt(ast, statements[i])) {
...
fn categorize_stmt(ast: *const Ast, stmt: Ast.Node.Index) StmtCategory {
    ...
    switch (node_tags[stmt]) {
        .builtin_call_two, .builtin_call_two_comma => {
            if (datas[stmt].lhs == 0) {
                return categorize_builtin_call(ast, main_tokens[stmt], &.{});
            } else if (datas[stmt].rhs == 0) {
                return categorize_builtin_call(ast, main_tokens[stmt], &.{datas[stmt].lhs});
            } else {
                return categorize_builtin_call(ast, main_tokens[stmt], &.{ datas[stmt].lhs, datas[stmt].rhs });
            }
        },
        .builtin_call, .builtin_call_comma => {
            const params = ast.extra_data[datas[stmt].lhs..datas[stmt].rhs];
            return categorize_builtin_call(ast, main_tokens[stmt], params);
        },
        .assign => {
            const infix = datas[stmt];
            if (is_discard_ident(ast, infix.lhs) and node_tags[infix.rhs] == .identifier) {
                const name_bytes = ast.token_slice(main_tokens[infix.rhs]);
...
fn categorize_builtin_call(
    ...
    const name_bytes = ast.token_slice(builtin_token);
...
fn is_matching_ident(ast: *const Ast, node: Ast.Node.Index, string: []const u8) bool {
    ...
    switch (node_tags[node]) {
        .identifier => {
            ...
            const name_bytes = ast.token_slice(token_index);
